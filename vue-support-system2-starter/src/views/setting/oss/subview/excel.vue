<template>
  <div class="content">
    ...
    <div class="btn-group">
      <button v-for="sheet in sheets" :key="sheet.id" style="padding: 0 30px"
        :type="sheetIndex === sheet.id ? 'primary' : 'default'" @click="handleSheet(sheet.id)">
        {{ sheet.name }}
      </button>
    </div>
  </div>
...
import { indexedColors } from "@/utils/color";
import { camelCase, captain, fixMatrix, load } from "@/utils/excel";
...
export default {
  ...
  created() {
    this.initial().then(res => {
      new Promise((r, j) => {
        debugger
        const wb = new ExcelJS.Workbook();
        this.workbook = wb.xlsx.load(new Uint8Array(res));
        debugger
        // 注册自定义渲染
  ...
  computed: {
    ...
    sheets() {
      if (this.workbook && this.workbook.worksheets) {
  ...
  methods: {
    ...
    async initial() {
      return await this.$API.common.blob.get(getQueryString('url') + (getQueryString('bucket') || '') + (getQueryString('id') || '') + '?fromPath=' + (getQueryString('fromPath') || ''));
    },
    hotTable() {

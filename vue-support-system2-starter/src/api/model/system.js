...
export default {
	mapping : {
		...
		save: {
			url: `${config.API_URL}/v1/mapping/save`,
			name: "添加映射",
		...
		update: {
			url: `${config.API_URL}/v1/mapping/update`,
			name: "更新映射",
			put: function(params){
		...
		delete: {
			url: `${config.API_URL}/v1/mapping/delete`,
			name: "删除映射",
			delete: function(p){
				return http.delete(this.url + `?mapId=${p.libId}`, {params: p});
			}
		}
	},
	library: {

<template>
	<el-dialog title="上传数据文件" v-model="visible" :width="500" destroy-on-close @closed="$emit('closed')" draggable>
...
export default {
	...
	methods: {
		...
		async submit() {
			...
			const row = { "getDatabaseFile": this.getDriverFile };
			Object.assign(row, this.form);
			var res = await this.$API.gen.database.uploadDriver.post(row);

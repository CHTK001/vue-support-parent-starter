      ...
      <el-form ref="dialogForm" :status-icon="true" :model="form" :rules="rules1" :disabled="mode == 'show'" label-width="160px" label-position="left">
        ...
        <div v-if="form.limitType == 0">
          <el-form-item label="限流地址" prop="limitUrl">
            <el-input v-model="form.limitUrl" clearable placeholder="请输入限流地址" />
          </el-form-item>
        </div>
        <div v-else>
          <el-form-item label="限流地址" prop="limitAddress">
            <ip-input v-model="form.limitAddress" />
          </el-form-item>
        </div>
        <el-form-item label="每秒访问次数" prop="limitPermitsPerSecond">
          <el-input v-model="form.limitPermitsPerSecond" type="number" clearable placeholder="请输入限流次数" />
        </el-form-item>
        <el-form-item label="是否开启" prop="limitDisable">
          <el-switch v-model="form.limitDisable" clearable :active-value="1" :inactive-value="0" />
        </el-form-item>
      </el-form>
      <template #footer>

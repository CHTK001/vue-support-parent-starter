    ...
    <el-dialog v-model="visible" :title="title" :width="700" destroy-on-close draggable @closed="$emit('closed')">
      <div style="padding: 12px">
        <el-form :model="config">
          <div v-for="row in baseConfig" :key="row">
            <el-form-item :label="row.description || row.name" :prop="row.name">
              <el-row style="width: 100%">
                <el-col :span="12">
                  <el-switch v-if="row.type == 'BOOLEAN'" v-model="databaseConfig[row.name]" active-value="true" inactive-value="false" />
                  <el-input v-else-if="row.type == 'STRING'" v-model="databaseConfig[row.name]" />
                  <el-input v-else-if="row.type == 'NUMBER'" v-model="databaseConfig[row.name]" type="number" />
                  <el-input v-else-if="row.type == 'JSON'" v-model="databaseConfig[row.name]" type="textarea" />
                </el-col>
                <el-col :span="12">
                  <el-button title="保存" type="primary" icon="el-icon-lock" style="margin-left: 10px" @click="saveConfig(row.name, databaseConfig[row.name], row.description, row.type)" />
                </el-col>
              </el-row>
            </el-form-item>
          </div>
...
export default {
  ...
  methods: {
    ...
    saveConfig(key, value, desc, type) {
      ...
      this.$API.proxy_config.setting.save
        .post({

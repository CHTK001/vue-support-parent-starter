<template>
  ...
  <div class="map-counter-wrap">
    <div v-for="(item, index) in state.counterList" :key="index" class="counter-item">
      <div class="content">
        <DigitalTransform class="num" :value="item.num" :useGrouping="false" :interval="2000" />
      </div>
      <div class="name">
        {{ item.name }}
        <span class="unit">({{ item.unit }})</span>
      </div>
      <echartLine :barColor="item.barColor" />
    </div>
  </div>
</template>
<script setup>

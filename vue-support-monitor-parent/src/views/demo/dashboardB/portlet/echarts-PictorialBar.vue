...
const processData = () => {
  let { legend, colors, colorsD, yAxis, data } = state.chartData,
    processedData1 = {
      ...
      itemStyle: {
        color: item => {
          return $c.fade(colorsD[item.dataIndex], 0.8);
        }
      },
      data: []
    },
    processedData2 = {
      ...
      itemStyle: {
        color: item => {
          return $c.fade(colors[item.dataIndex], 0.8);
        }
      },
      data: [],
  ...
  data.forEach((item, i) => {
    processedData1.data.push({ value: 100, symbol: symbol[0] });
    processedData2.data.push({ value: item, symbol: symbol[0] });
  });
...
<template>
  <echartsInit :chartOption="state.chartOption" />
</template>

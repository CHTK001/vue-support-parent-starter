...
export default {
	...
	methods: {
		async doSearch(conf) {
			...
			this.$API.spider.config.page.get(this.form).then((res) => {
				if (res.code === '00000') {
		...
		del(row) {
			this.$confirm(`确认删除 ${row.taskName} 计划任务吗？`, '提示', {
				...
				this.$API.spider.config.delete.delete({ taskTid: row.taskTid }).then(res => {
					if (res.code !== '00000') {
						this.$notify.error({ title: '提示', message: res.msg })
						return !1;
					}
		...
		run(row) {
			this.$API.spider.config.run.get(row).then(res => {
				let type = 'success';
				if (res.code !== '00000') {
					type = 'error';
				} else {

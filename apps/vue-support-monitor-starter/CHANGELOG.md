...
### 🐳 Docker 软件库 - 在线搜索与多默认仓库

- 仓库支持设置多个默认；再次点击星标可取消默认
- 软件库新增「在线搜索」按钮，弹出在线搜索弹框
- 在线搜索支持关键词输入自动检索（400ms 防抖），仅检索默认仓库（后端SPI实现）
- 支持将在线检索结果异步保存到软件库
- 文档补充在线搜索与默认仓库 API 规范（docs/API_SOFT_MANAGEMENT.md）

          ...
          <a-row justify="end">
            <a-form layout="inline">
              <a-space>
                ...
                <a-form-item>
                  <a-tooltip
                    v-if="!props.isHideRefresh"
                    :title="$t('i18n_694fc5efa9')"
                  >
                    <ReloadOutlined
                    ...
                    <template #title>
                      <div class="custom-column-list__title">
                        ...
                        <a-button
                          type="link"
                          size="small"
                          @click="resetCustomColumn"
                          >{{ $t("i18n_4b9c3271dc") }}</a-button
                        >
                      </div>
                    </template>
                    <template #content>
                        ...
                        >
                          ...
                          >
                            <VerticalLeftOutlined
                              v-if="!!item.fixed"
                              class="custom-column-list__icon"
                            />
                            <HolderOutlined
                              v-else
                              class="custom-column-list__icon"
              ...
              <a-row :gutter="[16, 16]">
                <template v-if="props.dataSource && props.dataSource.length">
                  <a-col
                    ...
                    :span="6"
                  >
                    <slot name="cardBodyCell" :item="item" :index="index" />
                  </a-col>
                </template>
...
export default defineComponent({
  ...
  setup(props, { attrs, slots, emit }) {
    ...
    const { autoRefreshTime, isAutoRefresh } =
      storageService.getRefreshConfig();
    // 倒计时
    const getCountdown = () => {
      return (
        Date.now() +
        1000 *
          (autoRefreshTime !== -1 ? autoRefreshTime : props.autoRefreshTime + 0)
      );
    };
    const countdownSwitch = ref(
      isAutoRefresh !== -1 ? isAutoRefresh : props.defaultAutoRefresh,
    );
    ...
    const canChangeLayout = computed(() => {
      ...
      return (
        Object.keys(slots).filter((key) => key === "cardBodyCell").length > 0
      );
    });
    ...
    const customColumn = computed(() => {
      ...
      return customColumnList.value.filter(
        (item: CustomColumnType) => item.checked,
      );
    });
    const resetCustomColumn = () => {
      customColumnList.value = props.columns.map((item: CustomColumnType) => ({
        ...item,
        checked: true,
      }));
    };
    const onCheckChange = (checkedValues: CheckboxValueType[]) => {
      customColumnList.value = customColumnList.value.map(
        (item: CustomColumnType) => ({
          ...item,
    ...
    const onDrop = (dropResult: any) => {
      customColumnList.value = dropApplyDrag<CustomColumnType>(
    ...
    watch(
      ...
      (val) => {
        ...
        if (
          ...
          (catchStorage.length > 0 &&
            catchStorage.some((key) => typeof key === "string")) ||
          !compareArrays(

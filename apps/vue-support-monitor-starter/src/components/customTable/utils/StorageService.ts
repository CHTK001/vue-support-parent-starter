...
import type { CustomColumnType, TableLayoutType } from "../types";
...
export type StorageServiceOptions = {
  ...
  prefix: string;
  beforeStorage?: (
    storageObject: StorageObjectType,
    defaultConfig: StorageObjectType,
  ) => StorageObjectType;
};
...
export class StorageService {
  ...
  setStorage(path: string, value: any) {
    ...
    let storageObject = this.getStorage<StorageObjectType>(
      "",
      getDefaultConfig(),
    );
    ...
    for (let i = 0; i < keys.length - 1; i++) {
      if (typeof currentObj !== "object" || currentObj === null) {
        throw new Error(
          `Cannot set property '${path}' on object. Intermediate property does not exist.`,
        );
      }
      const key = keys[i];
    ...
    if (this.beforeStorage) {
      ...
      if (
        JSON.stringify(storageObject) === JSON.stringify(getDefaultConfig())
      ) {
        isDel = true;
      }
    }
  ...
  getTableSizeConfig() {
    return this.getStorage<StorageObjectType["tableSize"]>(
      "tableSize",
      undefined,
    );
  }

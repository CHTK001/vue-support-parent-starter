<template>
  <div>
    <a-timeline>
        ...
        <a-space>
          ...
          <template v-if="temp.upgrade !== undefined">
            <a-tag v-if="temp.upgrade" color="pink" @click="upgrageVerion"
              >{{ $t("i18n_ac2f4259f1") }}{{ temp.newVersion }}
              {{ temp.newBeta ? "/beta" : "" }}
              <DownloadOutlined />
            </a-tag>
      ...
      <a-timeline-item>
        <span
          >{{ $t("i18n_b57ecea951") }}{{ formatDuration(temp.upTime) }}</span
        >
      </a-timeline-item>
      <a-timeline-item>
        <span>
      ...
      <a-col span="22">
        <a-space direction="vertical" style="width: 100%">
          ...
          >
            ...
            <a-button v-else
              ><UploadOutlined />{{ $t("i18n_d615ea8e30") }}</a-button
            >
          </a-upload>
          <a-row v-if="percentage">
            <a-col span="20">
              <a-progress :percent="percentage" style="width: 100%" />
            </a-col>
          </a-row>
          <a-button
            type="primary"
            :disabled="fileList.length === 0 || !!percentage"
            @click="startUpload"
            >{{ $t("i18n_3dd6c10ffd") }}</a-button
          >
        </a-space>
      </a-col>
    </a-row>
...
export default {
  ...
  methods: {
    ...
    renderMarkdown(markdown) {
      return (
        (this.markdownit && this.markdownit.render(markdown)) ||
        this.$t("i18n_aeade8e979")
      );
    },
    // 加载数据
    loadData() {
    ...
    startUpload() {
      ...
      const liArray = [
        this.$t("i18n_a38ed189a2"),
        this.$t("i18n_a5daa9be44"),
        this.$t("i18n_a52a10123f"),
      ];
      ...
      this.timer = setInterval(() => {
        systemInfo({
          ...
          .then((res) => {
            ...
            if (
              res.code === 200 &&
              manifest?.timeStamp !== this.temp.timeStamp
            ) {
              this.globalLoading({
                spinning: false,
              });
          ...
          .catch((error) => {
            ...
            if (this.checkCount > RESTART_UPGRADE_WAIT_TIME_COUNT) {
              ...
              this.globalLoading({
                spinning: true,
                tip:
                  (msg || this.$t("i18n_589060f38e")) +
                  this.$t("i18n_9ba71275d3"),
              });
            }
          });
        ...
        if (tip) {
          $notification.success({
            message: this.temp.upgrade
              ? this.$t("i18n_2314f99795") + data.tagName
              : this.$t("i18n_e6cde5a4bc"),
          });
        }
    ...
    upgrageVerion() {
      ...
      const alterB = this.$t("i18n_ddf0c97bce");
      const li = [
        this.$t("i18n_a94feac256"),
        this.$t("i18n_b55f286cba"),
        this.$t("i18n_a52a10123f"),
      ];
    ...
    handleChangeBetaRelease(beta) {
      ...
      if (beta) {
        ...
        const li = [
          this.$t("i18n_d31d625029"),
          this.$t("i18n_d8db440b83"),
          this.$t("i18n_73b7e8e09e"),
        ];

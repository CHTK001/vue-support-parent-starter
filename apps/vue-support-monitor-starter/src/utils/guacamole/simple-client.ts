...
export class SimpleGuacamoleClient {
  ...
  constructor(
    config: SimpleGuacamoleConfig,
    events: SimpleGuacamoleEvents = {},
  ) {
    this.config = config;
    this.events = events;
  ...
  async connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (
          this.client &&
          this.client.currentState === Guacamole.Client.CONNECTED
        ) {
          resolve();
          return;
        }
  ...
  private hasConfigChanges(newConfig: Partial<SimpleGuacamoleConfig>): boolean {
    ...
    for (const key of importantKeys) {
      if (newConfig[key] !== undefined && newConfig[key] !== this.config[key]) {
        console.log(
          `配置变更检测: ${key} 从 ${this.config[key]} 变更为 ${newConfig[key]}`,
        );
        return true;
      }
    }
  ...
  isConnected(): boolean {
    return (
      this.client && this.client.currentState === Guacamole.Client.CONNECTED
    );
  }

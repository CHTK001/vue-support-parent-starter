        ...
        <a-timeline>
          ...
          <a-timeline-item v-if="temp.fileSize">
            <a-space>
              ...
              <a-button size="small" type="primary" class="btn" @click="clear('fileSize')">{{ $t("i18n_288f0c404c") }}</a-button>
            </a-space>
          </a-timeline-item>
          <a-timeline-item v-if="temp.oldJarsSize">
            <a-space>
              <span>{{ $t("i18n_413d8ba722") }}{{ renderSize(temp.oldJarsSize) }}</span>
              <a-button size="small" type="primary" class="btn" @click="clear('oldJarsSize')">{{ $t("i18n_288f0c404c") }}</a-button>
            </a-space>
          </a-timeline-item>
          ...
          <a-timeline-item>
            <a-space>
              ...
              <a-button v-if="temp.pidPort" size="small" type="primary" class="btn" @click="clear('pidPort')">{{ $t("i18n_288f0c404c") }}</a-button>
            </a-space>
          </a-timeline-item>
          <a-timeline-item>
            ...
            <template #renderItem="{ item }">
              <a-list-item>
                <a-space>
                  ...
                  <a-button type="primary" size="small" danger @click="openCorrectLonely(item, 'project')">{{ $t("i18n_23231543a4") }}</a-button>
                </a-space>
              </a-list-item>
            </template>
            <template #header>
            ...
            <template #renderItem="{ item }">
              <a-list-item>
                <a-space>
                  ...
                  <span>{{ $t("i18n_e0fcbca309") }}{{ item.workspaceId }}</span>
                  <span>{{ $t("i18n_2256690a28") }}{{ item.nodeId }}</span>
                  <a-button type="primary" size="small" danger @click="openCorrectLonely(item, 'script')">{{ $t("i18n_23231543a4") }}</a-button>
                </a-space>
              </a-list-item>
            </template>
            <template #header>
      ...
      <a-space direction="vertical" style="width: 100%">
        ...
        <a-form :model="temp" :label-col="{ span: 6 }" :wrapper-col="{ span: 14 }">
          <a-form-item :label="$t('i18n_7e2b40fc86')" name="nodeId">
            ...
            >
              <a-select-option v-for="item in nodeList" :key="item.id">【{{ item.workspace && item.workspace.name }}】{{ item.name }}</a-select-option>
            </a-select>
          </a-form-item>
        </a-form>
      </a-space>
...
import TaskStat from "@/views/maintenance/system/taskStat.vue";

<template>
  <div>
    <CustomModal
      ...
      "
    >
      <a-alert v-if="!nodeList || !nodeList.length" :message="$t('i18n_4b027f3979')" type="warning" show-icon style="margin-bottom: 10px">
      ...
      <a-form ref="editScriptForm" :rules="rules" :model="temp" :label-col="{ span: 3 }" :wrapper-col="{ span: 19 }">
        ...
        <a-form-item :label="$t('i18n_7e2b40fc86')">
          <a-select v-model:value="temp.nodeId" :disabled="!!temp.nodeId" allow-clear :placeholder="$t('i18n_f8a613d247')">
        ...
        <template v-if="temp.nodeId">
          ...
          <a-form-item :label="$t('i18n_3bdd08adab')" name="description">
            <a-textarea v-model:value="temp.description" :rows="3" style="resize: none" :placeholder="$t('i18n_ae653ec180')" />
          </a-form-item>
        </template>
      </a-form>
    </CustomModal>

<template>
  <div class="">
              ...
              <template #overlay>
                <a-menu>
                  <a-menu-item>
                    ...
                    <a-tooltip :title="`${record.scriptType === 'server-sync' ? $t('i18n_1f0d13a9ad') : $t('i18n_2f4aaddde3')}`">
                      <a-button size="small" :disabled="record.scriptType === 'server-sync'" type="primary" danger @click="handleDelete(record)">{{ $t("i18n_2f4aaddde3") }}</a-button>
                    </a-tooltip>
                  </a-menu-item>
                  <a-menu-item>
    ...
    <CustomDrawer
      ...
      "
    >
      <script-console v-if="drawerConsoleVisible" :id="temp.id" :node-id="temp.nodeId" :def-args="temp.defArgs" :script-id="temp.scriptId" />
    </CustomDrawer>
    <!-- 脚本日志 -->
    <CustomDrawer
    ...
    <!-- 触发器 -->
    <CustomModal v-if="triggerVisible" v-model:open="triggerVisible" destroy-on-close :title="$t('i18n_4696724ed3')" width="50%" :footer="null">

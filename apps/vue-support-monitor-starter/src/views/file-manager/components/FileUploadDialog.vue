...
const beforeUpload = (file: File) => {
  ...
  if (file.size > maxSize) {
    message(`文件 ${file.name} 大小超过 ${maxSizeText} 限制`, {
      type: "error",
    });
    return false;
  }
...
const handleUploadSuccess = (response: any, file: UploadFile) => {
  ...
  if (uploadingFiles.length === 0) {
    ...
    if (errorCount === 0) {
      ...
      message(`上传完成: 成功 ${successCount}个, 失败 ${errorCount}个`, {
        type: "warning",
      });
    }
  }
...
const updateUploadStats = () => {
  ...
  uploadStats.success = fileList.value.filter(
    (f) => f.status === "success",
  ).length;
...
const handleClose = () => {
  if (isUploading.value) {
    ElMessageBox.confirm("上传正在进行中，确定要关闭吗？", "确认关闭", {

<template>
  <el-dialog
    ...
    :title="`配置 ${filterSetting?.systemServerSettingName}`"
    width="800px"
  ...
  >
    <div class="config-content" v-if="filterSetting">
      <el-alert
        :title="`正在配置 ${filterSetting.systemServerSettingType} 类型的ServletFilter`"
        type="info"
        :closable="false"
        show-icon
      ...
      >
        ...
        <el-form-item label="Filter描述">
          <el-input
            v-model="filterSetting.systemServerSettingDescription"
            type="textarea"
            :rows="2"
        ...
        <el-form-item label="启用状态">
          <el-switch
            v-model="filterSetting.systemServerSettingEnabled"
            active-text="启用"
            inactive-text="禁用"
          />
        </el-form-item>
...
const resetToDefault = async () => {
  ...
  try {
    const response = await resetServletFilterConfig(
      props.filterSetting.systemServerSettingId
    );
    if (response.success) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await updateServletFilterConfig(
      props.filterSetting.systemServerSettingId,
      config
    );

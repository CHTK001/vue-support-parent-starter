        ...
        <el-card class="installed-card" shadow="never">
          <template #header>
            <div class="card-header-row">
              ...
              <div class="header-actions">
                <el-button type="primary" size="small" @click="onAddClick"><IconifyIconOnline icon="ri:add-line" />新增存储</el-button>
              </div>
            </div>
          </template>
          ...
          <el-scrollbar v-else class="installed-list">
            ...
            >
              ...
              <div class="item-sub">
                <span v-if="s.fileStorageType === 'LOCAL'">{{ s.fileStorageBasePath || '未配置路径' }}</span>
                <span v-else>{{ s.fileStorageBucket || '-' }} @ {{ s.fileStorageEndpoint || '-' }}</span>
              </div>
              <div class="item-actions" @click.stop>
                <el-switch v-model="s.fileStorageEnabled" size="small" />
                <el-button type="danger" link size="small" @click="removeStorage(idx)"><IconifyIconOnline icon="ri:delete-bin-line" />移除</el-button>
              </div>
            </div>
          </el-scrollbar>
        </el-card>
...
type SpiOption = { name: string; describe?: string; value?: string; label?: string };
...
async function loadOptionalProviderOptions() {
  try {
    ...
    if (settingRes?.success) {
      ...
      const list = raw.map((it: any) => (typeof it === 'string' ? { name: it } : { name: it?.name ?? it?.value ?? it?.label, describe: it?.describe ?? it?.label }))
    ...
    if (global.value.settingsStr) imageSettingSelection.value = global.value.settingsStr.split(',').map(s => s.trim()).filter(Boolean);
    if (global.value.pluginsStr) imageFilterSelection.value = global.value.pluginsStr.split(',').map(s => s.trim()).filter(Boolean);
  } catch {}
}
...
async function handleSave() {
  ...
  for (let i = 0; i < storages.value.length; i++) {
    ...
    if (!s.fileStorageType) { ElMessage.error(`存储 #${i + 1} 未选择类型`); return; }
...
.item-title .seq { color:#64748b }
.storage-form { padding: 8px 8px 0 8px }
.kv-list { padding: 0 8px }
...
.installed-item .item-title { grid-column: 1 / -1; display: flex; align-items: center; gap: 8px; font-weight: 500 }
.installed-item .item-sub { color: #64748b; font-size: 12px }

<template>
  ...
  >
    <template #before>
      <a-space>
        <span v-if="status">
          {{ $t("i18n_e703c7367c") }}
          <a-tooltip :title="`${$t('i18n_e703c7367c')} ${statusMap[status]} ${statusMsg ? $t('i18n_8d13037eb7') + statusMsg : ''}`">
  ...
  <CustomModal v-if="environmentVisible" v-model:open="environmentVisible" destroy-on-close :title="$t('i18n_aacd9caa4a')" :footer="null" :mask-closable="false" width="50%">

  ...
  <div>
    <a-space direction="vertical" style="width: 100%">
      <a-descriptions bordered size="small">
        ...
        <a-descriptions-item :label="$t('i18n_6f854129e9')">{{ data.branchName }} {{ data.branchTagName }}</a-descriptions-item>
        ...
        <a-descriptions-item :label="$t('i18n_f98994f7ec')">{{ releaseMethodMap[data.releaseMethod] }}</a-descriptions-item>
        ...
        <a-descriptions-item :label="$t('i18n_4b96762a7e')">
          {{ parseTime(data.modifyTimeMillis) }}
        </a-descriptions-item>
        ...
        <a-descriptions-item :label="$t('i18n_86e9e4dd58')" :span="3">{{ data.repositoryLastCommitId }}</a-descriptions-item>
      </a-descriptions>
            ...
            <a-space direction="vertical" style="width: 100%">
              ...
              <div v-if="item.repositoryLastCommitId">
                <div>
                  ...
                  <span v-if="item.repositoryLastCommitMsg">{{ $t("i18n_f27822dd8a") }}{{ item.repositoryLastCommitMsg || "" }}</span>
                </div>
              </div>
              <div>
              ...
              <div>{{ $t("i18n_b5a1e1f2d1") }}{{ triggerBuildTypeMap[item.triggerBuildType] || $t("i18n_1622dc9b6b") }}</div>
              <div>{{ $t("i18n_8dbe0c2ffa") }}{{ renderSize(item.resultFileSize) }}({{ $t("i18n_7dfcab648d") }})/{{ renderSize(item.buildLogFileSize) }}({{ $t("i18n_456d29ef8b") }})</div>

              <div>{{ $t("i18n_3c014532b1") }}{{ formatDuration((item.endTime || 0) - (item.startTime || 0), "", 2) }}</div>
              <div>
                {{ $t("i18n_e8321f5a61") }}
              ...
              <div>
                ...
                <a-space>
                  ...
                  <template v-if="item.releaseMethod !== 5">
                    <a-button size="small" :disabled="!item.hasFile || item.releaseMethod === 0" type="primary" danger @click="handleRollback(item)">{{ $t("i18n_d00b485b26") }}</a-button>
                  </template>
                  <template v-else>
                    <a-tooltip :title="$t('i18n_2d94b9cf0e')">
                      <a-button size="small" :disabled="true" type="primary" danger>{{ $t("i18n_d00b485b26") }}</a-button>
                    </a-tooltip>
                  </template>
                </a-space>
              </div>

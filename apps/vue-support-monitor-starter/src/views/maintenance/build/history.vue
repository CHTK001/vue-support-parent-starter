    ...
    >
      <template #title>
        <a-space wrap class="search-box">
          <a-input v-model:value="listQuery['%buildName%']" allow-clear class="search-input-item" :placeholder="$t('i18n_50a299c847')" @press-enter="loadData" />
          <a-select
          ...
          <a-button type="primary" danger :disabled="!tableSelections || tableSelections.length <= 0" @click="handleBatchDelete">
            {{ $t("i18n_7fb62b3011") }}
          </a-button>
        </a-space>
      </template>
      ...
      <template #tableBodyCell="{ column, text, record }">
        ...
        <template v-else-if="column.dataIndex === 'buildNumberId'">
          <a-tooltip :title="text + `( ${$t('i18n_aac62bc255')} )`">
            <a-tag color="#108ee9" @click="handleBuildLog(record)">
              #{{ text }}
              <template v-if="record.fromBuildNumberId">&lt;-{{ record.fromBuildNumberId }}</template>
            </a-tag>
          </a-tooltip>
        </template>
        ...
        <template v-else-if="column.dataIndex === 'resultFileSize'">
          <a-tooltip :title="`${$t('i18n_16646e46b1')}${renderSize(record.resultFileSize)}， ${$t('i18n_77e501b44b')} ${renderSize(record.buildLogFileSize)}`">
        ...
        <template v-else-if="column.dataIndex === 'endTime'">
          <a-tooltip :title="`${$t('i18n_61e84eb5bb')}${parseTime(record.startTime)}，${record.endTime ? $t('i18n_590dbb68cf') + parseTime(record.endTime) : ''}`">
                ...
                <a-menu>
                  <a-menu-item>
                    <template v-if="record.releaseMethod !== 5">
                      <a-button size="small" :disabled="!record.hasFile || record.releaseMethod === 0" type="primary" danger @click="handleRollback(record)">{{ $t("i18n_d00b485b26") }}</a-button>
                    </template>
                    <template v-else>

<template>
  <div>
    ...
    >
      <template #title>
        <a-space wrap class="search-box">
          <a-input v-model:value="listQuery['%name%']" class="search-input-item" :placeholder="$t('i18n_f967131d9d')" @press-enter="loadData" />
          <a-input v-model:value="listQuery['%gitUrl%']" class="search-input-item" :placeholder="$t('i18n_e4bea943de')" @press-enter="loadData" />
          <a-select v-model:value="listQuery.repoType" allow-clear :placeholder="$t('i18n_4ce606413e')" class="search-input-item">
      ...
      <template #toolPrefix>
        ...
        <a-dropdown>
          ...
          <a-upload name="file" accept=".csv" action="" :show-upload-list="false" :multiple="false" :before-upload="beforeUpload">
      ...
      <template #tableBodyCell="{ column, text, record, index }">
        ...
        <template v-else-if="column.dataIndex === 'operation'">
          <a-space>
            ...
            <a-button v-if="global" type="primary" size="small" @click="viewBuild(record)">{{ $t("i18n_1c3cf7f5f0") }}</a-button>
    ...
    <!-- 编辑区 -->
    <CustomModal v-if="editVisible" v-model:open="editVisible" destroy-on-close :confirm-loading="confirmLoading" :title="$t('i18n_ed39deafd8')" :mask-closable="false" width="60%" @ok="handleEditOk">
      <a-form ref="editForm" :rules="rules" :model="temp" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
        ...
        <a-form-item :label="$t('i18n_829abe5a8d')" name="group">
          <custom-select v-model:value="temp.group" :data="groupList" :input-placeholder="$t('i18n_bd0362bed3')" :select-placeholder="$t('i18n_9cac799f2f')">
            <template #suffix>
              <a-tooltip>
                <template #title>
                  <div>
        ...
        <a-form-item :label="$t('i18n_e4bea943de')" name="gitUrl">
          <a-input-group compact>
            <a-form-item-rest>
              <a-select v-model:value="temp.repoType" style="width: 20%" name="repoType" :placeholder="$t('i18n_4ce606413e')">
                <a-select-option :value="0">GIT</a-select-option>
                <a-select-option :value="1">SVN</a-select-option>
              </a-select>
            </a-form-item-rest>
            <a-input v-model:value="temp.gitUrl" style="width: 80%" :max-length="250" :placeholder="$t('i18n_e4bea943de')" />
          </a-input-group>
        </a-form-item>
          ...
          <a-form-item name="password">
            ...
            <template #help>
              <a-tooltip v-if="temp.id" :title="$t('i18n_b408105d69')">
                <a-button style="margin: 5px" size="small" type="primary" danger @click="restHideField(temp)">{{ $t("i18n_4403fca0c0") }}</a-button>
              </a-tooltip>
            </template>
          </a-form-item>
        </template>
        <a-form-item v-if="temp.repoType === 1 && temp.protocol === 1" :label="$t('i18n_7035c62fb0')" name="userName">
          <a-input v-model:value="temp.userName" :placeholder="$t('i18n_f04a289502')">
            ...
            <template #suffix>
              <a-tooltip v-if="temp.id" :title="$t('i18n_b408105d69')">
                <a-button size="small" type="primary" danger @click="restHideField(temp)">{{ $t("i18n_4403fca0c0") }}</a-button>
              </a-tooltip>
            </template>
          </a-input>
        </a-form-item>
          ...
          <a-form-item :label="$t('i18n_d0eddb45e2')" name="rsaPrv">
            <a-tooltip placement="topLeft">
              <template #title>
                <div>
                  <p style="color: #faa">
                    {{ $t("i18n_43c61e76e7") }} "{{ $t("i18n_3bc5e602b2") }}" {{ $t("i18n_9e560a4162") }}
                    <br />
                    {{ $t("i18n_8c66392870") }} "{{ $t("i18n_3bc5e602b2") }}" {{ $t("i18n_d0a864909b") }}
                    <br />
                  </p>
                  ...
                  <p>
                    ...
                    <br />
                    file:/Users/Hotstrip/.ssh/id_rsa
                  </p>
                </div>
              </template>
              <a-textarea v-model:value="temp.rsaPrv" :auto-size="{ minRows: 3, maxRows: 3 }" :placeholder="$t('i18n_d7ee59f327')" />
            </a-tooltip>
          </a-form-item>
          <!-- 公钥暂时没用到 -->
          <a-form-item v-if="false" :label="$t('i18n_b939d47e23')" name="rsaPub">
            <a-textarea v-model:value="temp.rsaPub" :auto-size="{ minRows: 3, maxRows: 3 }" :placeholder="$t('i18n_db686f0328')" />
          </a-form-item>
        </template>
        ...
        <a-form-item :label="$t('i18n_67425c29a5')" name="timeout">
          <a-input-number v-model:value="temp.timeout" :min="0" :placeholder="$t('i18n_ea9f824647')" style="width: 100%" />
        </a-form-item>
      </a-form>
    </CustomModal>
    <CustomModal v-if="giteeImportVisible" v-model:open="giteeImportVisible" destroy-on-close :title="$t('i18n_c8633b4b77')" width="80%" :footer="null" :mask-closable="false">
      <a-form ref="giteeImportForm" :label-col="{ span: 4 }" :rules="giteeImportFormRules" :model="giteeImportForm" :wrapper-col="{ span: 20 }">
        <a-form-item name="token" :label="$t('i18n_8ba971a184')" :help="$t('i18n_e30a93415b')">
          <a-form-item-rest>
            <a-tooltip :title="`${giteeImportForm.type} ${$t('i18n_32d0576d85')}${importTypePlaceholder[giteeImportForm.type]}`">
              <a-input-group compact>
        ...
        <a-form-item v-if="providerData[giteeImportForm.type]?.query" name="condition" :label="$t('i18n_e5f71fc31e')" :help="$t('i18n_bf0e1e0c16', { slot1: $t('i18n_e5f71fc31e') })">
          <a-input v-model:value="giteeImportForm.condition" :placeholder="$t('i18n_e72f2b8806')" />
        </a-form-item>
      </a-form>
      <a-table :loading="importLoading" size="middle" :columns="reposColumns" :data-source="repos" bordered row-key="full_name" :pagination="reposPagination" @change="reposChange">
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-else-if="column.dataIndex === 'operation'">
            <a-button type="primary" size="small" :disabled="record.exists" @click="handleGiteeRepoAdd(record)">{{ record.exists ? $t("i18n_cb951984f2") : $t("i18n_66ab5e9f24") }}</a-button>
          </template>
        </template>
      </a-table>
    </CustomModal>
    ...
    <!-- </div> -->
    <!-- 关联构建 -->
    <CustomModal v-if="viewBuildVisible" v-model:open="viewBuildVisible" destroy-on-close width="80vw" :title="$t('i18n_1c13276448')" :mask-closable="false" :footer="null">

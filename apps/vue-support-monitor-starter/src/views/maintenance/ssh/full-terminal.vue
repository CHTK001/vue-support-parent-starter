<template>
  <div>
        ...
        <template #title>
          <template v-if="sshData">
            <a-space>
              ...
              <a-button size="small" type="primary" :disabled="!sshData.fileDirs" @click="handleFile()">{{ $t("i18n_2a0c4740f1") }}</a-button>
            </a-space>
          </template>
    ...
    <!-- 文件管理 -->
    <CustomDrawer v-if="sshData && drawerVisible" destroy-on-close placement="right" width="90vw" :open="drawerVisible" @close="onClose">
      <template #title>
        {{ sshData.name }}
        <template v-if="sshData.host">({{ sshData.host }})</template>
        {{ $t("i18n_8780e6b3d1") }}

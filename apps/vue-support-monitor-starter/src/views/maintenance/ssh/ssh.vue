<template>
  <div>
      ...
      <a-result :title="$t('i18n_ce043fac7d')" :sub-title="$t('i18n_ace71047a0')">
        <template #extra>
          <router-link to="/system/assets/ssh-list">
            <a-button key="console" type="primary">{{ $t("i18n_6dcf6175d8") }}</a-button>
          </router-link>
        </template>
      </a-result>
    </template>
    ...
    >
      <template #title>
        <a-space wrap class="search-box">
          <a-input v-model:value="listQuery['%name%']" class="search-input-item" :placeholder="$t('i18n_46ad87708f')" @press-enter="loadData" />
          <a-select
        ...
        <template v-else-if="column.dataIndex instanceof Array && column.dataIndex.includes('status')">
          <a-tooltip :title="record.machineSsh && record.machineSsh.statusMsg">
            <a-tag :color="statusMap[record.machineSsh && record.machineSsh.status] && statusMap[record.machineSsh && record.machineSsh.status].color">
              {{ (statusMap[record.machineSsh && record.machineSsh.status] && statusMap[record.machineSsh && record.machineSsh.status].desc) || $t("i18n_1622dc9b6b") }}
            </a-tag>
          </a-tooltip>
        </template>
        <template v-else-if="column.dataIndex instanceof Array && column.dataIndex.includes('osName')">
          <a-popover :title="$t('i18n_b7ea5e506c')">
            <template #content>
              ...
              <p>{{ $t("i18n_8a745296f4") }}{{ formatDuration(record.machineSsh && record.machineSsh.osSystemUptime) }}</p>
            </template>
        ...
        <template v-else-if="column.dataIndex instanceof Array && column.dataIndex.includes('osMaxOccupyDisk')">
          <a-popover :title="$t('i18n_a74b62f4bb')">
            <template #content>
              ...
              <p>{{ $t("i18n_de17fc0b78") }}{{ formatPercent(record.machineSsh && record.machineSsh.osMaxOccupyDisk) }}</p>
        ...
        <template v-else-if="column.dataIndex === 'nodeId'">
          <template v-if="record.linkNode">
            <a-tooltip placement="topLeft" :title="`${$t('i18n_5d83794cfa')}${record.linkNode.name}`">
              <a-button size="small" style="width: 90px; padding: 0 10px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis" type="link" @click="toNode(record.linkNode)">
            ...
            <a-dropdown>
              ...
              <template #overlay>
                <a-menu>
                  ...
                  <a-menu-item>
                    <a-button size="small" type="primary" @click="handleViewLog(record)">{{ $t("i18n_3ed3733078") }}</a-button>
                  </a-menu-item>
                </a-menu>
              </template>
            </a-dropdown>
    ...
    <!-- 操作日志 -->
    <CustomModal v-if="viewOperationLog" v-model:open="viewOperationLog" destroy-on-close :title="$t('i18n_cda84be2f6')" width="80vw" :footer="null" :mask-closable="false">
      <OperationLog v-if="viewOperationLog" :ssh-id="temp.id" />
    </CustomModal>
    <!-- 同步到其他工作空间 -->
    <CustomModal

<template>
  <div>
    ...
    >
      <template #title>
        <a-space wrap class="search-box">
          <a-input v-model:value="listQuery['%name%']" :placeholder="$t('i18n_9c2a917905')" class="search-input-item" @press-enter="getCommandData" />
          <a-input v-model:value="listQuery['%desc%']" :placeholder="$t('i18n_3bdd08adab')" class="search-input-item" @press-enter="getCommandData" />
          <a-input v-model:value="listQuery['%autoExecCron%']" :placeholder="$t('i18n_6b2e348a2b')" class="search-input-item" @press-enter="getCommandData" />
          ...
          <template #title>
            ...
            <div>
              <ul>
                ...
                <li>
                  {{ $t("i18n_26f95520a5") }}
                  <b>#disabled-template-auto-evn</b>
                  {{ $t("i18n_bfacfcd978") }}({{ $t("i18n_8e872df7da") }})
                </li>
    ...
    >
      <a-form ref="editCommandForm" :rules="rules" :model="temp" :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
        ...
        <a-form-item name="command" :help="$t('i18n_77c1e73c08')">
          ...
          <a-form-item-rest>
            <code-editor v-model:content="temp.command" height="40vh" :options="{ mode: 'shell', tabSize: 2 }" :show-tool="true">
          ...
          <a-form-item-rest>
            <a-space direction="vertical" style="width: 100%">
              <a-row v-for="(item, index) in commandParams" :key="item.key">
                <a-col :span="22">
                  <a-space direction="vertical" style="width: 100%">
                    <a-input v-model:value="item.desc" :addon-before="$t('i18n_417fa2c2be', { index: index + 1 })" :placeholder="$t('i18n_3f414ade96', { slot1: $t('i18n_2b1015e902') })" />
                    <a-input v-model:value="item.value" :addon-before="$t('i18n_620489518c', { index: index + 1 })" :placeholder="`${$t('i18n_bfed4943c5')}${$t('i18n_e9f2c62e54')}`" />
                  </a-space>
                </a-col>
        ...
        <a-form-item :label="$t('i18n_bf91239ad7')" name="desc">
          <a-textarea v-model:value="temp.desc" :max-length="255" :rows="3" style="resize: none" :placeholder="$t('i18n_81d7d5cd8a')" />
        </a-form-item>
      </a-form>
    </CustomModal>
    ...
    <!-- 执行日志 -->
    <CustomModal v-if="logVisible" v-model:open="logVisible" destroy-on-close :width="'80vw'" :title="$t('i18n_c84ddfe8a6')" :footer="null" :mask-closable="false">

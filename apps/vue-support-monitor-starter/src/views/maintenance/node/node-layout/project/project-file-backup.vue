  ...
  <div>
      ...
      >
        ...
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-else-if="column.dataIndex === 'operation'">
            <a-space>
              <a-button size="small" type="primary" @click="handleBackupFile(record)">{{ $t("i18n_f26225bde6") }}</a-button>
              <a-button size="small" type="primary" danger @click="handlBackupeDelete(record)">{{ $t("i18n_2f4aaddde3") }}</a-button>
            </a-space>
          </template>
        </template>
      </a-table>
    ...
    <a-layout v-show="!viewList" class="file-layout">
      ...
      <a-layout-sider theme="light" class="sider" width="25%">
        ...
        <a-directory-tree v-model:expandedKeys="expandedKeys" v-model:selectedKeys="selectedKeys" :field-names="treeReplaceFields" :load-data="onTreeData" :tree-data="treeList" @select="nodeClick" />
      </a-layout-sider>
      <!-- 表格 -->
      <a-layout-content class="file-content">
        ...
        >
          <template #title>
            <a-popconfirm
              :title="`${uploadPath ? $t('i18n_bdd4cddd22') + uploadPath + $t('i18n_dadd4907c2') : ''} ${$t('i18n_aefd8f9f27')},${$t('i18n_500789168c')}`"
            ...
            <template v-else-if="column.dataIndex === 'operation'">
              <a-space>
                ...
                <template v-if="record.isDirectory">
                  <!-- record.filename -->
                  <a-popconfirm
                    :title="`${record.filename ? $t('i18n_bdd4cddd22') + record.filename + $t('i18n_dadd4907c2') : ''} ${$t('i18n_aefd8f9f27')},${$t('i18n_500789168c')}`"
                    :ok-text="$t('i18n_587a63264b')"
                    :cancel-text="$t('i18n_b1a09cee8e')"
                    :ok-button-props="{
                      loading: recoverLoading
                ...
                <template v-else>
                  <a-button size="small" type="primary" :loading="recoverLoading" @click="recover(record)">{{ $t("i18n_69de8d7f40") }}</a-button>
                </template>

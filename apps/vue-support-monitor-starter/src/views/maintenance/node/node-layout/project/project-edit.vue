    ...
    <a-spin :tip="$t('i18n_2770db3a99')" :spinning="loading">
      <a-form ref="editProjectForm" :rules="rules" :model="temp" :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
        <a-form-item :label="$t('i18n_4fdd2213b5')" name="id">
          ...
          <a-input v-if="temp.type === 'edit'" v-model:value="temp.id" :max-length="50" :disabled="temp.type === 'edit'" :placeholder="$t('i18n_7ce511154f')" />
          <template v-else>
            <a-input-search
            ...
            <a-tooltip>
              ...
              <template #title>
                <ul>
                  ...
                  <li>
                    <b>ClassPath</b>
                    java -classpath xxx {{ $t("i18n_fa4aa1b93b") }}
                  </li>
                  <li>
                    <b>Jar</b>
                    java -jar xxx {{ $t("i18n_fa4aa1b93b") }}
                  </li>
                  <li>
                    <b>JarWar</b>
                  ...
                  <li>
                    <b>File</b>
                    {{ $t("i18n_5d6f47d670") }},{{ $t("i18n_61955b0e4b") }}
                  </li>
                </ul>
              </template>
        ...
        <a-form-item v-show="temp.runMode === 'Dsl'" name="dslContent">
          <template #label>
            <a-tooltip>
              ...
              <template #title>
                ...
                <p>
                  <b>status</b>
                  {{ $t("i18n_ca69dad8fc") }}:$pid
                  <b>$pid {{ $t("i18n_07a8af8c03") }}</b>
                  {{ $t("i18n_d2f484ff7e") }}
                </p>
          ...
          <a-form-item-rest>
            ...
            <code-editor v-show="dslEditTabKey === 'demo'" v-model:content="PROJECT_DSL_DEFATUL" height="40vh" :show-tool="true" :options="{ mode: 'yaml', tabSize: 2, readOnly: true }">
              <template #tool_before>
                <a-segmented
                  v-model:value="dslEditTabKey"
                  :options="[
        ...
        <a-form-item
          ...
          name="javaExtDirsCp"
        >
          <a-input v-model:value="temp.javaExtDirsCp" :placeholder="`-Dext.dirs=xxx: -cp xx  ${$t('i18n_c53021f06d')}:xx】`" />
        </a-form-item>
        <a-form-item v-show="javaModes.includes(temp.runMode) || javaModes.includes(linkProjectData.runMode)" :label="$t('i18n_497bc3532b')" name="jvm">
          <a-textarea v-model:value="temp.jvm" :auto-size="{ minRows: 3, maxRows: 3 }" :placeholder="$t('i18n_eef3653e9a', { slot1: $t('i18n_3d0a2df9ec'), slot2: $t('i18n_eb5bab1c31') })" />
        </a-form-item>
        <a-form-item v-show="javaModes.includes(temp.runMode) || javaModes.includes(linkProjectData.runMode)" :label="$t('i18n_e5098786d3')" name="args">
          <a-textarea v-model:value="temp.args" :auto-size="{ minRows: 3, maxRows: 3 }" :placeholder="`Main ${$t('i18n_6a9231c3ba')}. ${$t('i18n_848e4e21da')}.port=8080`" />
        </a-form-item>
        <a-form-item v-if="temp.runMode === 'Dsl' || linkProjectData.runMode === 'Dsl'" name="dslEnv" :label="$t('i18n_fba5f4f19a')">
          <!-- <a-input
            v-model:value="temp.dslEnv"
            placeholder="DSL{{$t('i18n_3867e350eb')}},{{$t('i18n_9324290bfe')}}=values1&keyvalue2"
          /> -->
          <parameter-widget v-model:value="temp.dslEnv" />
        </a-form-item>
        ...
        <a-form-item v-show="noFileModes.includes(temp.runMode)" name="autoStart">
          ...
          <template #help>
            <div>
              {{ $t("i18n_71584de972") }}
              <b>{{ $t("i18n_1e4a59829d") }}</b>
              {{ $t("i18n_0360fffb40") }}
            </div>
          </template>
          <div>
            <a-switch v-model:checked="temp.autoStart" :checked-children="$t('i18n_8493205602')" :un-checked-children="$t('i18n_d58a55bcee')" />
            {{ $t("i18n_1022c545d1") }}
          </div>
        </a-form-item>
        ...
        <a-form-item name="disableScanDir">
          ...
          <div>
            <a-switch v-model:checked="temp.disableScanDir" :checked-children="$t('i18n_ced3d28cd1')" :un-checked-children="$t('i18n_56525d62ac')" />
          </div>
        </a-form-item>
        ...
        <a-form-item v-if="temp.runCommand" v-show="temp.type === 'edit' && javaModes.includes(temp.runMode)" :label="$t('i18n_ce559ba296')" name="runCommand">
...
import whiteList from "@/views/maintenance/node/node-layout/system/white-list.vue";

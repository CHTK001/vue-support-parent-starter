          ...
          <a-alert message="" type="warning" show-icon>
            <template #description>
              <ul>
                ...
                <li>
                  {{ $t("i18n_e222f4b9ad") }}
                  <b>{{ $t("i18n_57cadc4cf3") }}</b>
                </li>
                <li>{{ $t("i18n_d19bae9fe0") }}</li>
                <li style="color: red">{{ $t("i18n_282c8cda1f") }}</li>
                <li>
                  {{ $t("i18n_88c85a2506") }}
                  <b>{{ $t("i18n_310c809904") }}</b>
                  ,{{ $t("i18n_f7596f3159") }}
                </li>
                <li>
                  {{ $t("i18n_38da533413") }}
                  <b>{{ $t("i18n_0bac3db71c") }}</b>
                  ,{{ $t("i18n_0e052223a4") }}
                </li>
                <li>
                  {{ $t("i18n_a50fbc5a52") }}
                  <b>networkName</b>
                  {{ $t("i18n_9971192b6a") }}://192.168.31.175:2122/api/node/receive_push?token=xxx&workspaceId=xxx&networkName=en0
                </li>
              </ul>
            </template>
          </a-alert>
            ...
            <template #description>
              <a-space direction="vertical" style="width: 100%">
                ...
                <div v-if="item.type === 'multiIp'">
                  ...
                  <a-tag v-for="(itemIp, indexIp) in item.canUseIp" :key="indexIp" @click="confirmFastInstall(item.id, itemIp, item.port)">
                    {{ itemIp }}:{{ item.port }}
                    <ApiOutlined />
                  </a-tag>
                </div>
    ...
    fastInstall() {
      fastInstall().then(res => {
        if (res.code === 200) {
          ...
          this.fastInstallInfo.host = `${location.protocol}//${location.host}${res.data.url}?token=${res.data.token}\\`;
          let auth = "";
          if (res.data.auth) {

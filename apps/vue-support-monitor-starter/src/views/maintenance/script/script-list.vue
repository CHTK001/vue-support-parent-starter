    ...
    >
      <template #title>
        <a-space wrap class="search-box">
          <a-input v-model:value="listQuery['id']" :placeholder="$t('i18n_d18d658415')" allow-clear class="search-input-item" @press-enter="loadData" />
          <a-input v-model:value="listQuery['%name%']" :placeholder="$t('i18n_d7ec2d3fea')" allow-clear class="search-input-item" @press-enter="loadData" />
          <a-input v-model:value="listQuery['%description%']" :placeholder="$t('i18n_3bdd08adab')" class="search-input-item" @press-enter="loadData" />
          <a-input v-model:value="listQuery['%autoExecCron%']" :placeholder="$t('i18n_6b2e348a2b')" class="search-input-item" @press-enter="loadData" />
          ...
          <a-button v-if="mode === 'manage'" type="primary" :disabled="!tableSelections || !tableSelections.length" @click="syncToWorkspaceShow">{{ $t("i18n_398ce396cd") }}</a-button>
        </a-space>
      </template>
      <template #tableHelp>
        <a-tooltip>
              ...
              <a-dropdown>
                ...
                <template #overlay>
                  <a-menu>
                    ...
                    <a-menu-item>
                      <a-button size="small" type="primary" danger :disabled="!record.nodeIds" @click="handleUnbind(record)">{{ $t("i18n_663393986e") }}</a-button>
                    </a-menu-item>
                  </a-menu>
                </template>
              </a-dropdown>
    ...
    >
      <a-form ref="editScriptForm" :rules="rules" :model="temp" :label-col="{ span: 3 }" :wrapper-col="{ span: 19 }">
        ...
        <a-form-item :label="$t('i18n_4d9c3a0ed0')" name="context">
          <a-form-item-rest>
            <code-editor v-model:content="temp.context" height="40vh" :show-tool="true" :options="{ mode: 'shell', tabSize: 2 }">
        ...
        <a-form-item :label="$t('i18n_2171d1b07d')">
          <a-space direction="vertical" style="width: 100%">
            <a-row v-for="(item, index) in commandParams" :key="item.key">
              <a-col :span="22">
                <a-space direction="vertical" style="width: 100%">
                  <a-input v-model:value="item.desc" :addon-before="$t('i18n_0390e2f548', { count: index + 1 })" :placeholder="$t('i18n_9e78f02aad')" />
                  <a-input v-model:value="item.value" :addon-before="$t('i18n_a1f58b7189', { count: index + 1 })" :placeholder="$t('i18n_2d9569bf45')" />
                </a-space>
              </a-col>
              <a-col :span="2">
        ...
        <a-form-item :label="$t('i18n_6b2e348a2b')" name="autoExecCron">
          <a-auto-complete v-model:value="temp.autoExecCron" :placeholder="$t('i18n_5dff0d31d0')" :options="CRON_DATA_SOURCE">
            <template #option="item">{{ item.title }} {{ item.value }}</template>
          </a-auto-complete>
        </a-form-item>
        <a-form-item :label="$t('i18n_3bdd08adab')" name="description">
          <a-textarea v-model:value="temp.description" :max-length="200" :rows="3" style="resize: none" :placeholder="$t('i18n_ae653ec180')" />
        </a-form-item>
        ...
        <a-form-item v-if="temp.prohibitSync" :label="$t('i18n_b499798ec5')">
          ...
          <a-tag v-for="(item, index) in temp.nodeList" :key="index">{{ $t("i18n_5d83794cfa") }}{{ item.nodeName }} {{ $t("i18n_4d85c37f0d") }}{{ item.workspaceName }}</a-tag>
        </a-form-item>
        <a-form-item v-else>
          <template #label>
            <a-tooltip>

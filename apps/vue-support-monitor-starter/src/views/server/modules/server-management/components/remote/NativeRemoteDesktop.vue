<template>
  <div class="native-remote-desktop system-container modern-bg">
    <!-- 工具栏 -->
    <div class="toolbar">
...
const handleScreenData = (data: ArrayBuffer) => {
  ...
  try {
    ...
    if (
      formatLength <= 0 ||
      formatLength > 20 ||
      formatLength + 4 > data.byteLength
    ) {
...
const handleMouseMove = (event: MouseEvent) => {
  ...
  socketService.emit(
    RemoteTopics.DESKTOP.MOUSE_MOVE,
    JSON.stringify({
      x: pos.x,
      y: pos.y,
    }),
...
const handleMouseDown = (event: MouseEvent) => {
  ...
  const pos = getMousePosition(event);
  socketService.emit(
    RemoteTopics.DESKTOP.MOUSE_CLICK,
    JSON.stringify({
      x: pos.x,
      y: pos.y,
      button: event.button,
...
const handleMouseWheel = (event: WheelEvent) => {
  ...
  const pos = getMousePosition(event);
  socketService.emit(
    RemoteTopics.DESKTOP.MOUSE_WHEEL,
    JSON.stringify({
      x: pos.x,
      y: pos.y,
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  socketService.emit(
    RemoteTopics.DESKTOP.KEY_PRESS,
    JSON.stringify({
...
const handleKeyUp = (event: KeyboardEvent) => {
  ...
  socketService.emit(
    RemoteTopics.DESKTOP.KEY_RELEASE,
    JSON.stringify({
      keyCode: event.keyCode,
      keyChar: event.key,
      modifiers: getModifiers(event),
...
const disconnect = () => {
  if (socketService && props.server) {
    socketService.emit(
      RemoteTopics.DESKTOP.DISCONNECT,
      JSON.stringify({
        serverId: props.server.monitorSysGenServerId,
      }),
    );

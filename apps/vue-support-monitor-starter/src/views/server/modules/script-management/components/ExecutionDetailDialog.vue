    ...
    <div v-if="executionData" class="execution-detail">
      ...
      <div v-if="executionData.parameters" class="detail-section">
        ...
        <div class="parameters-content">
          <pre>{{ executionData.parameters }}</pre>
        </div>
      </div>

      <!-- 环境变量 -->
      <div v-if="executionData.envVars" class="detail-section">
        <h4>环境变量</h4>
        <div class="env-vars-content">
          <div
            v-for="(value, key) in executionData.envVars"
            :key="key"
            class="env-var-item"
          >
          ...
          <el-tab-pane label="标准输出" name="stdout">
            <div class="output-container">
              ...
              <div class="output-content">
                <pre v-if="executionData.stdout">{{ executionData.stdout }}</pre>
          ...
          <el-tab-pane label="错误输出" name="stderr">
            <div class="output-container">
              ...
              <div class="output-content error">
                <pre v-if="executionData.stderr">{{ executionData.stderr }}</pre>
      ...
      <!-- 性能统计 -->
      <div v-if="executionData.performance" class="detail-section">
        <h4>性能统计</h4>
        <div class="performance-stats">
          <div class="stat-item">
            <div class="stat-label">CPU使用率</div>
            <div class="stat-value">{{ executionData.performance.cpuUsage }}%</div>
          </div>
          <div class="stat-item">
            <div class="stat-label">内存使用</div>
            <div class="stat-value">{{ executionData.performance.memoryUsage }}MB</div>
          </div>
          <div class="stat-item">
            <div class="stat-label">磁盘读取</div>
            <div class="stat-value">{{ executionData.performance.diskRead }}MB</div>
          </div>
          <div class="stat-item">
            <div class="stat-label">磁盘写入</div>
            <div class="stat-value">{{ executionData.performance.diskWrite }}MB</div>
          </div>
        </div>
      </div>
    </div>

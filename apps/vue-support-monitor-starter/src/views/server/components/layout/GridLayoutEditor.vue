...
const getItemStyle = (item: any) => {
  ...
  const colWidth =
    (containerWidth - margin.value[0] * (cols.value + 1)) / cols.value;
  ...
  return {
    ...
    zIndex:
      (isDragging.value && dragItem.value?.i === item.i) ||
      (isResizing.value && resizeItem.value?.i === item.i)
        ? 1000
        : 1,
  };
};
...
const handleMouseMove = (event: MouseEvent) => {
  ...
  const newX = Math.max(
    0,
    Math.min(cols.value - dragItem.value.w, startPosition.value.x + deltaGridX),
  );
...
const handleResizeMove = (event: MouseEvent) => {
  ...
  if (resizeDirection.value.includes("e")) {
    newW = Math.max(
      1,
      Math.min(cols.value - resizeItem.value.x, startSize.value.w + deltaGridX),
    );
  }
    ...
    &.edit-mode {
      ...
      .grid-item {
        ...
        &:active {
          ...
          box-shadow:
            0 0 0 3px var(--el-color-primary),

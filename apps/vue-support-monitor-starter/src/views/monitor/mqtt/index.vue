    ...
    <ScArticleSlot ref="articleRef" :url="fetchPageForMqtt" :rowClick="handleRowClick">
      <template #top="{ row }">
        <el-icon :size="98" class="cover">
          <component :is="useRenderIcon('simple-icons:mqtt')" />
        </el-icon>
...
<script setup>
import { fetchPageForMqtt, fetchStartForMqtt, fetchStopForMqtt } from "@/api/monitor/mqtt";
...
const handleStop = async row => {
  fetchStopForMqtt({ monitorMqttServerId: row.monitorMqttServerId }).then(res => {
    if (res.code == "00000") {
...
const handleStart = async row => {
  fetchStartForMqtt({ monitorMqttServerId: row.monitorMqttServerId }).then(res => {
    if (res.code == "00000") {
      articleRef.value.refresh();
    }
  });

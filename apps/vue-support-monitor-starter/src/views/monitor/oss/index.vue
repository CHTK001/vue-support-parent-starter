        ...
        <scTable ref="table" :url="fetchOssProtocolPage" :params="searchParams" row-key="id" stripe>
          ...
          <el-table-column label="操作" fixed="right" align="right" width="260">
            <template #default="scope">
              <el-button class="btn-text" :icon="useRenderIcon('ri:settings-2-line')" @click="doDetail(scope.row)" />
              ...
              <span v-else>
                <el-button type="danger" class="btn-text" :icon="useRenderIcon('ri:stop-circle-line')"
                  @click="stop(scope.row)" />
              </span>
              <el-button type="primary" class="btn-text" :icon="useRenderIcon('ri:edit-box-line')"
                @click="save(scope.row, 'edit')" />
              <el-popconfirm v-if="scope.row.fileStorageProtocolStatus != 1" :title="$t('message.confimDelete')"
                @confirm="doDelete(scope.row, scope.$index)">
                <template #reference>
                  <el-button type="danger" class="btn-text" :icon="useRenderIcon('ri:delete-bin-2-line')" />
                </template>
              </el-popconfirm>
              <el-button text :icon="useRenderIcon('ri:file-copy-2-line')">
                <span

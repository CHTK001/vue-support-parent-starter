<template>
  <el-dialog v-model="visibleInner" :title="modelValue ? '编辑配置' : '新建配置'" width="860px" :close-on-click-modal="false" @close="handleClose" class="data-edit-dialog">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" class="form-grid">
      ...
      <div class="col left">
        ...
        <el-form-item label="主机/端口">
          <div style="display:flex; gap:8px; width:100%">
            ...
            <el-input-number v-model="form.systemDataSettingPort" :min="0" :max="65535" placeholder="端口" :disabled="!modeChosen" />
          </div>
        </el-form-item>
      ...
      <div class="col right">
        <el-form-item label="控制台类型">
          <el-select v-model="form.systemDataSettingConsoleType" placeholder="请选择" :disabled="!modeChosen">
...
const form = ref<SystemDataSetting>({ systemDataSettingName: '', systemDataSettingType: '', systemDataSettingEnabled: true, systemDataSettingTimeoutMs: 600000, systemDataSettingMode: 'REMOTE' })
const formRef = ref()
...
const jdbcDrivers = [
  { label: 'MySQL 8.x - com.mysql.cj.jdbc.Driver', value: 'com.mysql.cj.jdbc.Driver' },
  { label: 'MySQL 5.x - com.mysql.jdbc.Driver', value: 'com.mysql.jdbc.Driver' },
  { label: 'PostgreSQL - org.postgresql.Driver', value: 'org.postgresql.Driver' },
  { label: 'Oracle - oracle.jdbc.OracleDriver', value: 'oracle.jdbc.OracleDriver' },
  { label: 'SQLServer - com.microsoft.sqlserver.jdbc.SQLServerDriver', value: 'com.microsoft.sqlserver.jdbc.SQLServerDriver' },
  { label: 'SQLite - org.sqlite.JDBC', value: 'org.sqlite.JDBC' },
  { label: 'H2 - org.h2.Driver', value: 'org.h2.Driver' },
  { label: 'Calcite - org.apache.calcite.jdbc.Driver', value: 'org.apache.calcite.jdbc.Driver' }
]

const protocolOptions = [
  'jdbc:mysql', 'jdbc:postgresql', 'jdbc:sqlite', 'jdbc:oracle', 'jdbc:sqlserver',
  'redis', 'zookeeper', 'influxdb', 'mongodb', 'kafka'
]
...
const isJdbc = computed(() => {
  ...
  return t.includes('jdbc') || t.includes('sql') || url.startsWith('jdbc:')
})
...
function init() {
  form.value = props.modelValue ? { ...(props.modelValue as any) } : { systemDataSettingName: '', systemDataSettingType: '', systemDataSettingEnabled: true, systemDataSettingMode: 'REMOTE' }
}
...
async function onDriverFileChange(file: any) {
  try {
    ...
    // 假设后端返回驱动落盘路径到 data
    if (res.data) form.value.systemDataSettingDriverPath = res.data

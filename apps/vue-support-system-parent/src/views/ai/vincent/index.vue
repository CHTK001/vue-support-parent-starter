  ...
  <div class="common-layout overflow-hidden pt-1 pb-1 pl-1">
    ...
    <Transition>
      <el-container class="overflow-hidden">
        <el-aside style="height: 100%; border-right: 1px solid var(--el-border-color); width: var(--aside-width)" class="p-4 relative !overflow-visible" id="aside">
          <div>
            ...
            <el-form ref="formRef" :model="form" :rules="rules" v-if="settingOpen" label-width="100px">
              ...
              <el-form-item label="比例" prop="parameters.size" class="ration--GrtZmC3d">
                <el-radio-group v-model="form.parameters.size" class="flex justify-start items-start flex-wrap">
                  <el-radio-button class="!ml-0 mt-1 mr-[12px] item" round :key="item" :value="item" v-for="(item, index) in formSetting.sysAiVincentSupportSize?.split(',') || []">
                    <template #default>
              ...
              <el-form-item label="风格" prop="parameters.style">
                <el-select v-model="form.parameters.style" placeholder="请选择风格">
                  <el-option :key="item" :label="getStyleLabel(item)" :value="item" v-for="item in formSetting.sysAiVincentSupportStyle?.split(',') || []"> </el-option>
                </el-select>
              </el-form-item>
              ...
              <el-form-item label="参考图像" prop="refImg">
                <div class="flex w-full align-middle">
                  ...
                  <div class="flex-1 text-center text-template cursor-pointer" @click="showRefImage = !showRefImage">选择模板</div>
              ...
              <el-form-item label="输出张数" prop="parameters.number"> <el-input-number :min="1" :max="formSetting.sysAiVincentSupportNumber" :step="1" v-model="form.parameters.number" /> </el-form-item>
          ...
          <div class="overflow-hidde absolute w-[500px] top-0 ref-image p-3" v-if="showRefImage && settingOpen">
            <el-row>
              <el-col :span="8" v-for="row in CREATIVE_TEMPLATE" @click="form.input.refImg = row.cover" class="cursor-pointer">
                <div class="flex flex-col gap-2 px-2 ref-image-item">
                  <el-image :src="row.cover" fit="cover" class="img"></el-image>
                  <div>
                    <h2 class="text">{{ row.modelName }}</h2>
                  </div>
                  <div class="el-form-item-msg">{{ row.description }}</div>
                </div>
              </el-col>
            </el-row>
          </div>
        </el-aside>
        <el-main class="overflow-hidden main">
          <div class="pl-2">
            <vincent ref="vincentRef" :form="form" :env="env"></vincent>
            <HistoryLayout ref="historyLayoutRef" :form="form" class="overflow-hidden"></HistoryLayout>
          </div>
        </el-main>
      </el-container>
    </Transition>
...
const rules = {
  ...
  "input.prompt": [
    { required: true, message: "请输入正向提示词", trigger: "change" },
    { min: 1, max: 800, message: "长度在 1 到 800 个字符", trigger: "change" },
  ],
};
...
const createInterval = () => {
  intervalId = setInterval(() => {
    fetchGetTaskForVincent({ taskId: requestId(), sysProjectId: form.sysProjectId })
      .then((res) => {
...
const handleTrigger = async () => {
  ...
  window.aside.style.setProperty("--aside-width", settingOpen.value ? "500px" : "55px");
};

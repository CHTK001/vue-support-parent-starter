  ...
  <div class="h-full">
    ...
    <ScTable ref="tableRef" :url="fetchPageProjectForDevice" :params="form" :columns="env.columns"
      ...
      <el-table-column prop="sysDeviceResourceType" label="资源类型" align="center" show-overflow-tooltip width="120px">
        <template #default="{ row }">
          <el-button :title="row.sysDeviceDescription" plain class="btn-text"
            :icon="useRenderIcon(getResourceIcon(row.sysDeviceResourceType))"></el-button>
      ...
      <el-table-column label="操作" fixed="right" align="center" width="240px">
        <template #default="{ row }">
          <el-button size="small" plain link type="primary" :icon="useRenderIcon('ep:edit')"
            @click="deviceInstance.dialogOpen(saveDialogRef, row, 'edit')">
            {{ $t("buttons.update") }}
          </el-button>

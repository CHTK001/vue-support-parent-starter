<template>
  <el-dialog v-model="dialogVisible" draggable title="设备详情" width="700px" :close-on-click-modal="true"
    ...
    'device-detail-clickable': deviceData.sysDeviceResourceType === 'CAMERA' && deviceData.sysDeviceOnline == 1
        ...
        <div class="device-detail-status-text"
          :class="{ 'device-detail-online-text': deviceData.sysDeviceOnline == 1, 'device-detail-offline-text': deviceData.sysDeviceOnline != 1 }">
        ...
        <el-descriptions :column="1" border class="device-detail-descriptions">
          ...
          <el-descriptions-item label="设备序列号" class="device-detail-item">
            <div class="device-detail-copy-container">
              ...
              <el-button v-if="deviceData.sysDeviceSerialNumber" v-copy:click="deviceData.sysDeviceSerialNumber"
                type="primary" link size="small" :icon="useRenderIcon('ep:copy-document')"
                class="device-detail-copy-btn"></el-button>
            </div>
          </el-descriptions-item>
...
.device-detail-svg-container {
  ...
  &.device-detail-online {
    box-shadow: 0 0 0 8px rgba(48, 104, 20, 0.1), 0 10px 20px rgba(0, 0, 0, 0.1);
    animation: device-detail-pulse 2.5s infinite;
  }

  &.device-detail-offline {
    box-shadow: 0 0 0 8px rgba(240, 14, 14, 0.1), 0 10px 20px rgba(0, 0, 0, 0.1);
...
@keyframes device-detail-pulse {
  0% {
    box-shadow: 0 0 0 0 rgba(48, 104, 20, 0.4), 0 10px 20px rgba(0, 0, 0, 0.1);
  }

  70% {
    box-shadow: 0 0 0 10px rgba(48, 104, 20, 0), 0 10px 20px rgba(0, 0, 0, 0.1);
  }

  100% {
    box-shadow: 0 0 0 0 rgba(48, 104, 20, 0), 0 10px 20px rgba(0, 0, 0, 0.1);
  }
}

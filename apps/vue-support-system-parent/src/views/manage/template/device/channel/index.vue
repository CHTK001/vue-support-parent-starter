...
import { deepClean, deepCopy, message } from "@repo/utils";
...
const handleSaveIntoList = async () => {
  ...
  if (isAddingNew.value) {
    ...
    // 更新现有通道
    const index = form.channelList.findIndex(item =>
...
<template>
  <div>
    <el-dialog :title="title" draggable v-model="visible" :close-on-click-modal="false" @close="handleClose"
      width="800px" class="channel-dialog">
      <div class="channel-container">
        ...
        <div class="channel-list-section" :class="{ 'w-full': !isSelected, 'w-1/2': isSelected }">
          ...
          <div class="channel-grid">
            <div v-for="(item, index) in form.channelList" :key="index" class="channel-card-wrapper"
              @click="handleClickItem(item)">
              <el-card :class="{ 'is-selected': selectedItem.sysDeviceChannelId === item.sysDeviceChannelId }"
                class="channel-card">
        ...
        <div class="channel-detail-section w-full" v-if="isSelected">
          ...
          <el-form :model="selectedItem" label-position="top" class="detail-form">
            ...
            <el-form-item label="通道启用状态">
              <el-segmented v-model="selectedItem.sysDeviceChannelUse" :options="[
      {
        label: '启用',
        value: 1,
      },

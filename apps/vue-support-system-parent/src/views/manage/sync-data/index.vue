  ...
  <div class="sync-data-container system-container modern-bg">
    ...
    <ScCard class="filter-card">
      ...
      >
        ...
        <ScTableColumn prop="syncTaskId" label="ID" width="80" />
        <ScTableColumn
          prop="syncTaskName"
          label="任务名称"
          min-width="150"
...
const taskList = ref<MonitorSyncTask[]>([]);
const total = ref(0);
const tableRef = ref<TableInstance>();
const selectedRows = ref<MonitorSyncTask[]>([]);
...
const logsList = ref<MonitorSyncTaskLog[]>([]);
...
// 选择变化
const handleSelectionChange = (rows: MonitorSyncTask[]) => {
...
// 编辑任务
const handleEdit = (row: MonitorSyncTask) => {
...
// 删除任务
const handleDelete = (row: MonitorSyncTask) => {
...
// 启动任务
const handleStart = async (row: MonitorSyncTask) => {
  try {
...
// 停止任务
const handleStop = async (row: MonitorSyncTask) => {
  try {
...
// 执行一次
const handleExecuteOnce = async (row: MonitorSyncTask) => {
  try {
...
// 查看日志
const handleLogs = async (row: MonitorSyncTask) => {

import type { VideoResult, ApiResponse, VideoInfo, VideoSearchParams, VideoSearchResult } from "../types";
...
export const fetchVideoSearch = async (params: VideoSearchParams): Promise<ApiResponse<VideoSearchResult>> => {
  return http.request("get", "/v2/video/info/search", {
    params,
  });
};
 ...
 * @returns 模拟的视频搜索结果数组
 */
function generateMockVideoResults(keyword: string, count: number, type: string = "all", platform: string = "all"): VideoResult[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    mockResults.push({
      videoId: `result-${i}`,
      videoTitle: `${keyword} - ${selectedType}内容 ${i + 1}`,
      videoPlatform: selectedPlatform,
      videoThumbnail: `https://picsum.photos/300/200?random=${i}`,
      videoUrl: "#",
      videoViews: views,
      videoDuration: duration,
      videoPublishDate: date.toISOString().split("T")[0],

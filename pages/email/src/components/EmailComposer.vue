...
// 监听账户变化，更新发件人信息
watch(() => emailForm.value.emailAccountId, (newAccountId) => {
...
// 监听模式和原始邮件，初始化表单
watch([() => props.mode, () => props.originalEmail], ([newMode, newOriginalEmail]) => {
  if (newOriginalEmail) {
    // 根据不同模式设置不同的初始值
    if (newMode === 'reply') {
      ...
      if (newOriginalEmail.emailRecipients.emailCc) {
        emailForm.value.emailRecipients.emailCc = newOriginalEmail.emailRecipients.emailCc.filter(
          recipient => recipient.emailAddress !== selectedAccount.value.emailAccountAddress
        );
    ...
    <div class="email-composer__body">
      ...
      <div class="email-composer__field">
        ...
        <div class="email-composer__input">
          ...
          <el-input 
            ...
            @keyup.enter="(e) => e.target.value = addRecipient('to', e.target.value)"
      ...
      <div class="email-composer__field" v-if="showCc">
        ...
        <div class="email-composer__input">
          ...
          <el-input 
            placeholder="输入邮箱地址并按回车添加" 
            @keyup.enter="(e) => e.target.value = addRecipient('cc', e.target.value)"
            class="email-composer__recipient-input"
          />
        </div>
      </div>
      ...
      <div class="email-composer__field" v-if="showBcc">
        ...
        <div class="email-composer__input">
          ...
          <el-input 
            placeholder="输入邮箱地址并按回车添加" 
            @keyup.enter="(e) => e.target.value = addRecipient('bcc', e.target.value)"
            class="email-composer__recipient-input"
          />
        </div>
      </div>
        ...
        <div class="email-composer__attachments-list">
          ...
          >
            <div class="email-composer__attachment-icon">
              <IconifyIconOnline 
                :icon="
                  attachment.emailAttachmentType.includes('image') ? 'ri:image-line' :
                  attachment.emailAttachmentType.includes('pdf') ? 'ri:file-pdf-line' :
                  attachment.emailAttachmentType.includes('word') ? 'ri:file-word-line' :
                  attachment.emailAttachmentType.includes('excel') || attachment.emailAttachmentType.includes('sheet') ? 'ri:file-excel-line' :
                  'ri:file-line'
                "
              />
            </div>

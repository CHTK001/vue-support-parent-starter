import type { MonitorVideoResult, ApiResponse, VideoInfo, VideoSearchParams, VideoSearchResult } from "../types";
import { videoHotKeywords, videoList, videoPlatforms, videoTypes } from "./mockData";
// 模拟延迟
...
export const fetchVideoSearch = async (params: VideoSearchParams): Promise<ApiResponse<VideoSearchResult>> => {
  ...
  if (params.keyword) {
    ...
    filteredVideos = filteredVideos.filter((video) => video.monitorVideoTitle.toLowerCase().includes(keyword) || video.monitorVideoDescription.toLowerCase().includes(keyword) || video.monitorVideoAuthor.toLowerCase().includes(keyword));
  }

  // 类型过滤
  if (params.selectedType && params.selectedType !== "all") {
    filteredVideos = filteredVideos.filter((video) => video.monitorVideoType === videoTypes.find((t) => t.videoId === params.selectedType)?.videoName);
  }

  // 平台过滤
  if (params.platformId && params.platformId !== "all") {
    filteredVideos = filteredVideos.filter((video) => video.monitorVideoPlatform === videoPlatforms.find((p) => p.videoId === params.platformId)?.videoName);
  }
...
function generateMockVideoResults(keyword: string, count: number, type: string = "all", platform: string = "all"): MonitorVideoResult[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // 生成随机的视频描述
    const descriptions = [`这是一个关于${keyword}的精彩${selectedType}`, `${author}带来的${selectedType}作品，探索${keyword}的奥秘`, `高质量${selectedType}内容，深度解析${keyword}`, `最新${selectedType}：${keyword}完整版`, `独家${selectedType}：${keyword}全程高能`];
...
// 导出类型
export type { ApiResponse, VideoInfo, VideoSearchParams, VideoSearchResult };

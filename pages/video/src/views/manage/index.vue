          ...
          <el-table-column label="操作" width="200" fixed="right">
            <template #default="{ row }">
              <div class="flex items-center space-x-2">
                <el-button size="small" type="primary" @click="playVideo(row)">
                  播放
                </el-button>
      ...
      <div v-else class="grid-view p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          <div 
            ...
            :key="video.videoId"
            class="video-card bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow cursor-pointer"
            @click="viewVideoDetail(video)"
          >
            <!-- 视频封面 -->
            <div class="video-cover-container relative">
              <img 
                :src="video.videoCover || '/placeholder-video.jpg'"
              ...
              <div class="selection-checkbox absolute top-2 left-2">
                <el-checkbox 
                  :model-value="selectedVideos.includes(video)"
                  @change="(checked) => handleVideoSelection(video, checked)"
                  @click.stop
                />
              </div>
            </div>
            ...
            <div class="video-info p-3">
              ...
              <div class="video-actions flex items-center justify-between">
                <div class="action-buttons flex items-center space-x-1">
                  <el-button size="small" type="primary" @click.stop="playVideo(video)">
                    播放
                  </el-button>
                ...
                <div class="action-icons flex items-center space-x-1">
                  <el-button 
                    size="small" 
                    :type="video.isFavorite ? 'warning' : 'info'"
                    circle
      ...
      <div v-if="currentVideo" class="video-detail">
        <div class="flex">
          ...
          <div class="video-cover mr-6">
            <img 
              :src="currentVideo.videoCover || '/placeholder-video.jpg'"
      ...
      >
        ...
        <template #tip>
          <div class="el-upload__tip">
            支持 mp4, avi, mkv, mov 等格式，单个文件不超过 2GB
          </div>
        </template>
      </el-upload>
    </el-dialog>
...
import {
  Refresh, Upload, Download, VideoPlay, FolderOpened, Star, View, Clock,
  Search, List, Grid, ArrowDown, MoreFilled, UploadFilled
...
const loadVideos = async () => {
  ...
  try {
    ...
    const response = await getVideoList(request);
 ...
 * 切换收藏状态
 */
const toggleFavorite = async (video: VideoInfo) => {
  try {
    const response = video.isFavorite
      ? await removeVideoFromCollection(video.videoId)
      : await addVideoToCollection(video.videoId);
 ...
 * 处理视频命令
 */
const handleVideoCommand = async (command: string, video: VideoInfo) => {
  switch (command) {
    case 'favorite':
    ...
    case 'info':
      ...
      break;
    case 'delete':
      await deleteVideoConfirm(video);
      break;
  }
};
 ...
 * 分享视频
 */
const shareVideo = async (video: VideoInfo) => {
  try {
 ...
 * 删除视频确认
 */
const deleteVideoConfirm = async (video: VideoInfo) => {
  try {
    await ElMessageBox.confirm(
      `确定要删除视频 "${video.videoTitle}" 吗？`,
      '确认删除',
      {
    ...
    const response = await deleteVideo(video.videoId);
    if (response.code === 1000) {
...
const batchFavorite = async (favorite: boolean) => {
  try {
    const promises = selectedVideos.value.map(video => 
      favorite 

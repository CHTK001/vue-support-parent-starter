<template>
  <div class="device-container system-container modern-bg">
      ...
      <el-header class="device-header">
        <div class="device-left-panel">
          ...
          >
            ...
            >
              <ScSelect
                v-model="deviceForm.sysProjectId"
    ...
    <ScTable
      ...
      :url="fetchPageProjectForDevice"
      :params="deviceForm"
      class="device-card-table"
    >
      <template #default="{ row }">
        <ScCard shadow="hover" class="device-card">
          <template #header>
            <div class="device-card-header">
              <div class="device-card-title">
                ...
                />
                <div>
                  <div class="device-card-name">{{ row.sysDeviceName || "未命名设备" }}</div>
                  <div class="device-card-serial">{{ row.sysDeviceSerialNumber || "-" }}</div>
                </div>
              </div>
              <ScTag :type="row.sysDeviceOnline === 1 ? 'success' : 'danger'">
                {{ row.sysDeviceOnline === 1 ? "在线" : "离线" }}
              </ScTag>
            </div>
          </template>
          <div class="device-card-body">
            <div class="device-card-item">
              <span>组织</span>
              <strong>{{ row.sysDeviceOrgName || row.sysDeviceOrgCode || "-" }}</strong>
            </div>
            <div class="device-card-item">
              <span>IP</span>
              <strong>{{ row.sysDeviceNetAddress || "-" }}</strong>
            </div>
            <div class="device-card-item">
              <span>通道</span>
              <strong>{{ row.sysDeviceChannelCount || 0 }}</strong>
            </div>
            <div class="device-card-item">
              <span>更新时间</span>
              <strong>{{ getTimeAgo(row.updateTime || row.createTime) }}</strong>
            </div>
          </div>
          <div class="device-card-actions">
            ...
            <ScButton
              ...
              type="primary"
              @click.stop="
                deviceInstance.handlePreviewUrl(cameraPreviewDialogRef, row, 'view')
              "
            >
...
import { fetchListDeviceMerchant } from "../../api/manage/device-merchant";
...
const onSearch = async () => {
  if (viewMode.value === "card") {
    cardTableRef.value?.reload(deviceForm);
    return;
  }
...
onMounted(async () => {
  if (route.query.sysProjectId) {
    deviceForm.sysProjectId = Number(route.query.sysProjectId);
  }
  const res = await fetchListDeviceMerchant();
  merchantOptions.value =
    res?.data?.map((item) => ({
      label: item.sysProjectName,
      value: item.sysProjectId,
    })) || [];
});
</script>

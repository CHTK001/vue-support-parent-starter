...
import { fetchDeleteProjectForSms, fetchPageProjectForSms, fetchSyncProjectForSms, fetchUpdateProjectForSms } from "../../../api/manage/project-sms";
    ...
    <el-container>
        ...
        <div class="left-panel">
          <el-form ref="formRef" :inline="true" :model="form" class="search-form bg-bg_color pl-6 pt-[10px] overflow-auto">
            <el-form-item label="模板名称" prop="sysSmsTemplateName">
              <el-input v-model="form.sysSmsTemplateName" placeholder="请输入模板名称" clearable class="!w-[180px]" />
            </el-form-item>
      ...
      <el-main>
        <ScTable ref="tableRef" border :search="false" :url="fetchPageProjectForSms" :params="params" class="custom-table-row">
          ...
          <el-table-column prop="sysSmsTemplateCategory" label="模板类型" show-overflow-tooltip>
            <template #default="{ row }">
          ...
          <el-table-column prop="updateTime" label="最后一次更新时间" align="center">
            <template #default="{ row }">
              <span :title="'更新来自' + row.sysSmsTemplateOrigin">{{ row.updateTime || row.createTime }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" fixed="right" align="center">
            <template #default="{ row }">
              ...
              <el-popconfirm v-if="row.sysSmsTemplateDisabled == 0" :title="$t('message.confimDelete')" @confirm="onDelete(row)">
                <template #reference>

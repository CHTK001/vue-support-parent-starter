<template>
  <div
    class="w-full min-h-screen bg-slate-950 text-cyan-400 font-thin overflow-hidden relative selection:bg-cyan-500 selection:text-white"
  >
    <!-- Ambient Background -->
    <div class="absolute inset-0 z-0 pointer-events-none">
      <div
        class="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-black opacity-80"
      />
      <div
        class="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-cyan-900/10 rounded-full blur-[100px] animate-pulse"
      />
      <div
        class="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-indigo-900/10 rounded-full blur-[100px] animate-pulse delay-1000"
      />
    </div>
    ...
    <TechDeco
      class="absolute bottom-20 right-10 z-0 opacity-30 rotate-180 scale-125"
    />
    ...
    <div class="relative z-10 flex flex-col h-full p-4 gap-4">
        ...
        <template #right>
          <div class="flex items-center gap-4 text-xs font-mono tracking-wider">
            <span class="text-cyan-600"
              >SYS.STATUS:
              <span class="text-cyan-400 animate-pulse">ONLINE</span></span
            >
            <span class="text-cyan-600"
              >NET: <span class="text-emerald-400">MAINNET</span></span
            >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
        <TechPanel
          ...
          :key="index"
          class="h-32 group hover:bg-cyan-900/5 transition-all duration-500"
        >
          <div class="flex flex-col justify-between h-full p-4">
            <div class="flex justify-between items-start">
              <span class="text-xs tracking-[0.2em] text-cyan-600 uppercase">{{
                stat.label
              }}</span>
              <TechDeco
                :small="true"
            ...
            <div class="flex items-end gap-2">
              <span
                class="text-3xl font-light text-white font-mono tracking-tighter"
                >{{ stat.value }}</span
              >
              <span
            ...
            <div
              class="w-full h-1 bg-slate-800 mt-2 relative overflow-hidden rounded-full"
            >
              <div
                class="absolute top-0 left-0 h-full bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.5)]"
                :style="{ width: Math.abs(stat.trend) * 10 + '%' }"
              />
            </div>
          </div>
        </TechPanel>
        ...
        <div class="lg:col-span-4 flex flex-col gap-4">
          ...
          <TechPanel class="flex-1 min-h-[250px]">
            ...
            <div class="p-4 h-full overflow-auto text-xs font-mono">
              <div
                class="grid grid-cols-3 text-slate-500 mb-2 border-b border-slate-800 pb-1"
              >
                <span>PRICE</span>
              ...
              <div class="flex flex-col gap-1">
                <div
                  v-for="(ask, i) in asks"
                  :key="'ask-' + i"
                  class="grid grid-cols-3 relative group hover:bg-slate-800/50 cursor-pointer"
                >
                  ...
                  <div
                    class="absolute right-0 top-0 h-full bg-rose-500/10"
                    :style="{ width: ask.amount * 10 + '%' }"
                  />
                </div>
                <div
                  class="py-2 text-center text-lg text-white font-bold border-y border-slate-800 my-1"
                >
                ...
                >
                  ...
                  <div
                    class="absolute right-0 top-0 h-full bg-emerald-500/10"
                    :style="{ width: bid.amount * 10 + '%' }"
                  />
                </div>
              </div>
            </div>
          ...
          <TechPanel class="h-1/3 min-h-[200px]">
            ...
            <div
              class="p-4 font-mono text-xs flex flex-col gap-2 text-slate-400 h-full overflow-hidden"
            >
...
const logs = ref([
  ...
  { time: "10:42:05", type: "INFO", message: "Syncing blockchain data..." },
  {
    time: "10:42:12",
    type: "WARN",
    message: "High latency detected on node US-EAST-4",
  },

...
export type FileSystemWebSocketMessageType = typeof FILE_SYSTEM_WS_MESSAGE_TYPE[keyof typeof FILE_SYSTEM_WS_MESSAGE_TYPE];
...
export function useFileSystemWebSocket() {
  ...
  // 消息处理器映射
  const messageHandlers = new Map<FileSystemWebSocketMessageType, Set<(message: FileSystemWebSocketMessage) => void>>();
  ...
  const handleMessage = (message: FileSystemWebSocketMessage) => {
    try {
      ...
      // 处理系统状态更新
      if (message.messageType === FILE_SYSTEM_WS_MESSAGE_TYPE.SYSTEM_STATUS && message.data) {
        Object.assign(realtimeStatus, message.data);
      }

      // 处理队列状态更新
      if (message.messageType === FILE_SYSTEM_WS_MESSAGE_TYPE.QUEUE_STATUS && message.data) {
...
export function useFileUploadProgress() {
  ...
  // 监听上传进度
  const unsubscribeProgress = onMessage(MESSAGE_TYPE.UPLOAD_PROGRESS, (message) => {
  ...
  // 监听上传完成
  const unsubscribeCompleted = onMessage(MESSAGE_TYPE.UPLOAD_COMPLETED, (message) => {
    if (message.fileId) {

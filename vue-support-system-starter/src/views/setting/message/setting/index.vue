            ...
            <scTable ref="table"  :apiObj="apiObj" height="auto" paginationLayout="total, prev, pager, next" hideDo>
                ...
                <el-table-column label="配置类型" prop="configType" >
                    <template #default="scope">
                        <el-tag>{{ showType(scope.row.configType) }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="详情" prop="configType" width="300">
                    <template #default="scope">
                        <div v-if="scope.row.configType == 'MAIL'">
                            主体邮箱: {{ scope.row.configAccessKey }}
                        </div>
                    </template>
                </el-table-column>
...
export default {
    ...
    data() {
        return {
            ...
            returnData: [],
            returnTotal: 0,
            apiObj: this.$API.system.messageConfig.page
        }
    },
    computed: {
        _height() {
    ...
    methods: {
        ...
        doDelete(item) {
            this.$API.system.messageConfig.delete.delete(item).then(res => {
                if (res.code != '00000') {

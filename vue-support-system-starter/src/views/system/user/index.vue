...
function handleStatusChange(row: { [key: string]: any }) {
	...
	ElMessageBox.confirm("确认要" + text + row.username + "用户吗?", "警告", {
	  ...
	  type: "warning",
			...
			.then(() => {
				return updateUserStatus(row.userId, row.userStatus);
			})
			.then(() => {
            ...
            >
              <template #default="scope">
                <el-popover effect="light" trigger="hover" placement="top" width="auto">
                  <template #default>
                    <div>省份: {{ scope.row.userIpProvince }}</div>
                    <div>城市: {{ scope.row.userIpCity }}</div>
                    <div>最后一次访问地址: {{ scope.row.lastIp }}</div>
                  </template>
                  <template #reference>
		...
		>
			...
			>
				...
				<el-form-item label="性别" prop="userGender">
					<el-select v-model="formData.userGender" placeholder="请选择">

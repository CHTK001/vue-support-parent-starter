...
import {DeptForm, DeptQuery, DeptVO} from "@/api/dept/types";
...
const rules = reactive({
	deptParentId: [{ required: true, message: "上级部门不能为空", trigger: "blur" }],
...
async function openDialog(parentId?: number, deptId?: number) {
  ...
  if (deptId) {
    ...
    dialog.title = "新增部门";
    formData.deptParentId = parentId ?? 0;
  }
}
...
function resetForm() {
  ...
  formData.deptParentId = 0;
  formData.deptStatus = 1;
  formData.deptSort = 1;
}
  ...
  <div class="app-container">
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column prop="deptSort" label="排序" width="100" />
        ...
        <el-table-column label="操作" fixed="right" align="left" width="200">
          <template #default="scope">
            <el-button
              ...
              link
              size="small"
              @click.stop="openDialog(scope.row.deptId, undefined)"
              ><i-ep-plus />新增
            </el-button>
            <el-button
              ...
              link
              size="small"
              @click.stop="openDialog(scope.row.deptParentId, scope.row.deptId)"
              ><i-ep-edit />编辑
            </el-button>
            <el-button
              v-hasPerm="['sys:dept:delete']"
              ...
              link
              size="small"
              @click.stop="handleDelete(scope.row.deptId)"
            >
              <i-ep-delete />删除
            </el-button>
          </template>

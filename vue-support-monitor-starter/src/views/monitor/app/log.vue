<template>
    ...
    <el-main class="nopadding">
        <scTable ref="table" :filter="filter" :dataTotal="total" :pageSize="form.pageSize" :data="data" :params="params" :initiSearch="false" height="auto" paginationLayout="total, prev, pager, next" hideDo>
            <el-table-column label="应用名称" prop="configApplicationName" width="150">
                <template #default="scope">
                    <el-tag >{{ appName}}</el-tag>
                </template>
            </el-table-column>
            <el-table-column label="环境" prop="configProfile" width="150" show-overflow-tooltip>
                <template #default="scope">
                    <el-tag >{{ profile}}</el-tag>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        open(item) {
            ...
            this.data.length = 0;
            this.apiObj.get(this.params).then(res => {
                if(res.code === '00000') {

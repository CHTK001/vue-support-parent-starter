...
const CAMERA_Z = 3.4;
const MODEL_URL = "/models/bee_minecraft.glb";

/**
 * 加载蜜蜂模型
 */
async function loadBeeModel(): Promise<{
  group: Group;
  dispose: () => void;
}> {
...
async function initThreeRenderer(): Promise<void> {
  ...
  try {
    const beeModel = await loadBeeModel();
    rootGroup = beeModel.group;
    modelGroup = beeModel.group;
    scene.add(rootGroup);
    ...
    console.error("[AI外观][蜜蜂模型]加载失败:", error);
    disposeRenderer();
  }
}
...
.three-shell {
  ...
  border-radius: 50%;
  overflow: hidden;
  background: radial-gradient(circle at 30% 0, #fbbf24, #eab308);
  box-shadow:

...
console.log('🚀 lay-tag 初始主题:', currentTheme.value);

const handleThemeChange = (themeKey: string) => {
  console.log('🎨 lay-tag 收到主题变化:', themeKey);
...
watch(storageTheme, (newTheme) => {
  if (newTheme && newTheme !== currentTheme.value) {
    console.log('🔄 lay-tag 检测到 storage 主题变化:', newTheme);
  ...
  mutations.forEach((mutation) => {
    if (mutation.type === 'attributes' && mutation.attributeName === 'data-skin') {
      ...
      if (newTheme !== currentTheme.value) {
        console.log('🔄 lay-tag 检测到 data-skin 属性变化:', newTheme);
        currentTheme.value = newTheme;
      }
    }
  });
...
<template>
  <DefaultTag v-if="currentTheme === 'default'" />
  <SpringFestivalTag v-else-if="currentTheme === 'spring-festival'" />
  <CyberpunkTag v-else-if="currentTheme === 'cyberpunk'" />
  <MidAutumnTag v-else-if="currentTheme === 'mid-autumn'" />
  <ChristmasTag v-else-if="currentTheme === 'christmas'" />
  <NewYearTag v-else-if="currentTheme === 'new-year'" />
  <DefaultTag v-else />
</template>

  ...
  <div class="coord-transform-example">
    ...
    <div class="control-panel">
      ...
      <div class="result-panel">
        <h4>转换结果：</h4>
        <div v-if="results.wgs84">
          WGS84: {{ results.wgs84[0].toFixed(6) }},
          {{ results.wgs84[1].toFixed(6) }}
        </div>
        <div v-if="results.gcj02">
          GCJ02: {{ results.gcj02[0].toFixed(6) }},
          {{ results.gcj02[1].toFixed(6) }}
        </div>
        <div v-if="results.bd09">
          BD09: {{ results.bd09[0].toFixed(6) }},
          {{ results.bd09[1].toFixed(6) }}
        </div>
      </div>
    </div>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const addMarker = () => {
      ...
      const transformed = gcoord.value.transform(
        [lngNum, latNum],
        sourceSystem.value,
        targetSystem,
      );
    ...
    const transformCoord = () => {
      ...
      if (sourceSystem.value === CoordSystem.WGS84) {
        results.value = {
          wgs84: input,
          gcj02: gcoord.value.transform(
            input,
            CoordSystem.WGS84,
            CoordSystem.GCJ02,
          ),
          bd09: gcoord.value.transform(
            input,
            CoordSystem.WGS84,
            CoordSystem.BD09,
          ),

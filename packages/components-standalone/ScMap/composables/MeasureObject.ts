...
export class MeasureObject {
  ...
  private eventListeners: {
    [key in MeasureEventType]: MeasureEventListener[];
  } = {
    "measure-end": [],
  };
  ...
  private addDistanceLabel(
    ...
    // 计算角度以确定标签位置偏移
    const angle =
      (Math.atan2(point2.lat - point1.lat, point2.lng - point1.lng) * 180) /
      Math.PI;
    ...
    if (this.points.length > 2) {
      ...
      const isLastPointFromDblClick =
        !this.enabled && point2.equals(this.points[this.points.length - 1]);
      ...
      if (!isLastPointFromDblClick) {
        ...
        // 格式化累计距离
        const formattedCumulativeDistance =
          this.formatDistance(cumulativeDistance);

...
export class CoordinateObject {
  ...
  private mouseMoveHandler: ((event: L.LeafletMouseEvent) => void) | null =
    null;
  ...
  constructor(mapInstance: L.Map, options: CoordinateOptions = {}) {
    ...
    this.options = {
      ...
      showProjected:
        options.showProjected !== undefined ? options.showProjected : true,
    };
    ...
    this.currentInfo = {
      ...
      y: 0,
      projection:
        this.mapInstance.options.crs === L.CRS.EPSG3857 ? "EPSG:3857" : "GCJ02", // 根据地图CRS判断投影类型
  ...
  private handleMouseMove(event: L.LeafletMouseEvent): void {
    ...
    const point = this.mapInstance.project(
      latlng,
      this.mapInstance.getMaxZoom(),
    );
  ...
  public setCoordinate(coordinate: number[]): void {
    ...
    if (this.gcoord) {
      transformedLngLat = this.gcoord.transform(
        [lng, lat],
        CoordSystem.WGS84,
      ) as [number, number];
    }
    ...
    // 更新坐标信息
    const point = this.mapInstance.latLngToContainerPoint(
      L.latLng(transformedLngLat[1], transformedLngLat[0]),
    );
    ...
    this.currentInfo = {
      ...
      projected: [point.x, point.y],
      formatted: this.formatCoordinate(
        transformedLngLat[0],
        transformedLngLat[1],
      ),
    };
  ...
  public lnglatToPoint(lnglat: [number, number]): [number, number] {
    ...
    if (this.gcoord) {
      transformedLngLat = this.gcoord.transform(lnglat, CoordSystem.WGS84) as [
        number,
        number,
      ];
    }
  ...
  public pointToLnglat(point: [number, number]): [number, number] {
    const latlng = this.mapInstance.containerPointToLatLng(
      L.point(point[0], point[1]),
    );

    // 使用GcoordObject反向转换坐标从地图坐标系到WGS84
    let result: [number, number] = [latlng.lng, latlng.lat];
    if (this.gcoord) {
      result = this.gcoord.transform(
        [latlng.lng, latlng.lat],
        this.gcoord.getTargetSystem(),
        CoordSystem.WGS84,
      ) as [number, number];
    }
  ...
  private formatCoordinate(
    lng: number,
    lat: number,
  ): { lng: string; lat: string; x?: string; y?: string } {
    const decimals = this.options.decimals || 6;
    return {

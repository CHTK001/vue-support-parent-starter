...
const checkClusterPlugin = (): boolean => {
  if (!L.MarkerClusterGroup) {
    console.warn(
      "L.MarkerClusterGroup 未找到。请确保已安装 leaflet.markercluster 库。",
    );
...
export class MarkerObject {
  ...
  constructor(mapInstance: L.Map, config?: MarkerConfig) {
    ...
    this.zoomEndHandler = () => {
      ...
      if (
        this.clusterEnabled &&
        this.clusterLayer &&
        (!this.clusterLayer._map || !this.clusterLayer._map.getBounds)
      ) {
  ...
  private reopenTooltipsAfterZoom(): void {
    try {
      ...
      this.markers.forEach((marker) => {
        try {
          if (
            marker.options._reopenTooltipAfterZoom &&
            marker.getTooltip() &&
            marker.getTooltip() &&
  ...
  public addMarker(options: MarkerOptions): string {
    ...
    try {
      ...
      if (
        ...
        isNaN(options.position[0]) ||
        isNaN(options.position[1])
      ) {
      ...
      if (
        Math.abs(options.position[0]) > 90 &&
        Math.abs(options.position[1]) <= 90
      ) {
        // 可能是[lng, lat]格式，需要交换
        lat = options.position[1];
        lng = options.position[0];
        logger.debug(
          `检测到坐标顺序为[lng, lat]，已自动转换为[lat, lng]: [${lat}, ${lng}]`,
        );
      }
      ...
      if (options.icon?.iconUrl) {
        ...
        options.group &&
        this.config.groupIcon &&
        this.config.groupIcon[options.group]
      ) {
        // 使用分组图标
        ...
        if (typeof groupIcon === "string") {
          ...
          icon = L.icon({
            ...
            iconAnchor: groupIcon.iconAnchor ||
              options.icon?.iconAnchor || [16, 32],
            popupAnchor: groupIcon.popupAnchor ||
              options.icon?.popupAnchor || [0, -40],
      ...
      (marker.options as any)._clickContentTemplate =
        options.clickContentTemplate;
      ...
      if (options.title && (options.showLabel || false) && this.labelsVisible) {
        ...
        try {
          marker.bindTooltip(options.title, {
            ...
            offset: options.labelOptions?.offset
              ? L.point(
                  options.labelOptions.offset[0],
                  options.labelOptions.offset[1],
                )
              : undefined,
  ...
  public setClickListener(
    listener: (event: L.LeafletMouseEvent) => void,
  ): void {
    ...
    try {
      ...
      if (options.position) {
        ...
        if (
          ...
          lng = options.position[0];
          logger.debug(
            `updateMarker: 检测到坐标顺序为[lng, lat]，已自动转换为[lat, lng]: [${lat}, ${lng}]`,
          );
        }
      ...
      if (
        ...
        options.icon?.html ||
        (options.group && this.config.groupIcon[options.group])
      ) {
        let icon: L.Icon | L.DivIcon;
        ...
        if (options.icon?.iconUrl) {
          ...
          if (typeof groupIcon === "string") {
            ...
            icon = L.icon({
              iconUrl: groupIcon.iconUrl || "",
              iconSize: groupIcon.iconSize ||
                options.icon?.iconSize || [32, 32],
              iconAnchor: groupIcon.iconAnchor ||
                options.icon?.iconAnchor || [16, 32],
              popupAnchor: groupIcon.popupAnchor ||
                options.icon?.popupAnchor || [0, -40],
  ...
  public destroy(): void {
    ...
    if (
      this.clusterEnabled &&
      this.clusterLayer &&
      this.mapInstance.hasLayer(this.clusterLayer)
    ) {
  ...
  public checkMarkerClick(pixel: number[]): { id: string } | null {
    // 将像素坐标转换为地图坐标
    const point = this.mapInstance.containerPointToLatLng(
      L.point(pixel[0], pixel[1]),
    );
  ...
  public setClusterClickHandler(
    handler?: (features: any[], coordinate: number[]) => void,
  ): void {
    // 保存点击处理函数
    ...
    try {
      ...
      if (this.clusterOptions.onClusterClick) {
        this.clusterLayer.on("clusterclick", (e: any) => {
          ...
          const latlng = cluster.getLatLng();
          this.clusterOptions.onClusterClick!(features, [
            latlng.lat,
            latlng.lng,
          ]);
        });

...
const addRandomMarkers = (count: number) => {
  for (let i = 0; i < count; i++) {
    ...
    addLog(
      `添加随机点 ${i + 1}: WGS84 [${lat.toFixed(6)}, ${lon.toFixed(6)}] -> EPSG:3857 [${epsg3857Coord[0].toFixed(2)}, ${epsg3857Coord[1].toFixed(2)}]`,
    );
  }
};
...
const addWgs84Marker = () => {
  ...
  if (Array.isArray(epsg3857Point)) {
    ...
    addLog(
      `添加WGS84点: [${wgs84Point[0].toFixed(6)}, ${wgs84Point[1].toFixed(6)}] -> EPSG:3857 [${epsg3857Point[0].toFixed(2)}, ${epsg3857Point[1].toFixed(2)}]`,
    );
  }
};
...
const addGcj02Marker = () => {
  ...
  if (Array.isArray(epsg3857Point)) {
    ...
    addLog(
      `添加GCJ02点: [${gcj02Point[0].toFixed(6)}, ${gcj02Point[1].toFixed(6)}] -> EPSG:3857 [${epsg3857Point[0].toFixed(2)}, ${epsg3857Point[1].toFixed(2)}]`,
    );
  }
};
...
const displayCoordinate = (wgs84Coords: [number, number]) => {
  // 转换为EPSG:3857坐标
  const epsg3857Coords = GcoordUtils.wgs84ToEpsg3857Precise(
    wgs84Coords[0],
    wgs84Coords[1],
  );

  // 转换为当前地图坐标系
  const mapCoords = GcoordUtils.epsg3857ToMapCoord(
    epsg3857Coords,
    mapType.value,
  );

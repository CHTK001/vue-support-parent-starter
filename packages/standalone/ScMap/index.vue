/** * 地图组件 - Leaflet版本 * @author 根据ScLayer组件移植实现 */
<template>
  <div class="sc-map" :style="{ height: config.height + 'px' }">
    ...
    <LayerPanel
      ...
      :map-config="configObject?.getMapConfig() || DEFAULT_MAP_CONFIG"
    ...
    <!-- 增加引入OverviewMap组件 -->
    <OverviewMap
      v-if="showOverviewMap && mapObj?.getMapInstance()"
      :main-map="mapObj.getMapInstance()!"
...
const mapToolbarRef =
  ref<
    ComponentPublicInstance<{ setToolbarObj: (obj: ToolbarObject) => void }>
  >();
...
const initMap = async () => {
  try {
    ...
    // 设置工具状态变化回调
    toolbarObject.setToolStateChangeCallback(
      (toolId, active, toolType, data) => {
    ...
    if (markerObject) {
      markerObject.setClickListener((event: any) => {
        ...
        if (event.markerData) {
          ...
          if (
            markerData.clickContentTemplate ||
            markerData.content ||
            markerData.title
          ) {
          ...
          // 如果有弹窗内容或点击模板，显示面板
          if (
            markerInfo.clickContentTemplate ||
            markerInfo.popupContent ||
            markerInfo.title
          ) {
            activeMarker.value = {
              ...markerInfo,
...
const determineLayerPanelPosition = ():
  ...
  if (toolbarObject) {
    ...
    const toolbarPosition =
      toolbarConfig.position || (DEFAULT_TOOLBAR_CONFIG.position as string);
    const toolbarDirection =
      toolbarConfig.direction || (DEFAULT_TOOLBAR_CONFIG.direction as string);
...
const handleTrackPlayerCollapseChange = (collapsed: boolean) => {
  if (toolbarObject) {
    // 通知工具栏轨迹播放器状态已变更
    toolbarObject.triggerToolStateChange(
      "trackPlayer",
      !collapsed,
      "TRACK_PLAYER",
    );
...
// 显示标记点面板
const showMarkerPanelForMarker = (
  marker: any,
...
defineExpose({
  ...
  updateMarker: (id: string, options: Partial<MarkerOptions>) =>
    markerObject?.updateMarker(id, options),
  ...
  updateShape: (id: string, options: Partial<ShapeOption>) =>
    shapeObject?.updateShape(id, options),

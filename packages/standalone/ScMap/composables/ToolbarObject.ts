...
export class ToolbarObject {
  ...
  private coordinateCallback: ((coordinate: CoordinateInfo) => void) | null =
    null;
  ...
  private initOverviewMapObject(): void {
    ...
    // 仅保留空实现，确保API兼容性
    logger.debug(
      "[Overview] 跳过内置鹰眼对象初始化，使用自定义OverviewMap组件",
    );
  ...
  handleToolClick(toolId: string): void {
    ...
    logger.debug(
      `处理工具点击: ${toolId}, 当前状态: ${tool.active ? "激活" : "未激活"}`,
    );
  ...
  handleSubMenuClick(parentToolId: string, toolId: string): void {
    ...
    logger.debug(
      `处理子菜单项点击: ${parentToolId} > ${toolId}, 当前状态: ${tool.active ? "激活" : "未激活"}`,
    );
  ...
  activateTool(toolId: string): void {
    ...
    logger.debug(
      `正在激活工具 ${toolId}, 多选模式: ${tool.multi ? "是" : "否"}`,
    );
    ...
    // 如果是绘制工具，检查删除工具是否激活
    if (isDrawingTool) {
      const deleteToolActive = this.tools.find(
        (t) => t.id === "clear-shapes",
      )?.active;
      if (deleteToolActive) {
        // 先停用删除工具
      ...
      // 检查编辑工具是否激活
      const editToolActive = this.tools.find(
        (t) => t.id === "edit-shape",
      )?.active;
      if (editToolActive) {
        // 先停用编辑工具
  ...
  deactivateTool(toolId: string): void {
    ...
    logger.debug(
      `正在停用工具 ${toolId}，当前活动工具ID: ${this.activeToolId}`,
    );
    ...
    switch (tool.id) {
      ...
      case "clear-shapes":
        ...
        if (this.shapeObj && (this.shapeObj as any).isEditMode()) {
          ...
          // 取消删除工具的激活状态
          const clearShapesTool = this.tools.find(
            (t) => t.id === "clear-shapes",
          );
          if (clearShapesTool) {
  ...
  public setClusterConfig(config: Partial<AggregationOptions>): void {
    ...
    if (
      typeof this.clusterConfig.maxClusterRadius === "number" &&
      this.clusterConfig.maxClusterRadius > 0
    ) {
      // 确保半径是有效的像素值
      ...
      logger.debug(
        `已将聚合半径设置为 ${this.clusterConfig.maxClusterRadius} ${this.clusterConfig.radiusUnit} (约 ${distanceInPixels} 像素)`,
      );
    }
  ...
  private enableCluster(): void {
    ...
    if (
      ...
      this.markerObj.setClusterDistance(distanceInPixels);
      logger.debug(
        `聚合半径设置为 ${this.clusterConfig.maxClusterRadius} ${this.clusterConfig.radiusUnit} (约 ${distanceInPixels} 像素)`,
      );
    }
  ...
  private setupClusterClickHandler(): void {
    ...
    if (this.clusterConfig.zoomToBoundsOnClick) {
      ...
      this.markerObj.setClusterClickHandler((features, coordinate) => {
        // 仅记录信息，不执行缩放操作
        logger.info(
          `点击了聚合点，包含 ${features.length} 个标记点，坐标: [${coordinate[0].toFixed(2)}, ${coordinate[1].toFixed(2)}]`,
        );
      });
  ...
  private handleTrackPlayerActivate(): void {
    // 确保工具真正被标记为激活
    const trackPlayerTool = this.tools.find(
      (tool) => tool.id === "track-player",
    );
    if (trackPlayerTool) {
    ...
    // 触发工具状态变化事件，传递轨迹对象
    this.triggerToolStateChange("track-player", true, "track-player", {
  ...
  public getTrackObject(): LeafletTrackplayerObject {
    if (!this.trackObj) {
      this.trackObj = new LeafletTrackplayerObject(
        this.mapObj.getMapInstance(),
      );
    }
  ...
  private handleGridActivate(gridType: GridType): void {
    ...
    logger.debug(
      `${gridType === GridType.GEOHASH ? "GeoHash" : "蜂窝"}网格已启用`,
    );
  ...
  private handleGridDeactivate(gridType: GridType): void {
    ...
    logger.debug(
      `${gridType === GridType.GEOHASH ? "GeoHash" : "蜂窝"}网格已禁用`,
    );
  ...
  public triggerToolStateChange(
    ...
    if (this.toolStateChangeCallback) {
      try {
        ...
        logger.debug(
          `已触发工具状态变化回调: ${toolId}, 激活状态: ${active}, 类型: ${toolType}`,
        );
      } catch (error) {
  ...
  private activateDeleteMode(): void {
    ...
    const deleteClickListener = (evt: any) => {
        ...
        mapInstance.forEachFeatureAtPixel(
          ...
          (feature) => {
            ...
            if (data && data.dataType === DataType.SHAPE) {
              ...
              // 检查是否处于编辑模式
              if (
                (this.shapeObj as any).isEditMode() ||
                (editTool && editTool.active)
              ) {
      ...
      if (!hasDeleted && this.markerObj) {
        ...
        if (
          this.shapeObj &&
          ((this.shapeObj as any).isEditMode() || (editTool && editTool.active))
        ) {
        ...
        if (!hasDeleted) {
          ...
          for (const [id, markerObj] of allMarkers) {
            ...
            if (id && visible && position) {
              ...
              // 确保坐标是地图使用的投影
              const markerMapCoord =
                mapInstance.getView().getProjection().getCode() === "EPSG:4326"
                  ? markerCoord
          ...
          if (closestMarkerId) {
            logger.debug(
              `找到距离点击位置最近的标记点: ${closestMarkerId}, 距离: ${minDistance}`,
            );
  ...
  private deactivateDeleteMode(): void {
    ...
    try {
      ...
      const events = (mapInstance as any).hasOwnProperty("ol_lm")
        ? (mapInstance as any).ol_lm.click
        : null;

      if (events && Array.isArray(events) && events.length > 0) {
        logger.debug(
          `地图上存在 ${events.length} 个点击监听器，检查是否需要清理`,
        );
        // 暂不删除其他监听器，避免影响其他功能
      }
    } catch (error) {
  ...
  private handleShapeEditActivate(): void {
    ...
    // 设置图形更新回调
    (this.shapeObj as any).setShapeUpdateCallback(
      (id: string, shapeType: string, feature: any) => {
    ...
    if (success) {
      ...
      if (mapInstance && mapInstance.getContainer()) {
        ...
        mapInstance.getContainer().title =
          "点击图形开始编辑，拖动控制点修改形状，点击地图空白处完成编辑";
      }
  ...
  private handleShapeEditDeactivate(): void {
    ...
    // 检查是否在编辑模式，记录编辑的图形ID
    const editInfo = (this.shapeObj as any).isEditMode()
      ? (this.shapeObj as any).getEditModeInfo()
      : null;

...
export class GeoHashGridObject {
  ...
  constructor(
    ...
    options?: Partial<GeohashGridOptions>,
  ) {
    if (mapInstance) {
  ...
  private initGridLayer(): void {
    ...
    try {
      ...
      const CustomGridLayer = L.GridLayer.extend({
        ...
        createTile: function (coords: any) {
          ...
          const se = map.unproject(
            [(coords.x + 1) * size, (coords.y + 1) * size],
            coords.z,
          );
          ...
          tile.style.backgroundColor = this.adjustColorOpacity(
            backgroundColor,
            self.options.gridOpacity || 0.35,
          );
      ...
      if (this.initAttempts < this.maxInitAttempts) {
        setTimeout(() => {
          logger.info(
            `[GeohashGrid] 正在重试初始化网格图层 (${this.initAttempts}/${this.maxInitAttempts})...`,
          );
          this.initGridLayer();
        }, 500);
      }
    ...
    try {
      ...
      const handleClick = (e: any) => {
        ...
        if (this.options.autoAdjustLevel) {
          precision = Math.max(
            1,
            Math.min(12, Math.ceil(this.mapInstance!.getZoom() / 2)),
          );
        }
  ...
  private highlightGeohash(geohash: string, bounds: any): void {
    ...
    try {
      ...
      if (this.highlightLayer._path) {
        this.highlightLayer._path.style.transition =

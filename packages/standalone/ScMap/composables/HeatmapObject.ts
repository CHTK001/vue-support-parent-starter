  ...
  private initLayers(): void {
    ...
    try {
      ...
      this.points.forEach((point, id) => {
        heatData.push([point.lat, point.lng, point.weight || 1]);
      });
  ...
  public addPoint(point: HeatmapPoint): string {
    ...
    try {
      ...
      if (this.heatLayer && this.mapInstance) {
        this.heatLayer.addLatLng([point.lat, point.lng, point.weight || 1]);
  ...
  public addPoints(points: HeatmapPoint[]): string[] {
    ...
    try {
      ...
      if (this.heatLayer && this.mapInstance) {
        ...
        // 转换所有点为热力图数据格式
        const heatData = Array.from(this.points.values()).map(

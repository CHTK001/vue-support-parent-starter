...
export class BaiduSearchProvider implements SearchDataProvider {
  ...
  async searchPlaces(
    ...
    try {
      ...
      logger.debug(
        `[BaiduSearchProvider] 搜索成功，找到 ${data.results?.length || 0} 条结果`,
      );
  ...
  async getNavigation(
    ...
    try {
      logger.debug(
        `[BaiduSearchProvider] 开始获取导航路径: ${origin} -> ${destination}, 交通方式: ${transportType}`,
      );
      ...
      logger.debug(
        `[BaiduSearchProvider] 使用API: ${apiUrl} 获取 ${transportType} 导航路径`,
      );

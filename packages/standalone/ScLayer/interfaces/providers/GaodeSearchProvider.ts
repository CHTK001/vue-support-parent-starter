...
export class GaodeSearchProvider implements SearchDataProvider {
  ...
  private readonly NAVIGATION_URL =
  ...
  async searchPlaces(
    ...
    try {
      ...
      logger.debug(
        `[GaodeSearchProvider] 搜索成功，找到 ${data.pois?.length || 0} 条结果`,
      );
  ...
  async getNavigation(
    ...
    try {
      logger.debug(
        `[GaodeSearchProvider] 开始获取导航路径: ${origin} -> ${destination}, 交通方式: ${transportType}`,
      );
      ...
      logger.debug(
        `[GaodeSearchProvider] 使用API: ${apiUrl} 获取 ${transportType} 导航路径`,
      );
  ...
  private convertTransitResponse(response: any): NavigationApiResponse {
    try {
      ...
      if (
        !response ||
        !response.route ||
        !response.route.transits ||
        response.route.transits.length === 0
      ) {
        ...
        if (transit.segments && transit.segments.length > 0) {
          transit.segments.forEach((segment: any) => {
            ...
            if (
              ...
              segment.walking.steps.length > 0
            ) {
            ...
            if (
              segment.railway &&
              segment.railway.spaces &&
              segment.railway.spaces.length > 0
            ) {
  ...
  async searchNearby(
    ...
    try {
      ...
      // 确保坐标是GCJ02坐标系
      const gcj02Coords = this.ensureGcj02Coordinates([
        location[0],
        location[1],
      ]);
      ...
      logger.debug(
        `[GaodeSearchProvider] 附近搜索成功，找到 ${data.pois?.length || 0} 条结果`,
      );
  ...
  async searchDistrict(
    ...
    try {
      ...
      logger.debug(
        `[GaodeSearchProvider] 行政区搜索成功，找到 ${data.districts?.length || 0} 条结果`,
      );

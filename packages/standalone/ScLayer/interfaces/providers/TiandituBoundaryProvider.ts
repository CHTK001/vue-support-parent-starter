import {
  ...
  BoundaryDataFormat,
  CoordinatePoint,
} from "../BoundaryDataProvider";
import { fetchTiandituBoundary } from "../../api/district";
 ...
 * 实现从天地图API获取区划数据
 */
export class TiandituBoundaryProvider implements BoundaryDataProvider {
  /**
   * 获取区划数据
   * @param adcode 行政区划代码
   * @param apiKey API密钥
  ...
  async fetchBoundaryData(
    ...
    try {
      logger.debug(`[TiandituBoundaryProvider] 开始获取区划数据: ${adcode}`);

      // 调用天地图API获取区划数据
      const data = await fetchTiandituBoundary({
        key: apiKey,
        url: url,
        adcode: adcode,
      });

      logger.debug(`[TiandituBoundaryProvider] 成功获取区划数据: ${adcode}`);

      // 将天地图数据转换为标准格式
      return this.convertToGaodeFormat(data, { adcode });
    } catch (error) {
      logger.error(
        `[TiandituBoundaryProvider] 获取区划数据失败: ${adcode}`,
        error,
      );
      throw error;
    }
  ...
  convertToGaodeFormat(data: any, options?: any): BoundaryDataFormat {
    ...
    if (data && data.geometry && data.geometry.coordinates) {
      // 将天地图坐标系的多边形坐标转换为坐标点数组
      polyline = this.convertCoordinatesToPolylineArray(
        data.geometry.coordinates,
      );
    } else if (data && data.bounds) {
      // 如果有bounds字段，解析为坐标点数组
      polyline = this.parsePolylineString(data.bounds, CoordSystem.EPSG4490);
    }
    ...
    const result: BoundaryDataFormat = {
      ...
      sourceCoordType: CoordSystem.EPSG4490,
    };
   ...
   * @returns 坐标点数组
   */
  private convertCoordinatesToPolylineArray(
    coordinates: number[][],
  ): CoordinatePoint[][] {
    ...
    const points: CoordinatePoint[] = coordinates.map((coord) => {
      ...
      try {
        ...
        logger.error(`天地图坐标转换失败: ${lng},${lat}`, error);
        // 使用备选方法：EPSG:4490与WGS84非常接近，可以直接使用wgs84ToEpsg3857Precise
        const epsg3857Coords = GcoordUtils.wgs84ToEpsg3857Precise(lat, lng);
   ...
   * @returns 坐标点数组
   */
  private parsePolylineString(
    polyline: string,
    sourceCoordType: CoordSystem,
  ): CoordinatePoint[][] {
    if (!polyline) {
      return [];
    }

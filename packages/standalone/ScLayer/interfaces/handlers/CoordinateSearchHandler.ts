    ...
    try {
      ...
      if (options.projection && options.projection !== CoordSystem.WGS84) {
        try {
          ...
          logger.debug(
            `[CoordinateSearchHandler] 坐标转换成功: WGS84(${lng}, ${lat}) -> EPSG3857(${epsg3857Coords.lng}, ${epsg3857Coords.lat})`,
          );
        } catch (error) {
          logger.warn(
            `[CoordinateSearchHandler] 坐标转换失败: ${error.message}`,
          );
        }
      }
      ...
      // 记录搜索结果
      logger.debug(
        `[CoordinateSearchHandler] 坐标搜索结果: ${JSON.stringify(result)}`,
      );

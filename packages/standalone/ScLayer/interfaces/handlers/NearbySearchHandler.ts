...
export class NearbySearchHandler implements SearchHandler {
  ...
  async handleSearch(
    ...
    try {
      ...
      // 执行附近搜索
      logger.debug(
        `[NearbySearchHandler] 执行附近搜索，关键词: ${keyword}, 位置: ${(options as any).location}`,
      );
      const results = await searchProvider.searchNearby(
        searchUrl,
        keyword,
        searchOptions,
      );
  ...
  getCacheKey(keyword: string, options: SearchOptions): string {
    ...
    const location = (options as any).location
      ? `${(options as any).location[0]},${(options as any).location[1]}`
      : "unknown";

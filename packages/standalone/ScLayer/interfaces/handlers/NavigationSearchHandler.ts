...
export class NavigationSearchHandler implements SearchHandler {
  ...
  async handleSearch(
    ...
    try {
      ...
      if (!origin || !destination) {
        logger.warn(
          '[NavigationSearchHandler] 导航搜索格式无效，请使用"起点-终点"格式',
        );
        return [];
      }
      ...
      const originResults = await searchProvider.search(
        searchUrl,
        origin,
        options,
      );
      ...
      const destinationResults = await searchProvider.search(
        searchUrl,
        destination,
        options,
      );
   ...
   * @returns 起点和终点的数组
   */
  private parseNavigationKeyword(
    keyword: string,

...
export class IconUtils {
    ...
    if (this.isIconValid(url)) {
      try {
        ...
        if (url.startsWith("http") || url.startsWith("https")) {
          ...
          this.log("debug", `使用ol-ext Photo样式处理远程图片: ${url}`, {
            size,
            photoOptions,
          });
          ...
          if (photoOptions?.shadow) {
            ...
            photoConfig.shadowColor =
              photoOptions.shadowColor || "rgba(0,0,0,0.5)";
          }
          ...
          if (url.startsWith("<svg")) {
            try {
              ...
              try {
                const svgDataUrl =
                  "data:image/svg+xml," + encodeURIComponent(url);
                iconOptions.src = svgDataUrl;
   ...
   * @returns 图标样式
   */
  public static createFontSymbolStyle(
    options: IconOptions,
    text: string = "",
    ...
    return new Style({
      ...
      renderer: function (coords, state) {
        ...
        if (shadowStyle && typeof shadowStyle.renderPoint === "function") {
          ...
          shadowStyle &&
          typeof shadowStyle.drawPoint_ === "function"
        ) {
   ...
   * @returns Photo样式
   */
  public static createPhotoStyle(
    imageUrl: string,
    options: {
      radius?: number; // 图标半径
    ...
    try {
      ...
      // 失败时返回默认样式
      return this.createSafeIconStyle(this.defaultIconUrl, scale, [
        radius * 2,
        radius * 2,
      ]);
    }
  ...
  public static createMarkerStyleFromOptions(
    ...
    const { iconType = "default", style = {}, zIndex = 10000 } = options as any;
    // 默认样式
    const defaultStyle = {
      scale: 1,
    ...
    if (size) {
      ...
      this.log(
        "debug",
        `使用size参数计算等比例缩放: [${size[0]}, ${size[1]}] => scale=${sizeScale.toFixed(2)}`,
      );
    }
    ...
    switch (iconType) {
      case "photo":
        ...
        if (icon && (icon.startsWith("http") || icon.startsWith("https"))) {
          ...
          const photoOptions = {
            ...
            stroke:
              options.data?.photoStroke !== undefined
                ? options.data?.photoStroke
                : 2,
          ...
          this.log("debug", `使用Photo样式渲染URL图标: ${icon}`, {
            photoOptions,
          });
      ...
      case "url":
        ...
        if (icon && (icon.startsWith("http") || icon.startsWith("https"))) {
          this.log(
            "debug",
            `使用标准Icon样式渲染URL图标: ${icon}, scale=${finalScale}`,
          );
      ...
      case "svg":
        ...
        if (icon && icon.startsWith("<svg")) {
          try {
            const svgDataUrl =
              "data:image/svg+xml;base64," +
              btoa(unescape(encodeURIComponent(icon)));
            iconUrl = svgDataUrl;
          } catch (e) {
            this.log("error", `SVG转换失败: ${e.message}`);
          }

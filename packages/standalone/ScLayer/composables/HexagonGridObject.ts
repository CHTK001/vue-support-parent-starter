...
export class HexagonGridObject {
  ...
  constructor(
    ...
    config?: Partial<HexagonGridConfig>,
  ) {
    if (mapInstance) {
  ...
  private updateHexagonGrid(): void {
    ...
    // 获取当前视图范围
    const extent = this.mapInstance
      .getView()
      .calculateExtent(this.mapInstance.getSize() || [0, 0]);
  ...
  private renderHexagon(
    hexagon: { id: string; coordinates: number[][] },
    source: VectorSource,
  ): void {
    // 创建多边形特征
    const feature = new Feature({
      geometry: new Polygon([
        hexagon.coordinates.map((coord) => fromLonLat(coord)),
      ]),
      id: hexagon.id,
      type: "hexagon",
  ...
  private generateHexagonGrid(
    ...
    // 转换为经纬度单位
    const latHeight =
      hexHeight / (metersPerDegree * Math.cos((bottomLeft[1] * Math.PI) / 180));
    ...
    for (let row = 0; row < numRows; row++) {
      ...
      for (let col = 0; col < numCols; col++) {
        ...
        // 生成六边形顶点坐标
        const coordinates = this.generateHexagonVertices(
          centerLat,
          centerLng,
          latHeight / 2,
          lngWidth / 2,

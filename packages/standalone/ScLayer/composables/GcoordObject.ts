...
export class GcoordObject {
  ...
  constructor(mapType: MapType = MapType.GAODE) {
    ...
    logger.debug(
      `坐标系统对象初始化完成，当前地图类型: ${mapType}，地图坐标系统: ${this.mapCoordSystem}，内部坐标系统: ${this.internalCoordSystem}`,
    );
  }
  ...
  public setMapType(mapType: MapType): void {
    ...
    this.mapCoordSystem = this.getCoordSystemByMapType(mapType);
    logger.debug(
      `坐标系统已更新，当前地图类型: ${mapType}，地图坐标系统: ${this.mapCoordSystem}，内部坐标系统: ${this.internalCoordSystem}`,
    );
  }
  ...
  public fromMapCoord(point: GeoPoint): GeoPoint {
    // 先转为WGS84
    const wgs84Point = GcoordUtils.transform(
      point,
      this.mapCoordSystem,
      CoordSystem.WGS84,
    );
   ...
   * @returns 转换后的坐标点
   */
  public convertFromInternalCoord(
    point: GeoPoint,
   ...
   */
  public convertPointsToInternal(
    points: GeoPoint[],
    fromCoordSystem: CoordSystem,
  ): GeoPoint[] {
    return points.map((point) =>
      this.convertToInternalCoord(point, fromCoordSystem),
    );
  }
   ...
   * @returns 转换后的坐标点数组
   */
  public convertPointsFromInternal(
    points: GeoPoint[],
    toCoordSystem: CoordSystem,
  ): GeoPoint[] {
    return points.map((point) =>
      this.convertFromInternalCoord(point, toCoordSystem),
    );
  }

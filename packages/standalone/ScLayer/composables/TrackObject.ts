...
export class TrackObject {
  ...
  // 类属性中添加相机动画相关属性
  private trackCameraAnimations = new Map<
    string,
    {
      targetCenter: number[];
  ...
  // 性能模式相关属性
  private trackAnimationPositions = new Map<
    string,
    {
      position: Point;
      lastTime: number;
  ...
  constructor(mapObjectOrInstance: any | null = null, config?: TrackConfig) {
    if (mapObjectOrInstance) {
      ...
      if (
        mapObjectOrInstance.getMapInstance &&
        typeof mapObjectOrInstance.getMapInstance === "function"
      ) {
        // 如果是MapObject
  ...
  public addTrack(
    ...
    if (pointsWithoutTime.length > 0) {
      this.log(
        "warn",
        `轨迹 "${track.id}" 包含 ${pointsWithoutTime.length} 个点没有时间戳，将自动设置为当前时间`,
      );
    ...
    for (let i = 0; i < sortedPoints.length; i++) {
      if (i < sortedPoints.length - 1) {
        ...
        const cross =
          (p2.lng - base.lng) * (p1.lat - base.lat) -
          (p2.lat - base.lat) * (p1.lng - base.lng);
    ...
    if (config) {
      ...
      if (config.showNodePopovers !== undefined)
        showNodePopovers = config.showNodePopovers;
      if (config.showNodeTime !== undefined) showNodeTime = config.showNodeTime;
      if (config.showSpeedPopovers !== undefined)
        showSpeedPopovers = config.showSpeedPopovers;
      if (config.showNodeSpeeds !== undefined)
        showNodeSpeeds = config.showNodeSpeeds;
      if (config.showNodeDistance !== undefined)
        showNodeDistance = config.showNodeDistance;
      if (config.showMovingPointName !== undefined)
        showMovingPointName = config.showMovingPointName;
      if (config.enableSpeedIcon !== undefined)
        enableSpeedIcon = config.enableSpeedIcon;
      if (config.showMovingInfo !== undefined)
        showMovingInfo = config.showMovingInfo;
      if (config.showMovingDistance !== undefined)
        showMovingDistance = config.showMovingDistance;
    }
    ...
    if (typeof document !== "undefined") {
      ...
      if (trackPlayerElement) {
        trackPlayerElement.dispatchEvent(
          new CustomEvent("track-added", {
            detail: { trackId: track.id },
          }),
        );
      }
  ...
  public removeTrack(id: string): boolean {
    ...
    if (
      this.trackCurrentNodeOverlay &&
      this.trackCurrentNodeOverlay.get("trackId") === id
    ) {
  ...
  public updateTrack(id: string, track: Partial<Track>): boolean {
    ...
    if (track.points) {
      ...
      // 验证每个点都有时间戳
      const invalidPoints = updatedTrack.points.filter(
        (p) => p.time === undefined,
      );
      if (invalidPoints.length > 0) {
        this.log(
          "warn",
          `轨迹 "${id}" 包含 ${invalidPoints.length} 个没有时间戳的点，将被设置为当前时间`,
        );
        ...
        // 重新排序
        updatedTrack.points = updatedTrack.points.sort(
          (a, b) => a.time - b.time,
        );
      }
    }
    ...
    if (track.points) {
      ...
      const notPassedLineOptions = this.config.notPassedLineOptions || {
        ...
        new Style({
          stroke: new Stroke({
            color: updatedTrack.color
              ? this.applyOpacity(
                  updatedTrack.color,
                  notPassedLineOptions.opacity,
                )
              : this.applyOpacity(
                  notPassedLineOptions.color,
                  notPassedLineOptions.opacity,
                ),
      ...
      const isTrackVisible =
        updatedTrack.visible !== false && this.tracksVisible;
      const shouldShowNodes =
        this.trackNodesVisible.get(id) === true && isTrackVisible;

      if (shouldShowNodes && this.trackPointLayer) {
        pointFeatures.forEach((feature) => {
          const showNodePopover =
            this.trackNodePopoversVisible.get(id) || false;
          feature.setStyle(
            this.createTrackPointStyle(
              id,
              feature.get("pointIndex"),
              showNodePopover,
            ),
          );
      ...
      if (track.color && this.trackFeatures.has(id)) {
        ...
        feature.setStyle(
          new Style({
            stroke: new Stroke({
              color: track.color
                ? this.applyOpacity(track.color, notPassedLineOptions.opacity)
                : this.applyOpacity(
                    notPassedLineOptions.color,
                    notPassedLineOptions.opacity,
                  ),
  ...
  public showTrack(id: string): boolean {
    ...
    if (this.tracksVisible) {
      ...
      if (
        this.trackPointFeatures.has(id) &&
        this.trackPointLayer &&
        this.trackNodesVisible.get(id) === true
      ) {
        const features = this.trackPointFeatures.get(id)!;
  ...
  public play(
    id: string,
    player?: Partial<ExtendedTrackPlayer>,
    pauseIfPlaying: boolean = false,
  ): boolean {
    if (!this.tracks.has(id)) {
    ...
    if (player) {
      ...
      if (player.hasOwnProperty("enableSpeedIcon")) {
        this.trackEnableSpeedIcon.set(
          id,
          (player as any).enableSpeedIcon !== false,
        );
      }
    ...
    const useAdvancedAnimation =
      this.trackPlayers.get(id)?.useAdvancedAnimation !== false;
  ...
  private setupTrackAnimation(id: string): void {
    ...
    // 添加postrender事件监听器
    this.trackAnimationListeners.set(
      id,
      this.trackLayer.on("postrender", (event) => {
        ...
        if (currentState === TrackPlayState.PLAYING) {
          ...
          // 1. 获取轨迹的实际时间范围（秒）
          const timeRange =
            track.points[track.points.length - 1].time - track.points[0].time;

          // 2. 应用倍速因子，计算进度增量
          // 进度变化 = 经过时间(ms) / (轨迹时间范围(s) * 1000) * 速度因子
          const progressChange =
            (elapsedTime * speedFactor) / (timeRange * 1000);
          ...
          if (newProgress > 1) {
            if (player.loop) {
              ...
              for (
                let i = 0;
                i <= Math.floor(progress * (track.points.length - 1));
                i++
              ) {
                passedPoints.add(i);
              }
  ...
  private drawPassedLine(
    ...
    // 计算当前已经过的点
    const passedPointCount = Math.max(
      1,
    ...
    if (passedPointCount < track.points.length - 1) {
      ...
      const lat =
        currentPoint.lat + (nextPoint.lat - currentPoint.lat) * fraction;
    ...
    const passedLineOptions = this.config?.passedLineOptions || {
    ...
    const passedLineStyle = new Style({
      stroke: new Stroke({
        color: track.color
          ? this.applyOpacity(track.color, passedLineOptions.opacity)
          : this.applyOpacity(
              passedLineOptions.color,
              passedLineOptions.opacity,
            ),
  ...
  private drawPositionMarker(
    ...
    if (this.config.trackSpeedGroup && this.config.trackSpeedGroup.length > 0) {
      const sortedGroups = [...this.config.trackSpeedGroup].sort(
        (a, b) => b.speed - a.speed,
      );
      for (const group of sortedGroups) {
        if (realSpeed > group.speed) {
          ...
          this.log(
            "debug",
            `根据速度 ${realSpeed} km/h 选择图标: ${iconUrl} (速度阈值: ${group.speed})`,
          );
          break;
        }
      }
    ...
    if (!iconUrl && track.iconGroup && track.iconGroup.length > 0) {
      for (const group of track.iconGroup) {
        if (realSpeed >= group.speed) {
          ...
          this.log(
            "debug",
            `使用轨迹iconGroup图标: ${iconUrl} (速度范围: ${group.speed})`,
          );
          break;
        }
      }
    ...
    if (iconUrl) {
      try {
        ...
        const safeIconSize: [number, number] = [
          iconSize[0] || 24,
          iconSize[1] || 24,
        ];
        style = IconUtils.createSafeIconStyle(
          iconUrl,
          1,
          safeIconSize,
          track.color || "rgba(24, 144, 255, 1)",
          undefined,
          iconType,
        );
        ...
        this.log(
          "error",
          `创建移动点图标样式失败: ${error.message || "未知错误"}, URL: ${iconUrl}`,
        );
        style = this.createDefaultMarkerStyle(
          track.color || "rgba(24, 144, 255, 1)",
        );
      }
    } else {
    ...
    try {
      ...
      const fallbackStyle = this.createDefaultMarkerStyle(
        track.color || "rgba(24, 144, 255, 1)",
      );
    ...
    // 获取显示设置
    const movingPointNameVisible =
      this.trackMovingPointNameVisible.get(id) === true;
    ...
    if (
      showMovingInfo &&
      ((movingPointNameVisible && position.title) ||
        showSpeed ||
        showNodeDistance)
    ) {
      try {
        ...
        if (overlayContent) {
          // 创建或更新移动点位Overlay
          this.createMovingOverlay(
            id,
            overlayContent,
            fromLonLat([position.lng, position.lat]),
          );
        }
      } catch (error) {
        this.log(
          "error",
          `创建移动点位Overlay失败: ${error.message || "未知错误"}`,
        );
      }
   ...
   * @returns 轨迹点
   */
  private calculatePositionAtProgress(
    track: Track,
    progress: number,
  ): TrackPoint {
    ...
    const nextPoint =
      index + 1 < points.length ? points[index + 1] : points[index];
  ...
  public setTrackNodesVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 节点可见性从 ${oldValue} 更改为 ${visible}, 节点锚点已自动设置为可见`,
    );
  ...
  private createTrackPointStyle(
    ...
    if (point.iconUrl) {
      ...
      try {
        // 使用安全图标创建方法
        pointStyle = this.createSafeIconStyle(
          point.iconUrl,
          1,
          iconSize,
          trackColor,
        );
        this.log("debug", `创建节点图标样式成功: ${point.iconUrl}`, {
          index: pointIndex,
        });
      } catch (error) {
        // 如果创建失败，使用默认样式
        this.log(
          "error",
          `创建节点图标样式失败: ${error.message}, URL: ${point.iconUrl}`,
        );
  ...
  public updateTrackPlayer(
    ...
    // 特殊处理相机跟随设置，必须立即生效
    if (
      player.withCamera !== undefined &&
      player.withCamera !== previousWithCamera
    ) {
      this.log(
        "info",
        `轨迹 "${id}" 相机跟随状态从 ${previousWithCamera} 更改为 ${player.withCamera}`,
      );

      if (player.withCamera) {
        // 如果开启了相机跟随，无论轨迹状态如何，都强制更新相机位置
        const track = this.tracks.get(id);
        if (track && track.points && track.points.length > 0) {
    ...
    if (playState === TrackPlayState.PLAYING) {
      // 使用适当的动画设置方法
      const useAdvancedAnimation = updatedPlayer.useAdvancedAnimation !== false;
      if (useAdvancedAnimation) {
        this.setupAdvancedTrackAnimation(id);
      } else {
        this.setupTrackAnimation(id);
  ...
  public updateTrackSpeed(id: string, speedFactor: number): boolean {
    ...
    if (playState === TrackPlayState.PLAYING) {
      ...
      this.log(
        "debug",
        `轨迹 "${id}" 速度因子已实时更新为: ${speedFactor}，并立即生效`,
      );
    } else {
      this.log(
        "debug",
        `轨迹 "${id}" 速度因子已更新为: ${speedFactor}，将在播放时生效`,
      );
    }
  ...
  public setMovingPointNameVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 移动点位名称可见性从 ${oldValue} 更改为 ${visible}`,
    );
  ...
  public setTrackNodeAnchorsVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 节点锚点可见性从 ${oldValue} 更改为 ${visible}`,
    );
  ...
  public setTrackNodeTimeVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 节点时间可见性从 ${oldValue} 更改为 ${visible}`,
    );
  ...
  private drawTrackNodes(
    ...
    const showNodeAnchors =
      showNodes && this.trackNodeAnchorsVisible.get(id) !== false;
    // 获取倍速因子
    const speedFactor = this.trackSpeedFactors.get(id) || 1.0;
    // 获取当前播放状态
    const playState = this.trackPlayStates.get(id);
      ...
      if (existingOverlays && existingOverlays.has(i)) {
        ...
        if (
          ...
          if (point && point.time) {
            ...
            // 如果样式不匹配时间，重新更新节点
            if (
              (sameDay && !hasBlueBackground) ||
              (!sameDay && !hasGrayBackground)
            ) {
              this.updateNodeOverlayToPassed(id, i);
            }
          }
    ...
    for (let i = 0; i < visiblePoints.length; i++) {
      ...
      if (showNodePopovers && point.title) {
        ...
        if (
          isPastNode &&
          (!overlayExists || (overlayExists && existingOverlays.get(i)))
        ) {
          ...
          if (overlayExists) {
            ...
            if (element) {
              ...
              // 更新箭头样式
              const arrows = element.querySelectorAll(
                'div[style*="border-top"]',
              );
              if (arrows && arrows.length > 0) {
                // 更新箭头边框
                if (arrows[0]) {
                  (arrows[0] as HTMLElement).style.borderTop =
                    "9px solid #91d5ff";
                }
                // 更新箭头
                if (arrows[1]) {
                  (arrows[1] as HTMLElement).style.borderTop =
                    "8px solid #e6f7ff";
                }
              }
            }
            ...
            const overlay = this.createNodeOverlay(
              id,
              i,
              nodeContent,
              coordinate,
              "track-node-overlay passed-node",
            );
  ...
  public setTrackNodePopoversVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 节点名称可见性从 ${oldValue} 更改为 ${visible}`,
    );
    ...
    if (this.trackPointFeatures.has(id) && this.trackNodesVisible.get(id)) {
      ...
      for (const feature of features) {
        // 更新样式以显示或隐藏节点名称
        feature.setStyle(
          this.createTrackPointStyle(id, feature.get("pointIndex"), visible),
        );
      }
    }
   ...
   * @param position 当前位置
   */
  private dispatchTrackProgressEvent(
    id: string,
    progress: number,
    position: TrackPoint,
  ): void {
    ...
    this.log(
      "debug",
      `轨迹进度更新: ${id}, 进度: ${Math.round(progress * 100)}%, 位置: [${position.lng.toFixed(6)}, ${position.lat.toFixed(6)}]`,
    );
  }
  ...
  /**   * 执行相机平滑动画   * @param id 轨迹ID   */ private animateCamera(
    id: string,
  ): void {
    const cameraAnimation = this.trackCameraAnimations.get(id);
    ...
    if (!currentCenter) {
      ...
      this.log(
        "debug",
        `相机位置初始化: 轨迹 "${id}" 设置到 [${cameraAnimation.targetCenter}]`,
      );
      // 确保视图立即更新
      this.mapInstance.render();
    ...
    if (player.stabilizeViewport && this.trackViewportStabilized.get(id)) {
      if (this.originalViewResolution !== null) {
        ...
        if (
          Math.abs(currentResolution - this.originalViewResolution) > 0.0001
        ) {
    ...
    const configuredSmoothness =
      player.cameraSmoothness !== undefined
        ? Math.max(0.03, Math.min(0.5, player.cameraSmoothness))
        : // 限制平滑度范围，提高下限和降低上限
          this.DEFAULT_CAMERA_SMOOTHNESS;
    // 计算当前时间和帧间隔
    const now = performance.now();
    const deltaTime = Math.min(now - cameraAnimation.lastFrameTime, 100);
    // 防止大间隔
    ...
    if (distance > 1000) {
      ...
      baseSmooth = configuredSmoothness;
    }
    const distanceFactor = Math.min(distance / 1000, 0.6);
    // 降低距离参考值，减少突变
    const adaptiveSmooth = baseSmooth + distanceFactor * 0.2;
    // 降低距离对平滑系数的影响
    // // 应用基于帧率的动态调整，并降低最大值，避免大幅度变化
    const smoothFactor = Math.min(
      adaptiveSmooth * (deltaTime / this.FRAME_TIME),
      0.5,
    );
    // 计算新的中心点坐标 (平滑插值)
    ...
    const minMoveThreshold = player.viewportUpdateThreshold || 1.5;
    // 提高最小移动阈值
    // // 特殊处理首次移动：如果是初始大距离移动，使用更直接的方式
    const isFirstMove = !cameraAnimation.hasMovedOnce && moveDistance > 100;
    if (isFirstMove) {
      // 对于首次大距离移动，使用更直接的方式，立即移动到目标点附近
      const directFactor = 0.8; // 移动到目标点80%的位置
      const newX = currentCenter[0] + dx * directFactor;
      const newY = currentCenter[1] + dy * directFactor;
      view.animate({
        center: [newX, newY],
        duration: 200,
        // 使用较短的动画时间
      ...
      // 标记已执行首次移动
      cameraAnimation.hasMovedOnce = true;
      this.log(
        "debug",
        `相机首次大距离移动: 轨迹 "${id}" 移动距离=${moveDistance.toFixed(2)}`,
      );
  ...
  private requestNextCameraFrame(id: string): void {
    ...
    window.requestAnimationFrame(() => {
      ...
      if (
        this.trackCameraAnimations.has(id) &&
        this.trackCameraAnimations.get(id)?.active
      ) {
        this.animateCamera(id);
      } else {
   ...
   */
  private createNodeOverlay(
    id: string,
    pointIndex: number,
    content: string,
    position: number[],
    className = "track-node-overlay",
  ): Overlay {
    const track = this.tracks.get(id);
    const point = track?.points[pointIndex];
    ...
    if (
      point &&
      point.iconUrl &&
      !content.includes(`<img src="${point.iconUrl}"`)
    ) {
      ...
      try {
        ...
        if (point.iconUrl.startsWith("data:image")) {
          ...
          point.iconUrl.startsWith("<svg") &&
          point.iconUrl.includes("</svg>")
        ) {
        ...
        this.log(
          "warn",
          `创建节点图标失败: ${e.message || "未知错误"}，URL: ${point.iconUrl}`,
        );
        // 错误处理：使用默认标记
    ...
    if (
      point &&
      point.time &&
      this.trackNodeTimeVisible.get(id) === true &&
      !content.includes("⏱")
    ) {
      // 获取格式化的时间信息
      const timeInfo = this.formatTimeDisplay(point.time);
      ...
      if (lastDivPos !== -1) {
        content =
          content.substring(0, lastDivPos) +
          timeHtml +
          content.substring(lastDivPos);
      } else {
        content += timeHtml;
      }
    }
    ...
    if (point && point.time) {
      ...
      if (
        !sameDay &&
        !className.includes("current-node") &&
        !className.includes("passed-node")
      ) {
    ...
    if (track?.iconUrl) {
      try {
        ...
        if (track.iconUrl.startsWith("data:image")) {
          ...
          track.iconUrl.startsWith("<svg") &&
          track.iconUrl.includes("</svg>")
        ) {
          // SVG 字符串格式，转换为 base64
        ...
        this.log(
          "warn",
          `创建移动点位图标失败: ${e.message || "未知错误"}，URL: ${track.iconUrl}`,
        );
      }
    }
  ...
  public selectTrack(
    ...
    if (mergedOptions.clearOthers) {
      ...
      for (const trackId of allTrackIds) {
        if (trackId !== id) {
          // 如果轨迹正在播放，先停止播放
          if (
            this.trackPlayStates.get(trackId) === TrackPlayState.PLAYING ||
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 已选中，clearOthers=${mergedOptions.clearOthers}, autoPlay=${mergedOptions.autoPlay}`,
    );
    return true;
  }
  ...
  private createSafeIconStyle(
    ...
    try {
      ...
      this.log(
        "error",
        `创建图标样式失败: ${error.message || "未知错误"}, URL: ${url}`,
      );
      // 使用默认圆点样式作为回退
  ...
  private saveOriginalViewState(id: string): void {
    ...
    this.log(
      "debug",
      `已保存轨迹 "${id}" 的原始视图状态，分辨率: ${this.originalViewResolution}`,
    );
  }
   ...
   */
  public getTracksArray(): Array<{
    id: string;
    track: Track;
    trackNameWithLength: string;
  }> {
    const tracksArray: Array<{
      id: string;
      track: Track;
      trackNameWithLength: string;
    }> = [];
    ...
    this.tracks.forEach((track, id) => {
      ...
      // 组合轨迹名称与长度
      const trackNameWithLength = track.name
        ? `${track.name} (${distanceText})`
        : `轨迹 #${id.substr(0, 6)} (${distanceText})`;
  ...
  public getDistanceBetweenPoints(
    ...
    if (
      ...
      fromIndex < 0 ||
      toIndex < 0 ||
      fromIndex >= track.points.length ||
      toIndex >= track.points.length
    ) {
      return 0;
    }
    ...
    if (fromPosition && startIndex < track.points.length - 1) {
      const posPoint = {
        ...
        time: track.points[startIndex].time,
      };
      distance += this.calculateDistance(
        posPoint,
        track.points[startIndex + 1],
      );
    }
    ...
    if (toPosition && endIndex < track.points.length) {
      const posPoint = {
        lat: toPosition.lat,
        lng: toPosition.lng,
        time: track.points[endIndex].time,
      };
  ...
  public setTrackNodeDistanceVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 节点距离信息可见性从 ${oldValue} 更改为 ${visible}`,
    );
    ...
    // 获取当前播放状态
    const playState = this.trackPlayStates.get(id) || TrackPlayState.STOPPED;

    // 清除现有节点Overlay
    this.clearNodeOverlays(id);

    // 重新创建所有节点的Overlay，无论轨迹状态如何
    if (
      this.trackNodesVisible.get(id) &&
      this.trackNodePopoversVisible.get(id)
    ) {
      const progress = this.trackProgressValues.get(id) || 0;
   ...
   * @returns 样式对象
   */
  private createMarkerStyleForAnimation(
    id: string,
    speed: number,
    position: TrackPoint,
  ): Style {
    ...
    if (enableSpeedIcon) {
      ...
      // 1. 首先检查TrackSpeedGroup配置 - 根据速度选择图标
      if (
        this.config.trackSpeedGroup &&
        this.config.trackSpeedGroup.length > 0
      ) {
        const sortedGroups = [...this.config.trackSpeedGroup].sort(
          (a, b) => b.speed - a.speed,
        );
        for (const group of sortedGroups) {
          if (realSpeed > group.speed) {
            ...
            this.log(
              "debug",
              `[性能模式] 根据速度 ${realSpeed} km/h 选择图标: ${iconUrl} (速度阈值: ${group.speed})`,
            );
            break;
          }
        }
      ...
      if (!iconUrl && track.iconGroup && track.iconGroup.length > 0) {
        for (const group of track.iconGroup) {
          if (realSpeed >= group.speed) {
            ...
            this.log(
              "debug",
              `[性能模式] 使用轨迹iconGroup图标: ${iconUrl} (速度范围: ${group.speed})`,
            );
            break;
          }
        }
    ...
    if (iconUrl) {
      try {
        ...
        this.log(
          "error",
          `[性能模式] 创建移动点图标样式失败: ${error.message || "未知错误"}, URL: ${iconUrl}`,
        );
        style = this.createDefaultMarkerStyle(
          track.color || "rgba(24, 144, 255, 1)",
        );
  ...
  private setupAdvancedTrackAnimation(id: string): void {
    ...
    // 创建初始样式
    const style = this.createMarkerStyleForAnimation(
      id,
      position.speed || 0,
      position,
    );
    ...
    this.trackAnimationListeners.set(
      ...
      this.trackLayer!.on("postrender", (event) => {
        ...
        // 计算当前位置
        const newPosition = this.calculatePositionAtProgress(
          track,
          newDistance,
        );
        ...
        const newPoint = new Point(
          fromLonLat([newPosition.lng, newPosition.lat]),
        );
        animState.position = newPoint;

        // 更新样式（如果需要根据速度变化）
        if (this.trackEnableSpeedIcon.get(id) !== false) {
          animState.style = this.createMarkerStyleForAnimation(
            id,
            newPosition.speed || 0,
            newPosition,
          );
        ...
        if (
          showMovingInfo &&
          ((movingPointNameVisible && newPosition.title) ||
            showSpeed ||
            showNodeDistance)
        ) {
          try {
            // 使用统一的方法创建移动点内容
            const overlayContent = this.createMovingPointContent(
              id,
              newPosition,
            );
    ...
    if (this.trackPlayStates.get(id) === TrackPlayState.PLAYING) {
      ...
      if (useAdvancedAnimation) {
        ...
        if (this.trackAnimationPositions.has(id)) {
          const animState = this.trackAnimationPositions.get(id)!;
          const progress = this.trackProgressValues.get(id) || 0;
          const position = this.calculatePositionAtProgress(
            this.tracks.get(id)!,
            progress,
          );
          ...
          const movingPointNameVisible =
            this.trackMovingPointNameVisible.get(id) === true;
          const showSpeed = this.trackSpeedPopoversVisible.get(id) === true;
          const showNodeDistance =
            this.trackNodeDistanceVisible.get(id) === true;
        ...
        try {
          ...
          if (geometry) {
            ...
            if (enabled) {
              ...
              // 使用默认样式
              style = this.createDefaultMarkerStyle(
                this.tracks.get(id)!.color || "rgba(24, 144, 255, 1)",
              );
            }
            ...
            const showNodeDistance =
              this.trackNodeDistanceVisible.get(id) === true;
            const showMovingInfo =
              this.trackMovingInfoVisible.get(id) !== false;
    ...
    try {
      ...
      if (this.trackNodesVisible.get(id)) {
        ...
        this.drawTrackNodes(
          ...
          null,
          track,
          this.trackProgressValues.get(id) || 0,
        );
      }
      ...
      if (this.trackPlayStates.get(id) === TrackPlayState.PLAYING) {
        ...
        if (useAdvancedAnimation) {
          ...
          // 更新移动点的覆盖物
          if (
            this.trackMovingPointNameVisible.get(id) ||
            this.trackSpeedPopoversVisible.get(id)
          ) {
  ...
  private createMovingPointContent(id: string, position: TrackPoint): string {
    ...
    if (showMovingDistance) {
      // 计算当前位置对应的点索引
      const exactIndex =
        this.trackProgressValues.get(id)! * (track.points.length - 1);
      const pointIndex = Math.floor(exactIndex);
      if (pointIndex >= 0) {
        const distanceFromStart =
          this.getDistanceFromStart(id, pointIndex, position) / 1000; // 公里
  ...
  public setMovingInfoVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 移动点信息可见性从 ${oldValue} 更改为 ${visible}`,
    );
  ...
  public setMovingDistanceVisible(id: string, visible: boolean): boolean {
    ...
    this.log(
      "debug",
      `轨迹 "${id}" 移动点距离信息可见性从 ${oldValue} 更改为 ${visible}`,
    );

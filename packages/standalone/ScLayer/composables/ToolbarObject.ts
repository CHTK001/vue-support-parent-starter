...
export class ToolbarObject {
    ...
    this.shapeObj.setDrawEndCallback((id, shapeType, feature) => {
      ...
      switch (shapeType) {
        case "Point":
          ...
          const point = pointGeom.getCoordinates
        ...
        case "Circle":
          ...
          if (geometry instanceof Circle) {
            ...
            // 处理其他未预期的几何图形类型情况
            logger.warn(
              `绘制圆形时遇到未预期的几何图形类型: ${geometry.getType ? geometry.getType() : "unknown"}`,
            );
      ...
      if (shapeOption && this.toolStateChangeCallback) {
        logger.debug(
          `图形绘制完成，自动调用addShape，类型: ${shapeType}, ID: ${id}`,
        );
   ...
   * @param data 附加数据
   */
  private triggerToolStateChange(
    toolId: string,
    active: boolean,
    toolType: string,
    data?: any,
  ...
  private activateDeleteMode(): void {
    ...
    const deleteClickListener = (evt: any) => {
      ...
      if (!hasDeleted && this.markerObj) {
        ...
        if (
          this.shapeObj &&
          (this.shapeObj.isEditMode() || (editTool && editTool.active))
        ) {
  ...
  private handleShapeEditDeactivate(): void {
    ...
    // 检查是否在编辑模式，记录编辑的图形ID
    const editInfo = this.shapeObj.isEditMode()
      ? this.shapeObj.getEditModeInfo()
      : null;

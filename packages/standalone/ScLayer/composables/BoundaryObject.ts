...
import {
  getCoordSystemByMapType,
  convertCoordSystemToProjection,
  wgs84ToGcj02,
  gcj02ToWgs84,
  GcoordUtils,
} from "../utils/GcoordUtils";
import { MapObject } from "./MapObject";
import { GeoPoint } from "../types/coordinate";
import { indexedDBProxy } from "@repo/utils";
import {
  BoundaryDataProviderFactory,
  BoundaryDataFormat,
  CoordinatePoint,
} from "../interfaces/BoundaryDataProvider";
...
export class BoundaryObject {
  ...
  // 钻取回调函数
  private drillCallback:
    | ((data: BoundaryData, isBack: boolean) => void)
    | null = null;
  ...
  public addBoundary(boundaryData: BoundaryData): void {
    ...
    if (this.selectedBoundaries.has(boundaryData.code)) {
      logger.debug(
        `边界已存在，不再重复添加: ${boundaryData.name}(${boundaryData.code})`,
      );
      return;
    }
    ...
    boundaryData.coordinates.forEach((ring, index) => {
      ...
      // 设置样式
      feature.setStyle(
        this.createBoundaryStyle(boundaryData.name, index === 0),
      );
  ...
  private createBoundaryStyle(name: string, showLabel: boolean): Style {
    ...
    const baseStyle = {
      stroke: new Stroke({
        ...
        lineDash:
          this.options.strokeStyle === "dashed"
            ? [5, 5]
            : this.options.strokeStyle === "dotted"
              ? [1, 3]
      ...
      fill: new Fill({
        color: this.options.fillBoundary ? getColorByName(name) : "transparent",
      }),
    };
    ...
    const textStyle =
      showLabel && this.options.showLabel
        ? new Text({
            ...
            backgroundFill: this.options.labelOptions?.backgroundColor
              ? new Fill({
  ...
  public updateAllBoundariesStyle(): void {
    ...
    features.forEach((feature) => {
      ...
      if (properties) {
        feature.setStyle(
          this.createBoundaryStyle(properties.name, properties.ringIndex === 0),
        );
      }
    });
   ...
   * @param adcode 行政区划代码
   */
  public async addBoundaryByAdcode(
    adcode: string,
   ...
   */
  private async fetchBoundaryData(
    adcode: string,
    options: BoundaryOptions,
  ): Promise<BoundaryDataFormat | null> {
    try {
      // 获取当前地图服务商
      ...
      if (!boundaryResponseData) {
        ...
        try {
          // 从工厂获取对应的区划数据提供者
          const dataProvider = BoundaryDataProviderFactory.getProvider(
            provider.toLowerCase(),
          );

          // 优先使用 apiUrls.boundary，然后是旧的 boundaryUrl 属性
          const boundaryUrl =
            options.apiUrls?.boundary ||
            options.boundaryUrl ||
            this.options.apiUrls?.boundary ||
      ...
      if (boundaryResponseData) {
        try {
          ...
          // 转换数据为高德格式
          const convertedData = dataProvider.convertToGaodeFormat(
   ...
   */
  private parseGaodePolylineToFeatures(
    polyline: string,
    ...
    logger.debug(
      `解析区划数据: 源坐标系统=${sourceCoordType}, 目标坐标系统=EPSG:3857`,
    );
        ...
        for (let i = 0; i < pointPairs.length; i += 2) {
          ...
          if (!isNaN(lng) && !isNaN(lat)) {
            ...
            try {
              ...
              mercatorCoord = fromLonLat(
                [coordObj.lng, coordObj.lat],
                "EPSG:3857",
              );
            } catch (error) {
              // 转换失败时的回退方案
  ...
  public async loadDistrictTree(
    ...
    try {
      ...
      const url =
        districtUrl ||
        this.options.apiUrls?.district ||
        this.options.districtUrl;

      if (url) {
        // 如果配置了URL，从URL获取数据
        // 优先使用传入的 mapKey，然后是 options 中的 mapKey
        const key =
          (mapKey || this.options.mapKey)?.[this.options.provider || "GAODE"] ||
          "";
        ...
        const gaodeDistrictData = (await import("../data/districts.json"))
          .default;
        districts = [
          {
            adcode: "100000",
   ...
   */
  public setDrillCallback(
    callback: (data: BoundaryData, isBack: boolean) => void,
  ): void {
        ...
        this.clickListener = this.map.on("click", (event) => {
          ...
          setTimeout(() => {
            ...
            if (new Date().getTime() - lastClickTime >= doubleClickDelay) {
              ...
              const layer = this.map.forEachFeatureAtPixel(
                event.pixel,
                (feature, layer) => layer,
              );
              ...
              const properties =
                feature.get("properties") || feature.getProperties();
              if (properties && properties.code) {
                // 如果是省级行政区，钻取到该区域
                logger.debug(
                  `点击了边界: ${properties.name}(${properties.code})`,
                );
  ...
  private createBoundary(
    ...
    try {
      ...
      if (typeof data.polyline === "string") {
        // 旧格式：字符串格式的 polyline
        features = this.parseGaodePolylineToFeatures(
          data.polyline,
   ...
   * @returns OpenLayers Feature 数组
   */
  private parseCoordinatePointsToFeatures(
    polylineArray: CoordinatePoint[][],
  ): Feature[] {

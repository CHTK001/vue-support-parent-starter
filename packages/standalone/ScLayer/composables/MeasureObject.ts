...
export class MeasureObject {
    ...
    this.layer = new VectorLayer({
      ...
      style: (feature) => {
        return this.createFeatureStyle(
          feature as Feature<LineString | Polygon | Point>,
        );
      },
    });
    ...
    if (this.measureTooltipElement) {
      ...
      deleteButton.onclick = (e) => {
        ...
        if (this.source && this.measureTooltips.length > 0) {
          const tooltipToRemove =
            this.measureTooltips[this.measureTooltips.length - 1];
  ...
  private createHelpTooltip(): void {
    ...
    this.mapInstance
      .getViewport()
      .addEventListener("pointermove", this.handlePointerMoveListener);
  }
  ...
  private removeHelpTooltip(): void {
    ...
    if (this.handlePointerMoveListener) {
      this.mapInstance
        .getViewport()
        .removeEventListener("pointermove", this.handlePointerMoveListener);
  ...
  private handlePointerMove(event: any): void {
    if (!this.mapInstance || !this.helpTooltipElement || !this.helpTooltip)
      return;
    ...
    window.requestAnimationFrame(() => {
      ...
      if (this.helpTooltip) {
        this.helpTooltip.setPosition(
          this.mapInstance!.getEventCoordinate(event),
        );
      }

      // 更新鼠标样式
      this.mapInstance!.getTargetElement().style.cursor = hit
        ? "pointer"
        : "crosshair";
    });
  }
        ...
        styles.push(
          new Style({
            ...
            image: new CircleStyle({
              ...
              stroke: new Stroke({
                color:
                  this.style.point?.stroke?.color || "rgba(24, 144, 255, 0.8)",
                width: this.style.point?.stroke?.width || 2,
              }),
              fill: new Fill({
                color:
                  this.style.point?.fill?.color || "rgba(255, 255, 255, 0.8)",
              }),
            }),
          }),
        );
 ...
 * @returns 测距对象
 */
export function createMeasureObject(
  mapInstance?: Map,
  style?: MeasureStyle,
): MeasureObject {

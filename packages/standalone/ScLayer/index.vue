/** * 地图组件 * @author CH * @date 2025-04-29 */
<template>
  <div class="sc-layer" :style="{ height: config.height + 'px' }">
    ...
    <BoundarySelector
      ...
      :boundary-url="props.boundaryUrl"
      :district-url="props.districtUrl"
    ...
    <SearchBox
      ...
      class="search-box"
      ref="searchBoxRef"
      :placeholder="props.searchBoxConfig.placeholder"
      :debounce-time="props.searchBoxConfig.debounceTime"
      :position="props.searchBoxConfig.position"
      :search-box-config="props.searchBoxConfig"
      @search="handleSearch"
    ...
    >
      ...
      <template v-if="$slots.markerDetailFooter" #footer="slotProps">
        <slot
          name="markerDetailFooter"
          :marker="slotProps.marker"
...
const emit = defineEmits<{
  ...
  (
    e: "toolbar-tool-activated",
    payload: { toolId: string; toolbarObj: ToolbarObject },
  ): void;
  (
    e: "toolbar-tool-deactivated",
    payload: { toolId: string; toolbarObj: ToolbarObject },
  ): void;
  (
    e: "toolbar-state-change",
    payload: { toolId: string; active: boolean; toolType: string; data?: any },
  ): void;
  (
    e: "map-initialized",
    payload: { map: MapObject; toolbar: ToolbarObject },
  ): void;
  (
    e: "marker-click",
    payload: { coordinates: number[]; data: MarkerOptions },
  ): void;
  (e: "marker-create", payload: { id: string; options: MarkerOptions }): void;
  (
    e: "marker-update",
    payload: { id: string; options: Partial<MarkerOptions> },
  ): void;
  (e: "marker-delete", payload: { id: string }): void;
  (e: "shape-create", payload: { id: string; options: ShapeOption }): void;
  (
    e: "shape-update",
    payload: { id: string; options: Partial<ShapeOption> },
  ): void;
  ...
  (
    e: "flight-line-selection-change",
    payload: { selectedIds: string[]; count: number },
  ): void;
  (e: "search", payload: SearchResult[]): void;
  (e: "search-select", payload: SearchResult): void;
}>();
...
let searchObject: SearchObject | null = null;
...
const coordinateOptions = computed<CoordinateOptions>(() => ({
  ...
  position:
    (props.coordinateOptions?.position as CoordinatePosition) || "bottom-right",
  showProjected:
    props.coordinateOptions?.showProjected !== undefined
      ? props.coordinateOptions.showProjected
      : true,
}));
...
const setupMouseMoveListener = () => {
  ...
  mapObj.setPointerMoveListener((event, coordinate) => {
    ...
    coordinateInfo.value = {
      ...
      position:
        coordinate.position ||
        (coordinateOptions.value.position as CoordinatePosition),
    };
  });
};
...
const handleToolStateByType = (
  ...
  const handlers: Record<string, () => void> = {
    ...
    coordinate: () => {
      ...
      logger.debug(
        `坐标工具状态变化: ${active ? "激活" : "停用"}, 面板显示: ${showCoordinatePanel.value}`,
      );
    },
    ...
    "current-location": () => {
      getCurrentPoint()
        .then((coordinate: GeoPoint) => {
          // 添加标记点 - 处理 GeoPoint 可能是数组或对象的情况
          const position = Array.isArray(coordinate)
            ? coordinate
    ...
    flightLine: () => {
      ...
      if (active) {
        ...
        if (isHeatmapActive.value) {
          ...
          if (heatmapObj && heatmapObj.isEnabled()) {
            try {
              heatmapObj.disable();
              logger.debug(
                "[FlightLine] 已临时禁用热力图，飞线图激活后热力图将被恢复",
              );
            } catch (error) {
        ...
        logger.debug(
          "[FlightLine] 飞线图已从工具栏禁用，解除面板锁定并隐藏面板",
        );
    ...
    "flight-line": () => {
      logger.debug(
        `[FlightLine] 收到旧版flight-line事件，转发到flightLine处理器`,
      );
      handlers["flightLine"]();
    },
    ...
    "shape-created": () => {
      if (toolType === "shape" && active && data) {
        ...
        if (id && options) {
          logger.debug(
            `[Shape] 接收到图形创建事件，ID: ${id}, 类型: ${options.type}`,
          );
    ...
    edit: () => {
      if (
        ...
        logger.debug(
          `[Shape] 接收到图形更新事件，ID: ${data.shapeId}, 类型: ${data.shapeType}`,
        );
    ...
    search: () => {
      ...
      if (active) {
        getCurrentPoint()
          .then((coordinate: GeoPoint) => {
            // 不再需要地图密钥
            getLocationCityCode(coordinate)
              .then((locationInfo) => {
                currentLocationInfo.value = locationInfo;
                logger.debug(
                  `[Search] 获取到当前位置的城市编码: ${locationInfo.cityCode}, 区划编码: ${locationInfo.adcode}`,
                );
...
const coordinateOptionsVersion = computed(() =>
  JSON.stringify(props.coordinateOptions),
);
...
const handleToolDeactivated = (toolId) => {
  ...
  if (toolId === "coordinate") {
    ...
    if (flightLineObj && flightLineObj.isEnabled()) {
      ...
      if (isHeatmapActive.value) {
        logger.debug(
          "[FlightLine] 飞线图已禁用，检测到热力图应该处于激活状态，重新启用热力图",
        );
...
const setupCoordinatePanelWatcher = () => {
  ...
  watch(
    () => toolbarObject.isCoordinatePanelVisible(),
    (visible) => {
...
const checkTrackPlayerState = () => {
  logger.debug(
    `[Track] 检查轨迹播放器状态: mapReady=${mapReady.value}, mapObj=${!!mapObj}, trackObj=${!!trackObj}`,
  );

  // 检查轨迹播放器工具是否激活
  const isTrackPlayerActive =
    toolbarObject && toolbarObject.getActiveToolId() === "track-player";
  ...
  if (mapReady.value && mapObj) {
    if (!trackObj) {
      ...
      if (isTrackPlayerActive) {
        logger.debug(
          "[Track] 地图已就绪，但轨迹对象未初始化，开始初始化轨迹对象",
        );
        initTrackObject();
      }
    } else {
...
const initTrackObject = () => {
  ...
  try {
    ...
    logger.info(
      `[Track] 轨迹对象初始化成功，显示状态: ${showTrackPlayer.value}`,
    );
  } catch (error) {
...
// 确定图层面板位置
const determineLayerPanelPosition = ():
  | "top-left"
  | "top-right"
  | "bottom-left"
  | "bottom-right" => {
  ...
  if (toolbarObject) {
    ...
    const toolbarPosition =
      toolbarConfig.position || DEFAULT_TOOLBAR_CONFIG.position;
    const toolbarDirection =
      toolbarConfig.direction || DEFAULT_TOOLBAR_CONFIG.direction;
    ...
    if (toolbarDirection === "horizontal") {
      ...
      if (toolbarPosition.startsWith("top-")) {
        ...
        return toolbarPosition === "bottom-left"
          ? "bottom-left"
          : "bottom-right";
      }
    } else {
...
// 确定鹰眼地图位置
const determineOverviewMapPosition = ():
  | "top-left"
  | "top-right"
  | "bottom-left"
  | "bottom-right" => {
...
const handleOverviewMapCollapseChange = (collapsed: boolean) => {
  logger.debug(
    `[Overview] 鹰眼地图折叠状态变化: ${collapsed ? "已折叠" : "已展开"}`,
  );
...
// 监听地图类型和图层变化
watch(
  [() => props.mapType, () => props.mapTile],
  ([newMapType, newMapTile]) => {
    if (!mapObj) return;
...
const addDemoTrack = () => {
  ...
  for (let i = 0; i < pointCount; i++) {
    ...
    const lng =
      startLng +
      (Math.random() - 0.5) * 0.001 * i +
      Math.sin((direction * Math.PI) / 180) * distanceFactor * i;
    const lat =
      startLat +
      (Math.random() - 0.5) * 0.001 * i +
      Math.cos((direction * Math.PI) / 180) * distanceFactor * i;
...
const flightLinePanelRef = ref<InstanceType<typeof FlightLinePanel> | null>(
  null,
);
 ...
 * 处理飞线图选择变更
 */
const handleFlightLineSelectionChange = (
...
const checkFlightLineState = () => {
  logger.debug(
    `[FlightLine] 检查飞线图状态: mapReady=${mapReady.value}, mapObj=${!!mapObj}, flightLineObj=${!!toolbarObject?.getFlightLineObject()}`,
  );

  // 检查飞线图工具是否激活
  const isFlightLineActive =
    toolbarObject && toolbarObject.getActiveToolId() === "flightLine";

  // 获取飞线对象
  const flightLineObj = toolbarObject?.getFlightLineObject();

  logger.debug(
    `[FlightLine] 飞线图工具激活状态: ${isFlightLineActive}, 飞线对象存在: ${!!flightLineObj}, 面板显示状态: ${showFlightLinePanel.value}, 面板引用: ${!!flightLinePanelRef.value}`,
  );
  ...
  if (mapReady.value && mapObj) {
    ...
    if (isFlightLineActive && !showFlightLinePanel.value) {
      logger.info(
        "[FlightLine] 发现飞线图工具已激活但面板未显示，显示飞线列表面板",
      );
      showFlightLinePanel.value = true;
      // 设置面板锁定状态，防止意外关闭
      flightLinePanelLocked.value = true;
  ...
  addMarker: (options: MarkerOptions) => {
    ...
    if (!options.icon && options.group && props.markerConfig?.groupIcon) {
      ...
      if (groupIcon) {
        ...
        logger.debug(
          `[Marker] 从分组 "${options.group}" 获取图标: ${groupIcon}`,
        );
      }
    }
    ...
    try {
      ...
      if (trackObj) {
        ...
        if (props.trackPlayerConfig?.showNodes !== undefined) {
          trackObj.setTrackNodesVisible(
            trackId,
            props.trackPlayerConfig.showNodes,
          );
        }

        // 设置节点锚点显示（当节点显示开启时生效）
        if (props.trackPlayerConfig?.showNodeAnchors !== undefined) {
          trackObj.setTrackNodeAnchorsVisible(
            trackId,
            props.trackPlayerConfig.showNodeAnchors,
          );
        }

        // 设置名称显示（点位名称）
        if (props.trackPlayerConfig?.showPointNames !== undefined) {
          trackObj.setMovingPointNameVisible(
            trackId,
            props.trackPlayerConfig.showPointNames,
          );
        }

        // 设置节点时间显示
        if (props.trackPlayerConfig?.showNodeTime !== undefined) {
          trackObj.setTrackNodeTimeVisible(
            trackId,
            props.trackPlayerConfig.showNodeTime,
          );
        }

        // 设置速度显示（移动速度）
        if (props.trackPlayerConfig?.showSpeed !== undefined) {
          trackObj.setTrackSpeedPopoversVisible(
            trackId,
            props.trackPlayerConfig.showSpeed,
          );
        }

        // 设置节点速度显示（节点速度）
        if (props.trackPlayerConfig?.showNodeSpeed !== undefined) {
          trackObj.setTrackNodeSpeedsVisible(
            trackId,
            props.trackPlayerConfig.showNodeSpeed,
          );
        }
      }
...
const aggregationOptionsVersion = computed(() =>
  JSON.stringify(props.aggregationOptions),
);
...
const checkBoundaryState = () => {
  ...
  logger.debug(
    `[Boundary] 检查区划工具状态: 激活=${isBoundaryActive}, 面板显示=${showBoundarySelector.value}`,
  );

  // 确保UI状态与工具状态一致
  if (isBoundaryActive && !showBoundarySelector.value) {
    logger.debug(
      "[Boundary] 发现区划工具已激活但面板未显示，显示区划选择器面板",
    );
    showBoundarySelector.value = true;
...
const handleMarkerClick = (payload: {
  ...
  if (mapObj && payload.coordinates && payload.coordinates.length >= 2) {
    // 使用地图对象将地理坐标转换为屏幕坐标
    const screenPosition = mapObj.getScreenPositionFromCoordinates(
      payload.coordinates,
    );
    if (screenPosition) {

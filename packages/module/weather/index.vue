...
<template>
  <div class="weather-card" @click="dialogVisible = true">
    <ScSkeleton :loading="loading" animated class="h-full">
      <template #template>
        <div class="skeleton-content">
          ...
          <div class="skeleton-text">
            <ScSkeletonItem variant="text" style="width: 50%" />
            <ScSkeletonItem variant="text" style="width: 80%" />
          </div>
        </div>
      </template>
      <template #default>
        <div v-if="!weatherStore.weather?.data?.cityName" class="empty-state">
          <ScEmpty description="暂无天气数据" :image-size="60" />
        </div>
        <div v-else class="weather-content">
          <div class="weather-main">
            ...
            <div class="weather-info">
              <div class="temperature">
                {{ weatherStore.weather?.data?.temperature
                }}<span class="unit">℃</span>
              </div>
              <div class="location-weather">
                <span class="city">{{
                  weatherStore.weather?.data?.cityName
                }}</span>
          ...
          <div class="weather-details">
            <div class="detail-item">
              ...
              <span class="value">{{
                weatherStore.weather?.data?.shidu || "--"
              }}</span>
            </div>
            <div class="detail-item">
              <span class="label">空气</span>
              <span class="value">{{
                weatherStore.weather?.data?.quality || "--"
              }}</span>
            </div>
            <div class="detail-item">
              <span class="label">风向</span>
              <span class="value">{{
                weatherStore.current?.windDirection || "--"
              }}</span>
            </div>
          </div>
        </div>

...
const gameLoop = (currentTime: number) => {
  ...
  for (const cloud of FAR_CLOUDS) {
    const cloudX =
      ((cloud.x - cameraX.value * 0.2) % (SCREEN_WIDTH + 200)) - 50;
  ...
  for (const cloud of MID_CLOUDS) {
    const cloudX =
      ((cloud.x - cameraX.value * 0.4) % (SCREEN_WIDTH + 200)) - 50;
    if (cloudX > -100 && cloudX < SCREEN_WIDTH + 100) {
      drawCloud(ctx, cloudX, cloud.y, cloud.size);
    }
  }
  ...
  for (const block of level.value) {
    ...
    // 只绘制可见的方块
    if (
      blockX + blockWidth > cameraX.value - 50 &&
      blockX < cameraX.value + SCREEN_WIDTH + 50
    ) {
  ...
  if (
    marioScreenX > cameraX.value - 50 &&
    marioScreenX < cameraX.value + SCREEN_WIDTH + 50
  ) {
    drawMario(
      ctx,
      marioScreenX,
      mario.value.y,
      mario.value.facingRight,
      mario.value.animFrame,
      mario.value.onGround,
    );
  }
...
<template>
  <div class="mario-game-container">
    <canvas ref="canvasRef" class="mario-canvas" />
    <div class="game-info">

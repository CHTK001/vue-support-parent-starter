 ...
 */
function convertElementChinaAreaData(data: ElementChinaAreaItem[], parentCode: string = "0", parentPath: string = "0", parentPathName: string = "", level: number = 1): RegionData[] {
  return data.map(item => {
    ...
    if (item.children && item.children.length > 0) {
      regionItem.children = convertElementChinaAreaData(item.children, normalizedCode, path, pathName, level + 1);
    }
 ...
 * 根据数据源类型获取地区数据
 */
export async function getRegionDataBySource(dataSource: DataSourceType, customData?: RegionData[]): Promise<RegionData[]> {
  switch (dataSource) {
 ...
 * 同步获取数据（用于初始化）
 */
export function getRegionDataBySourceSync(dataSource: DataSourceType, customData?: RegionData[]): RegionData[] {

          ...
          <el-form-item :prop="item.prop" :label="item.label" v-if="showQuery(index)">
            ...
            <div v-else-if="item.type == 'datePicker' ">
              <el-date-picker
               ...
               :editable="item.editable" :class="handleRenderWidth(item.width)" @change="(value) => {handleChangeValue(item, value)}" type="daterange" clearable :placeholder="item.placeholder" v-model="form[item.prop]" :format="item.format"></el-date-picker>
            </div>
            ...
            <div v-else-if="item.type == 'dateMonthPicker' ">
              <el-date-picker 
                :start-placeholder="handleRenderStartPlaceholder(item.timeOption)"
                :end-placeholder="handleRenderEndPlaceholder(item.timeOption)"
               :editable="item.editable" :class="handleRenderWidth(item.width)" @change="(value) => {handleChangeValue(item, value)}" type="daterange" clearable :placeholder="item.placeholder" v-model="form[item.prop]" format="YYYY-MM"></el-date-picker>
            </div>
    
            <div v-else-if="item.type == 'dateDayPicker' ">
              <el-date-picker :start-placeholder="handleRenderStartPlaceholder(item.timeOption)"
                :end-placeholder="item.endPlaceholder || '请选择结束时间'"  :editable="item.editable" :class="handleRenderWidth(item.width)" @change="(value) => {handleChangeValue(item, value)}" type="daterange" clearable :placeholder="item.placeholder" v-model="form[item.prop]" format="YYYY-MM-DD"></el-date-picker>
            </div>
    
            <div v-else-if="item.type == 'dateTimePicker' ">
              <el-datetime-picker :start-placeholder="handleRenderStartPlaceholder(item.timeOption)"
                :end-placeholder="item.endPlaceholder || '请选择结束时间'" :editable="item.editable" :class="handleRenderWidth(item.width)"  @change="(value) => {handleChangeValue(item, value)}" type="datetimerange" clearable :placeholder="item.placeholder" v-model="form[item.prop]" :format="item.format"></el-datetime-picker>
            </div>
    
            <div v-else-if="item.type == 'email' ">
               <el-autocomplete v-model="form[item.prop]" :fetch-suggestions="queryEmail" :trigger-on-focus="false" :placeholder="item.placeholder || '请输入邮箱'" clearable :class="handleRenderWidth(item.width)" />
            </div>
            ...
            <div v-else-if="item.type == 'amount' ">
              ...
              inputType="amount" isTip v-model="form[item.prop]" :placeholder="item.placeholder" :class="handleRenderWidth(item.width)" />
            </div>
    
            <div v-else-if="showQuery(index)">
              <t-input :maxlength="item.maxlength || 20" clearable v-model="form[item.prop]" :placeholder="item.placeholder" :class="handleRenderWidth(item.width)" :show-word-limit="true">
              </t-input>
            </div>
          </el-form-item>
        </template>

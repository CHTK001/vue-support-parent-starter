    ...
    try {
      ...
      if (config) {
        ...
        const completeConfig = {
          ...
          loop: typeof config.loop !== 'undefined' ? config.loop : trackPlayer.options.loop,
          // 保持原有的相机跟随设置，除非明确指定新值
          withCamera: typeof config.withCamera !== 'undefined' ? config.withCamera : trackPlayer.options.panTo,
          // 保持原有的速度因子，除非明确指定新值

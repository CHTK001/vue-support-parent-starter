...
export class ShapeObject {
  ...
  private clickListeners: Array<
    (id: string, event: L.LeafletMouseEvent) => void
  > = [];
  ...
  private patchLeafletEditable(): void {
    try {
      ...
      if (typeof L.Editable.prototype.onDrawingMouseDown === "function") {
        const originalOnDrawingMouseDown =
          L.Editable.prototype.onDrawingMouseDown;
        ...
        L.Editable.prototype.onDrawingMouseDown = function (e: any) {
          try {
            ...
            // 确保其他可能缺失的属性
            ["_leaflet_id", "_leaflet_events", "_map", "_latlng"].forEach(
              (prop) => {
                if (!this._drawingEditor.feature[prop]) {
                  this._drawingEditor.feature[prop] =
                    prop === "_latlng" ? L.latLng(0, 0) : {};
                }
              },
            );
      ...
      if (L.Editable.PolylineEditor && L.Editable.PolylineEditor.prototype) {
        ["commitDrawing", "cancelDrawing"].forEach((methodName) => {
          if (
            typeof L.Editable.PolylineEditor.prototype[methodName] ===
            "function"
          ) {
            const originalMethod =
              L.Editable.PolylineEditor.prototype[methodName];
    ...
    try {
      ...
      this.mapInstance.on("dblclick", (e: any) => {
        try {
          // 如果正在绘制多边形或折线，则完成绘制
          if (
            this.drawingMode === ShapeType.POLYGON ||
            this.drawingMode === ShapeType.POLYLINE
          ) {
            if (
              this._currentEditor &&
      ...
      try {
        switch (type) {
          case ShapeType.RECTANGLE:
            // 矩形绘制使用地图中心点作为起始点
            this._currentEditor = this.mapInstance.editTools.startRectangle(
              mapCenter,
              drawOptions,
            );
            logger.debug(
              `开始绘制矩形，起始点: ${mapCenter.lat}, ${mapCenter.lng}`,
            );
            break;

          case ShapeType.CIRCLE:
            // 圆形绘制使用地图中心点作为起始点
            this._currentEditor = this.mapInstance.editTools.startCircle(
              mapCenter,
              drawOptions,
            );
            logger.debug(
              `开始绘制圆形，中心点: ${mapCenter.lat}, ${mapCenter.lng}`,
            );
            break;

          case ShapeType.POLYGON:
            // 多边形绘制不使用预设的起始点，等待用户点击
            this._currentEditor = this.mapInstance.editTools.startPolygon(
              null,
              drawOptions,
            );
          ...
          case ShapeType.POLYLINE:
            ...
            this._currentEditor = this.mapInstance.editTools.startPolyline(
              null,
              drawOptions,
            );
    ...
    try {
      ...
      switch (shapeType) {
        case ShapeType.RECTANGLE:
          ...
          try {
            const bounds = editor.getBounds
              ? editor.getBounds()
              : editor.feature && editor.feature.getBounds
                ? editor.feature.getBounds()
                : null;
            ...
            layer = L.rectangle(bounds, {
              ...
              fillColor:
                shapeOptions.style?.fillColor || this.defaultStyle.fillColor,
              fillOpacity:
                shapeOptions.style?.fillOpacity ||
        ...
        case ShapeType.CIRCLE:
          ...
          try {
            const center = editor.getLatLng
              ? editor.getLatLng()
              : editor.feature && editor.feature.getLatLng
                ? editor.feature.getLatLng()
                : null;

            const radius = editor.getRadius
              ? editor.getRadius()
              : editor.feature && editor.feature.getRadius
                ? editor.feature.getRadius()
                : 0;
        ...
        case ShapeType.POLYLINE:
          ...
          try {
            const latLngs = editor.getLatLngs
              ? editor.getLatLngs()
              : editor.feature && editor.feature.getLatLngs
                ? editor.feature.getLatLngs()
                : null;
            ...
            if (shapeType === ShapeType.POLYGON) {
              layer = L.polygon(latLngs, {
                ...
                weight: shapeOptions.style?.weight || this.defaultStyle.weight,
                opacity:
                  shapeOptions.style?.opacity || this.defaultStyle.opacity,
                fillColor:
                  shapeOptions.style?.fillColor || this.defaultStyle.fillColor,
                fillOpacity:
                  shapeOptions.style?.fillOpacity ||
            ...
            logger.error(
              `创建${shapeType === ShapeType.POLYGON ? "多边形" : "折线"}图层失败:`,
              e,
            );
          }
          break;
  ...
  private initializeEditor(editor: any): void {
    try {
      ...
      methods.forEach((methodName) => {
        if (
          typeof editor[methodName] === "function" &&
          !(editor[methodName] as any).__wrapped
        ) {
          ...
          editor[methodName] = function () {
            try {
              ...
              // 尝试安全地停止绘制
              if (
                methodName === "commitDrawing" ||
                methodName === "cancelDrawing"
              ) {
                try {
                  if (this.map && this.map.editTools) {
    ...
    try {
      ...
      setTimeout(() => {
        if (
          this.mapInstance &&
          this.mapInstance.editTools &&
          this.mapInstance.editTools._currentDrawer
        ) {
          try {
  ...
  private removeTemporaryHandlers(): void {
    if (this.mapInstance) {
      try {
        ...
        if (
          this.mapInstance.editTools &&
          this.mapInstance.editTools.featuresLayer
        ) {
          try {
            this.mapInstance.editTools.featuresLayer.clearLayers();
          } catch (e) {
  ...
  private createCircle(id: string, options: ShapeOption): ShapeItem {
    ...
    const center = L.latLng(
      options.coordinates[0][0],
      options.coordinates[0][1],
    );
  ...
  private createPolygon(id: string, options: ShapeOption): ShapeItem {
    ...
    if (
      !options.coordinates ||
      !Array.isArray(options.coordinates) ||
      options.coordinates.length < 3
    ) {
      throw new Error("多边形坐标无效");
    }

    // 创建坐标点数组
    const latLngs = options.coordinates.map((coord) =>
  ...
  public addClickListener(
    listener: (id: string, event: L.LeafletMouseEvent) => void,
  ): void {
   ...
   * @param listener 监听器函数
   */
  public addCreateListener(
    listener: (id: string, shape: ShapeItem) => void,
  ): void {
    ...
    try {
      ...
      if (shape.layer) {
        ...
        if (options.style) {
          ...
          if (newStyle.weight !== undefined)
            layer.setStyle({ weight: newStyle.weight });
          if (newStyle.opacity !== undefined)
            layer.setStyle({ opacity: newStyle.opacity });
          if (newStyle.fillColor)
            layer.setStyle({ fillColor: newStyle.fillColor });
          if (newStyle.fillOpacity !== undefined)
            layer.setStyle({ fillOpacity: newStyle.fillOpacity });
          if (newStyle.dashArray)
            layer.setStyle({ dashArray: newStyle.dashArray });
    ...
    try {
      ...
      const isMatch =
        ...
        (normalizedType === "square" &&
          normalizedCurrentType === "rectangle") || // 处理Square等同于Rectangle
      ...
      if (!this.drawingMode || !isMatch) {
        // 检查是否有正在编辑的图形
        if (
          this.currentEditingShape &&
          this.shapes.has(this.currentEditingShape)
        ) {
          ...
          if (editingType) {
            ...
            const editingIsMatch =
              ...
              (normalizedType === "line" &&
                normalizedEditingType === "linestring") ||
              (normalizedType === "linestring" &&
                normalizedEditingType === "line") ||
              (normalizedType === "square" &&
                normalizedEditingType === "rectangle") ||
              (normalizedType === "rectangle" &&
                normalizedEditingType === "square");

            if (editingIsMatch) {
              logger.debug(
                `发现正在编辑的匹配图形类型: ${editingType}，停止编辑`,
              );
              this.stopEditing();
              return true;
            }
  ...
  public deleteShape(id: string): boolean {
    try {
      ...
      if (shape.layer) {
        try {
          // 添加删除效果类
          const element = shape.layer.getElement

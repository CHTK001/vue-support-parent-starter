...
export class FlightLineObject {
      ...
      if (lines.length > 0) {
        ...
        const linesSeries = {
          ...
          lineStyle: {
            ...
            // 添加平滑曲线相关配置
            smooth:
              this.config.smooth !== undefined ? this.config.smooth : true,
            smoothConstraint:
              this.config.smoothConstraint !== undefined
                ? this.config.smoothConstraint
                : true,
  ...
  public addCoordinates(coordinates: Record<string, [number, number]>): void {
    ...
    logger.debug(
      `[FlightLine] 批量添加飞线点坐标: ${Object.keys(coordinates).length} 个点`,
    );
  }
  ...
  public addFlightLine(flightLine: FlightLineData): string {
    ...
    const id =
      flightLine.id ||
      `flight-line-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    if (isMultiCoords) {
      logger.debug(
        `[FlightLine] 添加多组坐标飞线，共 ${flightLine.coords.length} 组坐标`,
      );
      ...
      multiCoords.forEach((coordGroup, index) => {
        if (coordGroup.from && coordGroup.to) {
          // 添加坐标到geoCoordMap
          const fromName =
            coordGroup.fromName || flightLine.fromName || `起点${index}`;
          const toName =
            coordGroup.toName || flightLine.toName || `终点${index}`;

          if (!this.geoCoordMap[fromName] && coordGroup.from) {
            this.geoCoordMap[fromName] = coordGroup.from;
            logger.debug(
              `[FlightLine] 为多组坐标飞线起点 ${fromName} 添加地理坐标: [${coordGroup.from}]`,
            );
          }
          ...
          if (!this.geoCoordMap[toName] && coordGroup.to) {
            ...
            logger.debug(
              `[FlightLine] 为多组坐标飞线终点 ${toName} 添加地理坐标: [${coordGroup.to}]`,
            );
          }
        }
      });
      ...
      // 检查坐标是否已经在地理坐标映射中
      if (
        !this.geoCoordMap[flightLine.fromName] &&
        coords &&
        ...
        if (Array.isArray(firstCoord) && firstCoord.length >= 2) {
          ...
          logger.debug(
            `[FlightLine] 为起点 ${flightLine.fromName} 添加地理坐标: [${firstCoord}]`,
          );
        }
      }

      if (
        !this.geoCoordMap[flightLine.toName] &&
        coords &&
        Array.isArray(coords) &&
        coords.length >= 2
      ) {
        ...
        if (Array.isArray(secondCoord) && secondCoord.length >= 2) {
          ...
          logger.debug(
            `[FlightLine] 为终点 ${flightLine.toName} 添加地理坐标: [${secondCoord}]`,
          );
        }
      }
    }
    ...
    // 记录添加时间
    logger.debug(
      `[FlightLine] 添加飞线 ${id}: ${flightLine.fromName} -> ${flightLine.toName}`,
    );

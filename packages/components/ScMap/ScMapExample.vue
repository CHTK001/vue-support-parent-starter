        ...
        <div class="preview-container">
          <div class="map-table-container">
            <div class="marker-table">
              <h5>标记点列表</h5>
              <el-table :data="markers" :height="300" style="width: 100%" border>
                <el-table-column prop="position" label="位置" width="180">
                  <template #default="scope">
                    {{ scope.row.position.join(', ') }}
                  </template>
                </el-table-column>
                <el-table-column prop="title" label="标题" width="180"></el-table-column>
                <el-table-column prop="data.id" label="ID">
                  <template #default="scope">
                    {{ scope.row.data?.id || '-' }}
                  </template>
                </el-table-column>
                <el-table-column label="操作" width="150">
                  <template #default="scope">
                    <el-button type="danger" size="small" @click="removeMarker(scope.$index)">
                      删除
                    </el-button>
                  </template>
                </el-table-column>
              ...
              >
                <template #drawingTools v-if="drawingControl && showCustomTools">
                  <div class="custom-drawing-tools">
                    <el-button type="primary" size="small" @click="startDrawing('circle')">
                      <i class="el-icon-circle-plus"></i> 绘制圆形
                    </el-button>
                    <el-button type="success" size="small" @click="startDrawing('polygon')">
                      <i class="el-icon-share"></i> 绘制多边形
                    </el-button>
                    <el-button type="warning" size="small" @click="startDrawing('rectangle')">
                      <i class="el-icon-crop"></i> 绘制矩形
                    </el-button>
                    <el-button type="danger" size="small" @click="clearShapes()">
                      <i class="el-icon-delete"></i> 清除图形
                    </el-button>
                  </div>
                </template>
              </ScMap>
          ...
          <div class="marker-info mt-4" v-if="currentMarker">
...
const drawingControl = ref(false);
const showCustomTools = ref(false);
const toolsPosition = ref('right-top');
...
const removeMarker = (index) => {
  ...
  if (currentMarker.value && markers.value.findIndex(m => 
    m.position[0] === currentMarker.value.position[0] && 
    m.position[1] === currentMarker.value.position[1]) === -1) {
    currentMarker.value = null;
  }
...
const codeExample = computed(() => {
  const clusterOptionsStr = enableCluster.value ? 
    `:cluster-options="{radius: ${clusterOptions.value.radius}, minClusterSize: ${clusterOptions.value.minClusterSize}, maxZoom: ${clusterOptions.value.maxZoom}}"` : '';
...
const markers = ref(${JSON.stringify(markers.value, null, 2)});

<template>
  <div v-show="modelValue" class="rightMenu" :style="{ top: position.y + 'px', left: position.x + 'px' }">
    <ul>
      ...
      >
        <RightMenuItem2 v-if="item.type !== 'LINE'" key="RightMenuItem2" class="menu" :menu="item" :theme="theme" :top="position.y" :left="position.x" />
      </li>
    </ul>
  </div>
</template>
...
<script>
import RightMenuItem2 from "./ItemIndex2.vue";
export default {
  name: "RightMenuList",
  components: { RightMenuItem2 },
  props: {
    modelValue: {
      type: Boolean,
      default: false

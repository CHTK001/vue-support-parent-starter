...
export class CesiumObject {
  ...
  constructor(
    olMap: OlMap | null,
    cesiumBaseUrl: string = "/node_modules/cesium/Build/Cesium/",
  ) {
  ...
  public setSunLightDirection(date?: Date): boolean {
    ...
    try {
      if (scene.sun) {
        ...
        if (
          Cesium.Transforms &&
          typeof (Cesium.Transforms as any).computeSunPosition === "function"
        ) {
          // 优先使用computeSunPosition
          const sunPosition = (Cesium.Transforms as any).computeSunPosition(
            julianDate,
          );

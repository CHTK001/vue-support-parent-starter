...
// 导入拖动交互控件
import { DragBox, Translate } from "ol/interaction.js";
...
const toggleCollapse = () => {
  ...
  logger.debug(
    `[Overview] 鹰眼地图折叠状态: ${collapsed.value ? "已折叠" : "已展开"}`,
  );
};
...
const createOverviewMap = async () => {
  ...
  try {
    ...
    const updateOverviewMap = () => {
      ...
      overviewView.setZoom(
        mainView.getZoom() ? mainView.getZoom() - config.zoomOffset : 0,
      );
...
watch(
  ...
  (newVisible) => {
    logger.debug(
      `[Overview] 鹰眼地图显示状态变更: ${newVisible ? "显示" : "隐藏"}`,
    );
...
function createTileLayer() {
  ...
  if (
    ...
    config.mapType &&
    config.mapTile &&
    props.mainMapObj.createTileLayer
  ) {
    logger.debug(
      "[Overview] 使用配置的地图类型创建瓦片图层:",
      config.mapType,
      config.mapTile,
    );
    try {
      // 尝试使用与主地图相同的方法创建瓦片图层
      return props.mainMapObj.createTileLayer(
        config.mapType,
        config.mapTile,
        config.map,
      );

...
import { Style, Icon, Text, Fill, Stroke, Circle } from "ol/style.js";
...
export class MarkerObject {
  ...
  private setupClickListener(): void {
    ...
    this.clickListener = this.mapInstance.on("click", (event) => {
        ...
        (feature, layer) => {
          ...
          if (feature.get("features")) {
            ...
            if (clusterFeatures.length === 1) {
              ...
              this.log(
                "debug",
                `点击了聚合点，包含 ${clusterFeatures.length} 个标记点`,
              );
              hasHandledClick = true;
            }
          } else {
      ...
      this.log(
        "debug",
        `点击事件处理结果: ${hasHandledClick ? "处理了标记点点击" : "未处理任何标记点"}`,
      );
    });

    this.log(
      "debug",
      "标记点点击监听已设置，增加了点击容差和改进的点击处理逻辑",
    );
  }
  ...
  private createClusterStyle(feature: Feature): Style | Style[] {
    ...
    if (mapElement) {
      ...
      if (toolbarConfig) {
        ...
        showCount =
          toolbarConfig.showCount !== undefined
            ? toolbarConfig.showCount
            : showCount;
        useWeightAsSize =
          toolbarConfig.useWeightAsSize !== undefined
            ? toolbarConfig.useWeightAsSize
            : useWeightAsSize;
        ...
        enablePulse =
          toolbarConfig.enablePulse !== undefined
            ? toolbarConfig.enablePulse
            : enablePulse;
        enableAnimation =
          toolbarConfig.enableAnimation !== undefined
            ? toolbarConfig.enableAnimation
            : enableAnimation;
        ...
        pulseOpacity =
          toolbarConfig.pulseOpacity !== undefined
            ? toolbarConfig.pulseOpacity
            : pulseOpacity;
        ...
        zoomToBoundsOnClick =
          toolbarConfig.zoomToBoundsOnClick !== undefined
            ? toolbarConfig.zoomToBoundsOnClick
            : zoomToBoundsOnClick;
    ...
    if (useWeightAsSize && size > 1) {
      ...
      pointSize =
        minClusterSize + sizeScale * (maxClusterSize - minClusterSize);
    }
    ...
    if (enablePulse) {
      ...
      const breathProgress = Math.sin(
        animationProgress * Math.PI * 2 * pulseFrequency,
      );
  ...
  public setClusterMode(enabled: boolean): void {
    ...
    if (enabled) {
      ...
      if (
        ...
        this.log(
          "debug",
          `切换聚合模式时应用配置的聚合距离: ${clusterDistance} 像素`,
        );
      }
    ...
    this.markerOptions.forEach((options, id) => {
      ...
      const shouldAddToCluster =
        options.clusterMode === MarkerClusterMode.CLUSTER ||
        options.clusterMode === MarkerClusterMode.BOTH;
      const shouldAddToMarker =
        options.clusterMode === MarkerClusterMode.NONE ||
        options.clusterMode === MarkerClusterMode.BOTH;
      // 添加到相应图层
  ...
  public addMarker(options: MarkerOptions): string {
    ...
    // 如果未指定ID，生成随机ID
    const id =
      options.id || `marker-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    options.id = id;

    // 设置默认值
    options.visible = options.visible !== undefined ? options.visible : true;
    options.clickable =
      options.clickable !== undefined ? options.clickable : true;
    ...
    options.showPopover =
      options.showPopover !== undefined ? options.showPopover : false;
    ...
    if (this.mapInstance && this.config.scaleWithZoom) {
      ...
      this.log(
        "debug",
        `标记点 "${id}" 设置基准缩放级别为当前缩放级别: ${currentZoom}`,
      );
    }
    ...
    if (options.group) {
      ...
      if (
        this.groupVisibility.has(options.group) &&
        !this.groupVisibility.get(options.group)
      ) {
        options.visible = false;
        this.log(
          "debug",
          `标记点 "${id}" 所在分组 "${options.group}" 不可见，自动设置为不可见`,
        );
      }
    }
    ...
    if (options.coordSystem && options.coordSystem !== CoordSystem.EPSG3857) {
      this.log(
        "debug",
        `标记点 "${id}" 坐标从 ${options.coordSystem} 转换为 EPSG:3857`,
      );
    }
    ...
    if (shouldAddToCluster) {
      // 在聚合模式下，添加到聚合图层
      (this.clusterLayer.getSource() as Cluster)
        .getSource()!
        .addFeature(marker);
      this.log("debug", `标记点 "${id}" 已添加到聚合图层`);
    }
    ...
    this.log(
      "info",
      `添加标记点 "${id}" 成功，位置: [${options.position[0]}, ${options.position[1]}]`,
    );
  ...
  private createMarkerStyle(options: MarkerOptions): Style | Style[] {
    ...
    // 使用IconUtils创建样式
    return IconUtils.createMarkerStyleFromOptions(
      options,
      baseZoom,
      currentZoom,
    );
  ...
  public updateMarker(id: string, options: Partial<MarkerOptions>): boolean {
    ...
    if (options.group !== undefined && options.group !== oldOptions.group) {
      ...
      this.log(
        "debug",
        `标记点 "${id}" 分组从 "${oldGroup || "无"}" 更新为 "${options.group || "无"}"`,
      );
      ...
      if (
        ...
        this.log(
          "debug",
          `标记点 "${id}" 新分组 "${options.group}" 不可见，强制设置为不可见`,
        );
      }
    }
    ...
    if (options.position) {
      ...
      if (coordSystem && coordSystem !== CoordSystem.EPSG3857) {
        this.log(
          "debug",
          `标记点 "${id}" 坐标从 ${coordSystem} 转换为 EPSG:3857`,
        );
      }
      ...
      if (this.config.scaleWithZoom && this.mapInstance) {
        ...
        this.log(
          "debug",
          `标记点 "${id}" 位置更新，重置基准缩放级别为当前缩放级别: ${currentZoom}`,
        );
      }

      this.log(
        "debug",
        `标记点 "${id}" 位置已更新为 [${options.position[0]}, ${options.position[1]}]`,
      );
    }

    // 聚合模式变化，需要从一个图层移到另一个图层
    if (
      options.clusterMode !== undefined &&
      options.clusterMode !== oldOptions.clusterMode
    ) {
      // 获取图层源
      const markerSource = this.markerLayer!.getSource() as VectorSource;
      const clusterSource = this.clusterLayer!.getSource() as Cluster;
    ...
    if (options.showPopover !== undefined) {
      if (
        this.labelsVisible &&
        options.showPopover === true &&
        (newOptions.title || newOptions.content) &&
        newOptions.visible
      ) {
        // 显示popover
        this.showMarkerPopover(id);
  ...
  public getAllMarkers(): MarkerOptions[] {
    return Array.from(this.markerOptions.values()).map((option) =>
  ...
  public showAllLabels(): number {
    ...
    this.log(
      "info",
      `已显示所有标记点标签（通过popover），共影响 ${count} 个标记点`,
    );
    return count;
  }
  ...
  private showMarkerPopover(markerId: string): void {
    ...
    if (!markerOption.title && !markerOption.content) {
      this.log(
        "debug",
        `标记点 "${markerId}" 没有标题和自定义内容，无法显示popover`,
      );
      return;
    }
    ...
    if (markerOption.showPopover === false) {
      this.log(
        "debug",
        `标记点 "${markerId}" 设置了showPopover=false，不显示popover`,
      );
      return;
    }
    ...
    if (!popover) {
      // 创建新的popover
      popover = this.createMarkerPopover(
        markerId,
        markerOption.title || "",
        coordinates,
      );
    ...
    this.log(
      "debug",
      `显示标记点 "${markerId}" 的popover ${markerOption.content ? "(使用自定义内容)" : ""}`,
    );
  }
   ...
   * @param handler 处理函数
   */
  public setClusterClickHandler(
    handler?: (clusterFeatures: Feature[], coordinate: number[]) => void,
  ): void {
    // 确保地图实例存在
    if (!this.mapInstance) {
  ...
  public setMarkerGroup(id: string, group: string | null): boolean {
    ...
    this.log(
      "info",
      `标记点 "${id}" 分组从 "${oldGroup || "无"}" 更新为 "${group || "无"}"`,
    );
    return true;
  }
  ...
  public setConfig(config: MarkerConfig): void {
    ...
    if (this.mapInstance) {
      ...
      if (
        oldScaleWithZoom !== this.config.scaleWithZoom &&
        this.config.scaleWithZoom
      ) {
        // 设置缩放变化监听
        this.mapInstance.getView().on("change:resolution", () => {
          // 只有在设置了根据zoom缩放且有标记点时才更新
  ...
  public setScaleParams(params: {
    ...
    if (params.scaleWithZoom !== undefined)
      update.scaleWithZoom = params.scaleWithZoom;
  ...
  public createNavigation(fromMarkerId: string, toMarkerId: string): boolean {
    ...
    // 添加到图层
    if (this.markerLayer) {
      (this.markerLayer.getSource() as VectorSource).addFeature(
        navigationMarker,
      );
    }
    ...
    this.log(
      "info",
      `已创建从 "${fromMarkerId}" 到 "${toMarkerId}" 的导航轨迹`,
    );
    return true;
  }

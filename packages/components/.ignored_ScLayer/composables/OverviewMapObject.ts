...
export class OverviewMapObject {
  ...
  public enable(forceReinit: boolean = false): void {
    ...
    try {
      ...
      this.overviewMapControl = new OverviewMap({
        ...
        rotateWithView:
          this.options.rotateWithView !== undefined
            ? this.options.rotateWithView
            : false,
      ...
      const hasOverviewControl = controls.some(
        (control) => control instanceof OverviewMap,
      );
  ...
  public setRotateWithView(rotateWithView: boolean): void {
    ...
    if (this.enabled && this.overviewMapControl) {
      this.log(
        "debug",
        `更新鹰眼控件旋转设置: ${rotateWithView ? "跟随旋转" : "不跟随旋转"}`,
      );
      // 重新创建控件以应用新设置
      this.enable(true);
    }
  ...
  private fixOverviewMapStyle(): void {
    ...
    try {
      ...
      // 查找并修正鹰眼地图容器样式
      const overviewMapContainer = overviewElement.querySelector(
        ".ol-overviewmap-map",
      );

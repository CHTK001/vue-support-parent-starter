...
export class MapObject {
  ...
  // 自定义鼠标移动事件监听器
  private customPointerMoveListener: (
    event: any,
  ...
  // 设置鼠标移动事件监听器
  public setPointerMoveListener(
    listener: (event: any, coordinate: CoordinateInfo) => void,
  ): void {
  ...
  public init(target: HTMLElement, emitter: any): boolean {
    ...
    try {
      ...
      const view = this.createMapView(
        center,
        this.configObject.getZoom(),
        projection,
      );
      ...
      logger.info(
        "当前地图的投影:",
        this.mapInstance.getView().getProjection().getCode(),
      );
      // 根据配置设置比例尺显示状态
      ...
      if (this.mapInstance.getLayers().getLength() < 1) {
        ...
        logger.info(
          `地图共有 ${this.mapInstance.getLayers().getLength()} 个图层`,
        );
      }
   ...
   */
  private transformMapCenterToWGS84(
   ...
   */
  private configureMapInteractions(
    dragging: boolean,
  ...
  public setCenter(lat: number, lon: number, animate: boolean = true): void {
    ...
    try {
      ...
      logger.debug(
        `地图中心点已设置为: [${lat}, ${lon}]（WGS84）-> [${coordinates[0]}, ${coordinates[1]}]（EPSG:3857）`,
      );
    } catch (error) {
  ...
  public getCenter(): [number, number] | null {
    ...
    try {
      ...
      logger.debug(
        `获取地图中心点: [${center[0]}, ${center[1]}]（EPSG:3857）-> [${result[0]}, ${result[1]}]（WGS84）`,
      );
      return result;
    } catch (error) {
   ...
   */
  public setInteractions(interactions: {
    dragging?: boolean;
    scrollWheelZoom?: boolean;
  }): void {
    if (!this.mapInstance || !this.dragPan || !this.mouseWheelZoom) return;
    ...
    if (typeof interactions.scrollWheelZoom === "boolean") {
      ...
      // 更新配置对象中的滚轮缩放状态
      this.configObject.updateConfig({
   ...
   */
  public getScreenPositionFromCoordinates(
    coordinates: number[],
  ...
  private createMapView(
    ...
    logger.debug(
      `创建地图视图: 中心点=[${center[0]}, ${center[1]}], 缩放级别=${zoom}, 投影=${projection}`,
    );
  ...
  public createTileLayer(
    ...
    logger.debug(
      `创建图层 ${mapType}/${tileType}，URL: ${url}，投影: ${projection}`,
    );

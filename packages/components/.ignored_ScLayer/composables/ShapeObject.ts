...
import { Draw, Snap, Modify, Select } from "ol/interaction.js";
...
import {
  ShapeStyle,
  Shape,
  ShapeOption as ImportedShapeOption,
  ShapePoint,
  DEFAULT_SHAPE_STYLE,
} from "../types/shape";
...
export class ShapeObject {
  ...
  private drawEndCallback:
    | ((id: string, shapeType: ShapeType, feature: Feature) => void)
    | null = null;
  // 删除模式状态
   ...
   */
  public setDrawEndCallback(
    callback: (id: string, shapeType: ShapeType, feature: Feature) => void,
  ): void {
    ...
    if (this.currentType === "Circle" && geometry instanceof Polygon) {
      ...
      try {
        if (geometry instanceof Circle) {
          ...
          // @ts-ignore
          dataCoordinates = geometry.getCoordinates
  ...
  private createFeatureStyle(feature: Feature): Style | Style[] {
    ...
    const mainStyle = new Style({
      stroke: new Stroke({
        color: isEditMode
          ? "rgba(0, 120, 255, 1)"
          : styleToUse.stroke?.color || "rgba(24, 144, 255, 1)",
      ...
      fill: new Fill({
        color: isEditMode
          ? "rgba(0, 120, 255, 0.3)"
          : styleToUse.fill?.color || "rgba(24, 144, 255, 0.2)",
      }),
    });
    ...
    if (isEditMode) {
      ...
      // 通用的顶点样式创建函数
      const createVertexStyle = (coord: number[]) =>
        new Style({
      ...
      this.selectInteraction.on("select", (e) => {
        try {
          ...
          if (e.selected.length > 0) {
            ...
            this.dispatchShapeEvent(
              "shape-edit-start",
              this.editFeatureId,
              feature.get("shapeType"),
              feature,
            );
        ...
        case "Circle":
          if (geometry instanceof Circle) {
            ...
            try {
              ...
              this.log(
                "debug",
                `更新圆形数据: 中心点=${data.center}, 半径=${radius}m`,
              );
            } catch (error) {
    ...
    try {
      ...
      switch (options.type) {
        case Shape.POINT:
          ...
          feature = new Feature({
            geometry: new Point(
              GcoordUtils.convertToOlCoordinate(
                options.coordinates as [number, number],
                options.coordSystem,
              ),
            ),
          });
        ...
        case Shape.LINE:
          ...
          feature = new Feature({
            geometry: new LineString(
              GcoordUtils.convertToOlCoordinates(
                options.coordinates as Array<[number, number]>,
                options.coordSystem,
              ),
            ),
          });
        ...
        case Shape.POLYGON:
          ...
          if (
            ...
            !Array.isArray(options.coordinates[0])
          ) {
            this.log("error", "添加多边形形状失败: 坐标格式错误");
            return "";
          } else {
            coords = GcoordUtils.convertToOlCoordinates(
              options.coordinates as Array<[number, number]>,
              options.coordSystem,
            );
          }
        ...
        case Shape.CIRCLE:
          ...
          feature = new Feature({
            geometry: new Circle(
              GcoordUtils.convertToOlCoordinate(
                options.center as [number, number],
                options.coordSystem,
              ),
              options.radius,
            ),
        ...
        case Shape.RECTANGLE:
          ...
          if (options.center && options.width && options.height) {
            const center = GcoordUtils.convertToOlCoordinate(
              options.center as [number, number],
              options.coordSystem,
            );
            ...
            rectangle = [
              ...
              GcoordUtils.convertToOlCoordinate(
                [coords[1][0], coords[0][1]] as [number, number],
                options.coordSystem,
              ),
              GcoordUtils.convertToOlCoordinate(coords[1], options.coordSystem),
              GcoordUtils.convertToOlCoordinate(
                [coords[0][0], coords[1][1]] as [number, number],
                options.coordSystem,
              ),
    ...
    try {
      ...
      if (this.mapInstance && (this.mapInstance as any)._deleteClickListener) {
        try {
          this.mapInstance.un(
            "click",
            (this.mapInstance as any)._deleteClickListener,
          );

...
export default {
	dict: {
        save: {
			url: `${config.API_CONF}/v1/device/dict/save`,
			name: "保存厂家配置",
		...
		update: {
			url: `${config.API_CONF}/v1/device/dict/update`,
			name: "更新厂家配置",
		...
		delete: {
			url: `${config.API_CONF}/v1/device/dict/delete`,
			name: "删除厂家配置",
		...
		page: {
			url: `${config.API_CONF}/v1/device/dict/page`,
			name: "查询厂家信息",
        ...
        list: {
			url: `${config.API_CONF}/v1/device/dict/list`,
			name: "查询厂家信息",
			get: async function(data, config={headers:{}}){
				return await http.get(this.url , data, config);
			}

		...
		series: [
			...
			},
			{
				...
				label: {
					...
					formatter: (params) => {
						return (
							" " + state.chartData.data[params.dataIndex] + "%"
						);
					},
				},
				data: [],
...
watch(
	...
	(val, preVal) => {
		...
		if (state.chartOption.series) {
			loading.show = false;
			state.chartOption.yAxis.data = (val?.data || []).map(
				(element) => element.typeName
			);
			state.chartOption.series[0].data = (val?.data || []).map(
				(element) => parseFloat(element.usage.toFixed(2))
			);
			state.chartOption.series[2].data = (val?.data || []).map(
				(element) => {
					return {
						value: parseFloat(element.usage.toFixed(2)),
						symbolPosition: "end",
					};
				}
			);

<template>
	<div id="mountNode1" class="h-full" v-if="!loading && !noData">
		<div
			ref="containerRef"
					...
					if (res.code === "00000") {
						...
						res.data.forEach((element) => {
							this.nodes.push({
								...
								img: this.getImg(
									element?.sourceName?.toLowerCase()
								),
							});
							this.nodes.push({
								...
								width:
									(this.$refs.containerRef.offsetWidth ||
										document.getElementById("pane-0")
											.offsetWidth) - 20, // Number，必须，图的宽度
								height:
									(this.$refs.containerRef.offsetHeight ||
										document.getElementById("pane-0")
											.offsetHeight) - 10, // Number，必须，图的高度
								defaultEdge: {
									type: "line-dash",
									style: {
										lineWidth: 2,
